...
class SpyTests(unittest.TestCase):
    ...
    def test_get_remote_branches_matching_pattern(self):
        shell: Shell = SpyShell(
            [
                FakeCommand(
                    "git ls-remote --heads origin cedra-release-v*",
                    RunResult(
                        0,
                        b"ref1 refs/heads/cedra-release-v1.0\nref2 refs/heads/cedra-release-v1.1",
                    ),
                )
            ]
        )
        ...
        branches = git.get_remote_branches_matching_pattern(
            "origin", "cedra-release-v*", r"refs/heads/(cedra-release-v\d+\.\d+)"
        )
        self.assertEqual(branches, ["cedra-release-v1.0", "cedra-release-v1.1"])
