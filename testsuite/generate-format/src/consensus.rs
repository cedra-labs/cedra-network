...
fn trace_crypto_values(tracer: &mut Tracer, samples: &mut Samples) -> Result<()> {
    ...
    let secp256k1_public_key = cedra_crypto::PrivateKey::public_key(&secp256k1_private_key);
...
pub fn get_registry() -> Result<Registry> {
    ...
    tracer.trace_value(
        &mut samples,
        &cedra_consensus_types::block::Block::make_genesis_block(),
    )?;
    ...
    tracer.trace_type::<cedra_types::keyless::EphemeralCertificate>(&samples)?;
    ...
    tracer.trace_type::<cedra_consensus::quorum_store::types::BatchResponse>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::round_timeout::RoundTimeoutReason>(&samples)?;
    tracer.trace_type::<cedra_consensus::network_interface::ConsensusMsg>(&samples)?;
    tracer.trace_type::<cedra_consensus::network_interface::CommitMessage>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::block_data::BlockType>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::block_retrieval::BlockRetrievalStatus>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::payload::PayloadExecutionLimit>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::common::Payload>(&samples)?;
    tracer.trace_type::<cedra_consensus_types::block_retrieval::BlockRetrievalRequest>(&samples)?;
