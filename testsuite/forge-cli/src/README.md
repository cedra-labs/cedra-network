...
```
cargo run -p cedra-forge-cli -- --suite "run_forever" --num-validators 4 test local-swarm
```
...
```
...
2022-09-01T15:41:27.228289Z [main] INFO crates/cedra-genesis/src/builder.rs:462 Building genesis with 4 validators. Directory of output: "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ"
...
...
...
2022-09-01T15:41:28.094800Z [main] INFO testsuite/forge/src/backend/local/node.rs:129 Started node 0 (PID: 78939) with command: ".../cedra-network/target/debug/cedra-node" "-f" "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ/0/node.yaml"
...
```
kill -9 <Node 0 PID>
cargo run -p cedra-node -- -f <Location to the node 0 configuration file displayed above>
```
...
```bash
cargo run -p cedra-faucet-service -- run-simple --key <key> --node-url <node_url>
```
...
```
cargo run -p cedra-forge-cli -- --suite "run_forever" --num-validators 3 --num-validator-fullnodes 1 test local-swarm
```
...
```
cargo run -p cedra-forge-cli --help
```
