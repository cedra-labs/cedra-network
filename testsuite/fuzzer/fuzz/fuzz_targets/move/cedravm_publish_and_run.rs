...
use cedra_vm_environment::prod_configs;
...
fn run_case(mut input: RunnableState) -> Result<(), Corpus> {
    ...
    let verifier_config = prod_configs::cedra_prod_verifier_config(&Features::default());
    let deserializer_config = DeserializerConfig::new(8, 255);

    for m in input.dep_modules.iter_mut() {
        // m.metadata = vec![]; // we could optimize metadata to only contain cedra metadata
