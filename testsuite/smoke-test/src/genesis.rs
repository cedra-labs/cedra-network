...
use cedra_types::{transaction::Transaction, waypoint::Waypoint};
...
/// 3. Use the cedra CLI to generate a genesis transaction that removes the last validator from the set.
/// 4. Use the cedra-debugger to manually apply the genesis transaction to all remaining validators.
/// 5. Verify that the network is able to resume consensus and that the last validator is no longer in the set.
/// 6. Use the cedra-debugger to manually apply the genesis transaction to all VFNs.
/// 7. Verify that the VFNs are able to sync with the rest of the network.
async fn test_fullnode_genesis_transaction_flow() {
    println!("0. Building the Cedra CLI and debugger!");
    let cedra_debugger = workspace_builder::get_bin("cedra-debugger");
    ...
    let (genesis_blob_path, genesis_transaction) =
        generate_genesis_transaction(&mut swarm, cedra_cli);
    ...
    let output = Command::new(cedra_debugger.as_path())
...
/// 4. Use the cedra CLI to generate a genesis transaction that removes the last validator from the set.
/// 5. Use the cedra-debugger to manually apply the genesis transaction to all remaining validators.
...
fn parse_waypoint(bootstrap_command_output: &str) -> Waypoint {
    let waypoint = Regex::new(r"Got waypoint: (\d+:\w+)")
        ...
        .ok_or_else(|| {
            anyhow!("Failed to parse `cedra-debugger cedra-db bootstrap` waypoint output!")
        });
