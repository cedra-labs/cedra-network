...
use cedra_backup_cli::metadata::view::BackupStorageState;
use cedra_forge::{reconfig, CedraPublicInfo, Node, NodeExt, Swarm, SwarmExt};
use cedra_logger::info;
use cedra_temppath::TempPath;
use cedra_types::{transaction::Version, waypoint::Waypoint};
...
async fn test_db_restore() {
    ...
    workspace_builder::get_bin("cedra-debugger");
    info!("---------- 1. pre-building finished.");

    let mut swarm = SwarmBuilder::new_local(4).with_cedra().build().await;
...
fn db_backup_verify(backup_path: &Path, trusted_waypoints: &[Waypoint]) {
    info!("---------- running cedra-debugger cedra-db backup-verify");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("cedra-debugger");
    let metadata_cache_path = TempPath::new();

    metadata_cache_path.create_as_dir().unwrap();

    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["cedra-db", "backup", "verify"]);
...
fn replay_verify(backup_path: &Path, trusted_waypoints: &[Waypoint]) {
    ...
    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["cedra-db", "replay-verify"]);
    trusted_waypoints.iter().for_each(|w| {
        cmd.arg("--trust-waypoint");
        cmd.arg(&w.to_string());
    });
...
pub(crate) fn db_backup(
    ...
    info!("---------- running cedra db tool backup");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("cedra-debugger");
...
pub(crate) fn db_restore(
    ...
    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["cedra-db", "restore", "bootstrap-db"]);
    trusted_waypoints.iter().for_each(|w| {
        cmd.arg("--trust-waypoint");
        cmd.arg(&w.to_string());
    });
...
async fn do_transfer_or_reconfig(info: &mut CedraPublicInfo) -> Result<()> {
...
async fn do_transfers_and_reconfigs(mut info: CedraPublicInfo, quit_flag: Arc<AtomicBool>) {
    // loop until aborted
...
async fn test_db_restart() {
    ...
    let mut pub_chain_info = CedraPublicInfo::new(
        chain_info.chain_id(),
