...
use cedra_gas_algebra::GasQuantity;
use cedra_gas_schedule::{CedraGasParameters, InitialGasSchedule, ToOnChainGasSchedule};
use cedra_release_builder::{
    components::{
...
use cedra_types::on_chain_config::{FeatureFlag as CedraFeatureFlag, OnChainConsensusConfig};
...
#[tokio::test]
/// This test verifies the flow of cedra framework upgrade process.
/// i.e: The network will be alive after applying the new cedra framework release.
async fn test_upgrade_flow() {
    // prebuild tools.
    let cedra_cli = workspace_builder::get_bin("cedra");
    ...
    let url = env.cedra_public_info().url().to_string();
    let private_key = env
        .cedra_public_info()
        .root_account()
        .private_key()
    ...
    // TODO: Replace this logic with cedra-gas
    let mut gas_parameters = CedraGasParameters::initial();
    gas_parameters.vm.txn.max_transaction_size_in_bytes = GasQuantity::new(100_000_000);

    let gas_schedule = cedra_types::on_chain_config::GasScheduleV2 {
    ...
    assert!(Command::new(cedra_cli.as_path())
    ...
    let config = cedra_release_builder::ReleaseConfig {
            ...
            Proposal {
                ...
                update_sequence: vec![
                    ReleaseEntry::FeatureFlag(Features {
                        enabled: CedraFeatureFlag::default_features()
                            .into_iter()
    ...
    let base_path_v1 = base_dir.join("src/cedra/package_publish_modules_v1/");

    move_test_helpers::publish_package(&mut env.cedra_public_info(), base_path_v1)
        .await
        .unwrap();
...
// to repeatedly compile all the cedra framework pacakges as dependency
//
#[ignore]
...
async fn test_release_validate_tool_multi_step() {
    let (mut env, _, _) = SwarmBuilder::new_local(1)
        ...
        .build_with_cli(2)
        .await;
    let config = cedra_release_builder::ReleaseConfig::default();
    ...
    let network_config = cedra_release_builder::validate::NetworkConfig {
    ...
    cedra_release_builder::validate::validate_config(config, network_config)
        .await
        .unwrap();

    let root_account = env.cedra_public_info().root_account().address();
    // Test the module publishing workflow
