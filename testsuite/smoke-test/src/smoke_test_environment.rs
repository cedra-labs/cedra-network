...
use cedra_config::{config::NodeConfig, keys::ConfigKey, utils::get_available_port};
use cedra_crypto::ed25519::Ed25519PrivateKey;
use cedra_faucet_core::server::{FunderKeyEnum, RunConfig};
use cedra_forge::{ActiveNodesGuard, Factory, LocalFactory, LocalSwarm, Node};
use cedra_framework::ReleaseBundle;
use cedra_genesis::builder::{InitConfigFn, InitGenesisConfigFn, InitGenesisStakeFn};
...
impl SwarmBuilder {
    ...
    pub fn with_cedra(mut self) -> Self {
        self.genesis_framework = Some(cedra_cached_packages::head_release_bundle().clone());
        self
    }

    pub fn with_cedra_testnet(mut self) -> Self {
        self.genesis_framework = Some(cedra_framework::testnet_release_bundle().clone());
        self
    }
...
// Gas is not enabled with this setup, it's enabled via forge instance.
pub async fn new_local_swarm_with_cedra(num_validators: usize) -> LocalSwarm {
