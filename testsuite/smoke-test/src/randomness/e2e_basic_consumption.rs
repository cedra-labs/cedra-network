...
use cedra::{common::types::GasOptions, move_tool::MemberId, test::CliTestFramework};
...
pub async fn publish_on_chain_dice_module(
    ...
    cli.init_package(
        ...
        package_addresses,
        Some(CliTestFramework::cedra_framework_dir()),
    )
    .await
    .unwrap();

    let content =
        include_str!("../../../../cedra-move/move-examples/on_chain_dice/sources/dice.move")
            .to_string();
