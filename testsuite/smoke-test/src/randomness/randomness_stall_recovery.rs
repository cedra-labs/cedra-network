...
use cedra_config::config::{OverrideNodeConfig, PersistableConfig};
use cedra_forge::{NodeExt, Swarm, SwarmExt};
use cedra_logger::{debug, info};
use cedra_types::{on_chain_config::OnChainRandomnessConfig, randomness::PerBlockRandomness};
use std::{
    ops::Add,
    sync::Arc,
...
script {
    ...
    fun main(core_resources: &signer) {
        let framework_signer = cedra_governance::get_signer_testnet_only(core_resources, @0x1);
        randomness_config_seqnum::set_for_next_epoch(&framework_signer, 2);
        cedra_governance::force_end_epoch(&framework_signer); // reconfigure() won't work at the moment.
    }
}
    "#;
