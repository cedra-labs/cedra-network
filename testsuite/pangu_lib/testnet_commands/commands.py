...
@click.option(
    "--framework-path",
    required=True,
    help="Pass in the path to the compiled move framework (head.mrb, or framework.mrb) file. To compile it, run: $ cargo run --locked --package cedra-framework -- release",
)
@click.option(
    "--num-of-validators",
    default=10,
...
@click.option(
    "--cedra-cli-path",
    default="cedra",
    help="Pass the path to the cedra CLI if it is not in your $PATH var.",
)
@click.option(
    "--dry-run",
...
def create(
    ...
    Args:
        ...
        cedra_cli_path (str): path to cedra cli
