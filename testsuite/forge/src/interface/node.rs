...
use anyhow::anyhow;
use cedra_config::{config::NodeConfig, network_id::NetworkId};
use cedra_inspection_service::inspection_client::InspectionClient;
use cedra_rest_client::{CedraBaseUrl, Client as RestClient};
...
pub trait NodeExt: Node {
    ...
    fn rest_client_with_timeout(&self, timeout: Duration) -> RestClient {
        RestClient::builder(CedraBaseUrl::Custom(self.rest_api_endpoint()))
            .timeout(timeout)
            .build()
    }
    ...
    async fn get_connected_peers(
        ...
        self.get_metric_with_fields_i64("cedra_connections", map)
            .await
    }
