...
/// at _forge_ compile time, decide what kind of build we will use for `cedra-node`
pub fn use_release() -> bool {
    option_env!("LOCAL_SWARM_NODE_RELEASE").is_some()
}

/// at _forge_ compile time, decide to build `cedra-node` only for consensus perf tests
pub fn build_consensus_only_node() -> bool {
    option_env!("CONSENSUS_ONLY_PERF_TEST").is_some()
}

/// at forge _run_ time, compile `cedra-node` without indexer
pub fn build_cedra_node_without_indexer() -> bool {
...
/// Get the cedra node binary from the current working directory
pub fn get_cedra_node_binary_from_worktree() -> Result<(String, PathBuf)> {
    ...
    let bin_path = cargo_build_cedra_node(&metadata.workspace_root, &metadata.target_directory)?;

    Ok((revision, bin_path))
}

/// This function will attempt to build the cedra-node binary at an arbitrary revision.
/// Using the `target/forge` as a working directory it will do the following:
///     1. Look for a binary named `cedra-node--<revision>`, if it already exists return it
...
///        binary and then move it to `target/forge/cedra-node--<revision>`
pub fn get_cedra_node_binary_at_revision(revision: &str) -> Result<(String, PathBuf)> {
    let metadata = metadata()?;
    ...
    let cedra_node_bin = forge_directory.join(format!(
        "cedra-node--{}{}",
        revision,
        env::consts::EXE_SUFFIX
    ));

    if cedra_node_bin.exists() {
        return Ok((revision, cedra_node_bin));
    }
    ...
    fs::rename(
        cargo_build_cedra_node(&checkout_dir, &forge_target_directory)?,
        &cedra_node_bin,
    )?;

    let _ = fs::remove_dir_all(&checkout_dir);

    Ok((revision, cedra_node_bin))
}
...
pub fn cargo_build_common_args() -> Vec<&'static str> {
    let mut args = if build_cedra_node_without_indexer() {
...
fn cargo_build_cedra_node<D, T>(directory: D, target_directory: T) -> Result<PathBuf>
where
    D: AsRef<Path>,
    T: AsRef<Path>,
{
    ...
    // build the cedra-node package directly to avoid feature unification issues
    args.push("--package=cedra-node");
    ...
    let output = Command::new("cargo")
        ...
        .args(&args)
        .output()
        .context("Failed to build cedra-node")?;
    ...
    if output.status.success() {
        ...
        if !bin_path.exists() {
            bail!(
                "Can't find binary cedra-node at expected path {:?}",
                bin_path
            );
        }
        ...
        bail!(
            "Failed to build cedra-node: 'cd {} && CARGO_TARGET_DIR={} cargo build --bin=cedra-node",
            directory.display(),
