...
use cedra_db::common::{LEDGER_DB_NAME, STATE_MERKLE_DB_NAME};
use cedra_logger::info;
use cedra_rest_client::Client as RestClient;
use cedra_sdk::types::PeerId;
use cedra_state_sync_driver::metadata_storage::STATE_SYNC_DB_NAME;
...
const CEDRA_DATA_DIR: &str = "/opt/cedra/data";
...
impl Node for K8sNode {
    ...
    async fn clear_storage(&self) -> Result<()> {
        ...
        let ledger_db_path = format!("{}/db/{}", CEDRA_DATA_DIR, LEDGER_DB_NAME);
        let state_db_path = format!("{}/db/{}", CEDRA_DATA_DIR, STATE_MERKLE_DB_NAME);
        let state_sync_db_path = format!("{}/db/{}", CEDRA_DATA_DIR, STATE_SYNC_DB_NAME);
