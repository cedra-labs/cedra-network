...
use crate::{
    ...
    NodeConfigFn, ReadWrite, Result, CEDRA_NODE_HELM_RELEASE_NAME, DEFAULT_ROOT_KEY,
...
async fn wait_node_haproxy(
    ...
    cedra_retrier::retry_async(k8s_wait_nodes_strategy(), || {
        ...
        Box::pin(async move {
            for i in 0..num_haproxy {
                let haproxy_deployment_name =
                    format!("{}-{}-haproxy", CEDRA_NODE_HELM_RELEASE_NAME, i);
...
/// Deletes a collection of resources in k8s as part of cedra-node
...
pub(crate) async fn delete_k8s_resources(client: K8sClient, kube_namespace: &str) -> Result<()> {
    ...
    let cedra_node_helm_selector = "app.kubernetes.io/part-of=cedra-node";
    let testnet_addons_helm_selector = "app.kubernetes.io/part-of=testnet-addons";
    let genesis_helm_selector = "app.kubernetes.io/part-of=cedra-genesis";
    ...
    // cross this with all the compute resources created by cedra-node helm chart
...
pub async fn uninstall_testnet_resources(kube_namespace: String) -> Result<()> {
    ...
    info!(
        "cedra-node resources for Forge removed in namespace: {}",
        kube_namespace
    );
...
fn get_node_default_helm_path() -> String {
    match ForgeRunnerMode::try_from_env().unwrap_or(ForgeRunnerMode::K8s) {
        ...
        ForgeRunnerMode::K8s => "/cedra/terraform/cedra-node-default-values.yaml",
    }
    .to_string()
}
...
/// Installs a testnet in a k8s namespace by first running genesis, and the installing the cedra-nodes via helm
...
pub async fn install_testnet_resources(
    ...
    let mut cedra_node_helm_values =
        get_default_helm_release_values_from_cluster(CEDRA_NODE_HELM_RELEASE_NAME)
    ...
    let cedra_node_helm_values_override = construct_node_helm_values_from_input(
        node_helm_config_fn,
    ...
    info!(
        "cedra_node_helm_values: {}",
        serde_yaml::to_string(&cedra_node_helm_values).unwrap()
    );
    info!(
        "cedra_node_helm_values_override: {}",
        serde_yaml::to_string(&cedra_node_helm_values_override).unwrap()
    );

    merge_yaml(&mut cedra_node_helm_values, cedra_node_helm_values_override);
    merge_yaml(&mut genesis_helm_values, genesis_helm_values_override);

    info!(
        "cedra_node_helm_values after override: {}",
        serde_yaml::to_string(&cedra_node_helm_values).unwrap(),
    );

    // disable uploading genesis to blob storage since indexer requires it in the cluster
    if enable_indexer {
        cedra_node_helm_values["genesis_blob_upload_url"] = "".into();
    }
    ...
    let config: serde_json::Value = serde_json::from_value(serde_json::json!({
        ...
        "testnet-values": cedra_node_helm_values,
