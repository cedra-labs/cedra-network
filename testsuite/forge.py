...
GAR_REPO_NAME = "us-docker.pkg.dev/cedra-registry/docker"
...
# o11y resources
GRAFANA_BASE_URL = "https://cedralabs.grafana.net/d/overview/overview?orgId=1&refresh=10s&var-Datasource=VictoriaMetrics%20Global%20%28Non-mainnet%29&var-BigQuery=Google%20BigQuery"

# helm chart default override values
HELM_CHARTS = ["cedra-node", "cedra-genesis"]
...
class ForgeContext:
    ...
    def forge_chain_name(self) -> str:
        forge_chain_name = self.forge_cluster.name.lstrip("cedra-")
...
def get_cpu_profile_link(
    ...
    base_url = "https://grafana.cedralabs.com/a/grafana-pyroscope-app/profiles-explorer"
...
def get_humio_link_for_test_runner_logs(
    ...
    columns = [
        ...
        },
        {
            ...
            "openInNewBrowserTab": "***",
            "style": "button",
            "hrefTemplate": 'https://github.com/cedra-labs/cedra-network/pull/{{fields["github_pr"]}}',
...
def get_axiom_link_for_test_runner_logs(
    ...
    logs_url = f"https://app.axiom.co/cedralabs-hghf/explorer?initForm={urlquote( json.dumps({ 'apl': apl_query, 'queryOptions': apply_axiom_time_filter(time_filter), }) )}"
...
def get_current_cluster_name(shell: Shell) -> str:
    ...
    matches = re.findall(r"cedra.*", current_context)
    if len(matches) != 1:
...
def test(
    ...
    # forge_cluster_name = "cedra-forge-0"
