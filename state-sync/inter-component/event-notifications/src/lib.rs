...
impl EventSubscriptionService {
    ...
    pub fn subscribe_to_events(
        ...
        let (notification_sender, notification_receiver) =
            cedra_channel::new(QueueStyle::KLAST, EVENT_NOTIFICATION_CHANNEL_SIZE, None);
    ...
    pub fn subscribe_to_reconfigurations(
        ...
        let (notification_sender, notification_receiver) =
            cedra_channel::new(QueueStyle::KLAST, RECONFIG_NOTIFICATION_CHANNEL_SIZE, None);
...
struct EventSubscription {
    ...
    pub notification_sender: cedra_channels::cedra_channel::Sender<(), EventNotification>,
}
...
struct ReconfigSubscription {
    pub notification_sender:
        cedra_channels::cedra_channel::Sender<(), ReconfigNotification<DbBackedOnChainConfig>>,
}
...
impl OnChainConfigProvider for DbBackedOnChainConfig {
    fn get<T: OnChainConfig>(&self) -> Result<T> {
        let bytes = self
            ...
            .ok_or_else(|| {
                anyhow!(
                    "no config {} found in cedra root account state",
                    T::CONFIG_ID
                )
            })?
            .bytes()
...
pub struct NotificationListener<T> {
    pub notification_receiver: cedra_channels::cedra_channel::Receiver<(), T>,
}
