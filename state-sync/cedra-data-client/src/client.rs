...
use cedra_config::{
    config::{CedraDataClientConfig, BaseConfig},
    network_id::PeerNetworkId,
};
use cedra_id_generator::{IdGenerator, U64IdGenerator};
use cedra_infallible::Mutex;
use cedra_logger::{info, sample, sample::SampleRate, trace, warn};
use cedra_network::{
...
/// An [`CedraDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over CedraNet.
///
/// The `CedraDataClient`:
///
/// 1. Sends requests to connected Cedra peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct CedraDataClient {
    /// The base config of the node.
    base_config: Arc<BaseConfig>,
    /// The config for the CedraNet data client.
    data_client_config: Arc<CedraDataClientConfig>,
    /// The underlying CedraNet storage service client.
...
impl CedraDataClient {
        ...
        E: Into<Error>,
    {
        ...
        for peer in peers {
            ...
            let cedra_data_client = self.clone();
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = CedraNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    cedra_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    cedra_storage_service_client::Error::StorageServiceError(err) => {
...
#[async_trait]
impl CedraDataClientInterface for CedraDataClient {
...
/// The CedraNet-specific request context needed to update a peer's scoring.
struct CedraNetResponseCallback {
    data_client: CedraDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for CedraNetResponseCallback {
...
impl fmt::Debug for CedraNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CedraNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
