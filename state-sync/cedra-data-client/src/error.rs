...
/// An error returned by the Cedra Data Client for failed API calls.
...
impl From<cedra_storage_service_client::Error> for Error {
    fn from(error: cedra_storage_service_client::Error) -> Self {
        Self::UnexpectedErrorEncountered(error.to_string())
    }
}

impl From<cedra_storage_service_types::responses::Error> for Error {
    fn from(error: cedra_storage_service_types::responses::Error) -> Self {
