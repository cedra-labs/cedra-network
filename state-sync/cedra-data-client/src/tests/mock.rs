...
use crate::{
    client::CedraDataClient,
    error::Result,
    global_summary::GlobalDataSummary,
    interface::{CedraDataClientInterface, Response, SubscriptionRequestMetadata},
    poller::DataSummaryPoller,
    priority::PeerPriority,
};
use cedra_channels::{cedra_channel, message_queues::QueueStyle};
use cedra_config::{
    config::{CedraDataClientConfig, BaseConfig, RoleType},
...
use cedra_storage_service_server::network::{NetworkRequest, ResponseSender};
...
pub struct MockNetwork {
    ...
    peer_mgr_reqs_rxs:
        HashMap<NetworkId, cedra_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>, // The peer manager request receivers
}

impl MockNetwork {
    pub fn new(
        base_config: Option<BaseConfig>,
        data_client_config: Option<CedraDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, CedraDataClient, DataSummaryPoller) {
        ...
        let (client, poller) = CedraDataClient::new(
            data_client_config,
            base_config.clone(),
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn CedraDataClientInterface + Send + Sync> {
    Arc::new(MockCedraDataClient::new())
}

// This automatically creates a MockCedraDataClient
mock! {
    pub CedraDataClient {}
...
impl DbReader for MockDatabaseReader {
    fn get_block_timestamp(&self, version: Version) -> cedra_storage_interface::Result<u64> {
        Ok(version * 100_000)
    }
}
