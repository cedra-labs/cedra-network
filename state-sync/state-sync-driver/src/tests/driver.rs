...
use cedra_config::config::{NodeConfig, RoleType, StateSyncDriverConfig};
use cedra_consensus_notifications::{ConsensusNotificationSender, ConsensusNotifier};
...
async fn create_driver_for_tests(
    ...
    // Create test cedra database
    let db_path = cedra_temppath::TempPath::new();
    db_path.create_as_dir().unwrap();
    let (_, db_rw) = DbReaderWriter::wrap(CedraDB::new_for_test(db_path.path()));

    // Bootstrap the genesis transaction
    let (genesis, _) = cedra_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    bootstrap_genesis::<CedraVMBlockExecutor>(&db_rw, &genesis_txn).unwrap();
    ...
    let (consensus_notifier, consensus_listener) =
        cedra_consensus_notifications::new_consensus_notifier_listener_pair(5000);
