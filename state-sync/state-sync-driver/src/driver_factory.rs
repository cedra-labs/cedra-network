...
use cedra_consensus_notifications::ConsensusNotificationListener;
use cedra_data_client::client::CedraDataClient;
use cedra_data_streaming_service::streaming_client::StreamingServiceClient;
use cedra_event_notifications::{EventNotificationSender, EventSubscriptionService};
use cedra_executor_types::ChunkExecutorTrait;
...
impl DriverFactory {
    ...
    >(
        ...
        let driver_runtime = if create_runtime {
            let runtime = cedra_runtimes::spawn_named_runtime("sync-driver".into(), None);
            Some(runtime)
        } else {
            None
        };
...
impl StateSyncRuntimes {
    pub fn new(
        cedra_data_client: Runtime,
        state_sync: DriverFactory,
        storage_service: Runtime,
        streaming_service: Runtime,
    ) -> Self {
        Self {
            _cedra_data_client: cedra_data_client,
            state_sync,
