...
use crate::{
    ...
    tests::utils::{
        ...
        initialize_logger, MockCedraDataClient, NoopResponseCallback, MAX_ADVERTISED_EPOCH_END,
...
use cedra_config::config::{
    CedraDataClientConfig, DataStreamingServiceConfig, DynamicPrefetchingConfig,
};
...
async fn test_stream_data_error() {
    ...
    let pending_response = PendingClientResponse::new_with_response(
        client_request.clone(),
        Err(cedra_data_client::error::Error::DataIsUnavailable(
            "Missing data!".into(),
        )),
    );
    insert_response_into_pending_queue(&mut data_stream, pending_response);
...
async fn test_continuous_stream_optimistic_fetch_timeout() {
    ...
    let data_client_config = CedraDataClientConfig {
...
async fn advertise_new_data_and_verify_requests(
    data_stream: &mut DataStream<MockCedraDataClient>,
...
fn create_state_value_stream(
    ...
    version: Version,
) -> (DataStream<MockCedraDataClient>, DataStreamListener) {
...
fn create_continuous_transaction_output_stream(
    ...
    DataStream<MockCedraDataClient>,
    DataStreamListener,
    TimeService,
) {
...
fn create_data_stream(
    ...
    // Create an cedra data client mock and notification generator
    let cedra_data_client = MockCedraDataClient::new(data_client_config, true, false, true, false);
...
fn create_stream_update_notifier() -> cedra_channel::Sender<(), StreamUpdateNotification> {
    let (stream_update_notifier, _) = cedra_channel::new(QueueStyle::LIFO, 1, None);
...
fn set_failure_response_in_queue(data_stream: &mut DataStream<MockCedraDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        cedra_data_client::error::Error::UnexpectedErrorEncountered("Oops!".into()),
    );
}

/// Sets the client response at the index in the pending
/// queue to contain a pending response.
fn set_pending_response_in_queue(data_stream: &mut DataStream<MockCedraDataClient>, index: usize) {
...
fn set_timeout_response_in_queue(data_stream: &mut DataStream<MockCedraDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        cedra_data_client::error::Error::TimeoutWaitingForResponse("Timed out!".into()),
    );
}
...
fn set_response_in_queue(
    data_stream: &mut DataStream<MockCedraDataClient>,
    index: usize,
    error_response: cedra_data_client::error::Error,
) {
    // Get the pending response at the specified index
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
    let pending_response = sent_requests.as_mut().unwrap().get_mut(index).unwrap();
...
async fn wait_for_data_client_to_respond(
    ...
    loop {
        if let Some(client_response) = &pending_response.lock().client_response {
            if !matches!(
                client_response,
                Err(cedra_data_client::error::Error::TimeoutWaitingForResponse(
                    _
                ))
            ) {
                return;
...
fn set_transaction_response_at_queue_head(data_stream: &mut DataStream<MockCedraDataClient>) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
