...
use cedra_data_client::{
    ...
    interface::CedraDataClientInterface,
};
...
pub struct DataStreamingService<T> {
    ...
    stream_update_listener: cedra_channel::Receiver<(), StreamUpdateNotification>,
...
impl<T: CedraDataClientInterface + Send + Clone + 'static> DataStreamingService<T> {
    pub fn new(
        data_client_config: CedraDataClientConfig,
        streaming_service_config: DataStreamingServiceConfig,
        cedra_data_client: T,
        ...
        let (stream_update_notifier, stream_update_listener) =
            cedra_channel::new(QueueStyle::LIFO, STREAM_PROGRESS_UPDATE_CHANNEL_SIZE, None);
...
fn spawn_global_data_summary_refresher<T: CedraDataClientInterface + Send + Clone + 'static>(
...
fn refresh_global_data_summary<T: CedraDataClientInterface + Send + Clone + 'static>(
    cedra_data_client: T,
    cached_global_data_summary: Arc<ArcSwap<GlobalDataSummary>>,
) {
    // Fetch the global data summary and update the cache
    match fetch_global_data_summary(cedra_data_client) {
...
fn fetch_global_data_summary<T: CedraDataClientInterface + Send + Clone + 'static>(
    cedra_data_client: T,
) -> Result<GlobalDataSummary, Error> {
    // Fetch the global data summary from the data client
    let global_data_summary = cedra_data_client.get_global_data_summary();
...
fn verify_optimal_chunk_sizes(optimal_chunk_sizes: &OptimalChunkSizes) -> Result<(), Error> {
    ...
    {
        Err(Error::CedraDataClientResponseIsInvalid(format!(
