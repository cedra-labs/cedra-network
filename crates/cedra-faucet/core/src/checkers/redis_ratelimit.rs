...
use crate::{
    endpoints::{CedraTapError, CedraTapErrorCode, RejectionReason, RejectionReasonCode},
...
impl RatelimitKeyProvider {
    ...
    /// JWT's `sub` field).
    pub async fn ratelimit_key_value(&self, data: &CheckerData) -> Result<String, CedraTapError> {
        match self {
...
/// worry about it as a followup: https://github.com/cedra-labs/cedra-tap/issues/15.
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, CedraTapError> {
        self.db_pool.get().await.map_err(|e| {
            CedraTapError::new_with_error_code(
                format!("Failed to connect to redis storage: {}", e),
                CedraTapErrorCode::StorageError,
            )
        })
    }
...
impl CheckerTrait for RedisRatelimitChecker {
    async fn check(
        ...
        let mut conn = self
            ...
            .await
            .map_err(|e| CedraTapError::new_with_error_code(e, CedraTapErrorCode::StorageError))?;
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), CedraTapError> {
