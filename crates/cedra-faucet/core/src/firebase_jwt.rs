...
impl FirebaseJwtVerifier {
    ...
    pub async fn validate_jwt(&self, headers: Arc<HeaderMap>) -> Result<String, CedraTapError> {
        ...
        let token_data = match verify.await {
            ...
            None => {
                return Err(CedraTapError::new(
                    ...
                    CedraTapErrorCode::AuthTokenInvalid,
                ));
            },
        };
...
pub fn jwt_sub(headers: Arc<HeaderMap>) -> Result<String, CedraTapError> {
    let is_jwt = headers
        .get(X_IS_JWT_HEADER)
