...
use cedra_faucet_metrics_server::{run_metrics_server, MetricsServerConfig};
use cedra_logger::info;
use cedra_sdk::{
    crypto::ed25519::Ed25519PrivateKey,
    types::{account_config::cedra_test_root_address, chain_id::ChainId},
};
use clap::Parser;
    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(cedra_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            CedraTapError, CedraTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn start_server(config_content: &'static str) -> Result<(u16, JoinHandle<Result<()>>)> {
        ...
        let port = cedra_config::utils::get_available_port();
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let cedra_error = CedraTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CedraError");
        assert!(!cedra_error.rejection_reasons.is_empty());
        ...
        let cedra_error = CedraTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CedraError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = cedra_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let cedra_node_api_client = cedra_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        cedra_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = cedra_sdk::bcs::to_bytes(&private_key)?;
        ...
        let cedra_error = CedraTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CedraError");
        assert_eq!(
            cedra_error.error_code,
            CedraTapErrorCode::FunderAccountProblem
        );
