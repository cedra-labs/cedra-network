...
use super::errors::CedraTapErrorResponse;
...
/// be a JSON representation of CedraTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an CedraTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    CedraTapErrorResponse::from(CedraTapError::new_with_error_code(
        error_string,
        CedraTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}
