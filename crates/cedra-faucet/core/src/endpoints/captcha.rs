...
use super::{errors::CedraTapErrorResponse, ApiTags, CedraTapError, CedraTapErrorCode};
...
impl CaptchaApi {
    ...
    async fn request_captcha(&self) -> Result<Response<Binary<Vec<u8>>>, CedraTapErrorResponse> {
        if !self.enabled {
            return Err(CedraTapError::new(
                "The CaptchaChecker is not enabled".to_string(),
                CedraTapErrorCode::EndpointNotEnabled,
            )
            .into());
        }
        ...
        let (key, image) = match captcha_manager.create_challenge() {
            ...
            Err(e) => {
                return Err(
                    CedraTapError::new_with_error_code(e, CedraTapErrorCode::CheckerError).into(),
                );
            },
        };
