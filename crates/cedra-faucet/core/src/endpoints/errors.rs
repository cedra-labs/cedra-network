...
pub struct CedraTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: CedraTapErrorCode,
...
impl CedraTapError {
    pub fn new(message: String, error_code: CedraTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: CedraTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
...
impl std::fmt::Display for CedraTapError {
...
impl From<CedraTapError> for CedraTapErrorResponse {
    fn from(error: CedraTapError) -> Self {
...
pub enum CedraTapErrorCode {
    ...
    /// Failed when making requests to the Cedra API.
    CedraApiError = 50,
...
impl CedraTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            CedraTapErrorCode::InvalidRequest
            | CedraTapErrorCode::AccountDoesNotExist
            | CedraTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            CedraTapErrorCode::Rejected
            | CedraTapErrorCode::SourceIpMissing
            | CedraTapErrorCode::TransactionFailed
            | CedraTapErrorCode::AuthTokenInvalid => StatusCode::FORBIDDEN,
            CedraTapErrorCode::CedraApiError
            | CedraTapErrorCode::TransactionTimedOut
            | CedraTapErrorCode::SerializationError
            | CedraTapErrorCode::BypasserError
            | CedraTapErrorCode::CheckerError
            | CedraTapErrorCode::StorageError => StatusCode::INTERNAL_SERVER_ERROR,
            CedraTapErrorCode::ServerOverloaded | CedraTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            CedraTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            CedraTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
