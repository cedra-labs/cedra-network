...
static MINTER_SCRIPT: &[u8] = include_bytes!(
    "../../../../../cedra-move/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
);
...
impl MintFunder {
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, CedraTapError> {
        match self.txn_config.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    CedraTapError::new_with_error_code(e, CedraTapErrorCode::CedraApiError)
                }),
        }
    }

    async fn get_transaction_factory(&self) -> Result<TransactionFactory, CedraTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        // Delegate minting to the account
        {
            ...
            client
                .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                    transaction_factory.payload(cedra_stdlib::cedra_coin_delegate_mint_capability(
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(cedra_stdlib::cedra_coin_claim_mint_capability()),
            ))
            .await
