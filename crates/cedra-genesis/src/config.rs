...
use cedra_crypto::{bls12381, ed25519::Ed25519PublicKey, x25519};
use cedra_types::{
    ...
    on_chain_config::{
        default_oracle_address, OnChainConsensusConfig, OnChainExecutionConfig,
...
pub struct Layout {
    ...
    #[serde(default = "default_oracle_address")]
        ...
        for (i, pool) in map.inner.into_iter().enumerate() {
            ...
            if pool.validator.join_during_genesis {
                ...
                if pool.validator.stake_amount < 100000000000000 {
                    errors.push(anyhow::anyhow!(
                        "Employee pool #{} is setup to join during genesis but has a low stake amount {} < 1000000 Cedra",
                        i,
