...
use cedra_vm_genesis::{AccountBalance, EmployeePool, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        cedra_vm_genesis::encode_cedra_mainnet_genesis_transaction(
            &self.accounts,
            &self.employee_vesting_accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &cedra_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let db_rw = DbReaderWriter::new(cedradb);
        cedra_executor::db_bootstrapper::generate_waypoint::<CedraVMBlockExecutor>(&db_rw, genesis)
    }
}
