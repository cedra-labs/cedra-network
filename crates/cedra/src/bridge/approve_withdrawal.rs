...
pub struct ApproveWithdrawal {
    /// Profile that signs the tx (must be the multisig executor)
    #[arg(long, default_value = "multisig")]
    pub profile: String,

    /// Framework package address that contains cedra_framework::bridge (e.g. 0x1)
    #[arg(long, value_name = "0x....")]
    pub framework: String,

    /// User to be approved to withdraw
    #[arg(long, value_name = "0x....")]
    pub user: String,
...
impl ApproveWithdrawal {
    ...
    pub async fn execute_serialized(self) -> CliResult {
        ...
        let func = format!("{}::bridge::approve_withdrawal", self.framework);
        ...
        // Move signature:
        // approve_withdrawal(multisig, user: address, l1_token: vector<u8>,
        //                    eth_recipient: vector<u8>, amount: u64, nonce: u64)
        let status = Command::new(cli)
            .arg("move").arg("run")
            .arg("--profile").arg(&self.profile)
            .arg("--function-id").arg(func)
            .arg("--assume-yes")
            .arg("--args").arg(format!("address:{}", self.user))
        ...
        if !status.success() {
            return Err("approve_withdrawal failed".to_string());
        }
