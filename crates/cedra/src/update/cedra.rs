...
impl BinaryUpdater for CedraUpdateTool {
    ...
    fn get_update_info(&self) -> Result<UpdateRequiredInfo> {
        ...
        let latest_release = loop {
            ...
            if release.version.starts_with("cedra-cli-") {
                break release;
            }
        };
    ...
    fn build_updater(&self, info: &UpdateRequiredInfo) -> Result<Box<dyn ReleaseUpdate>> {
        ...
        match installation_method {
            ...
            InstallationMethod::Homebrew => {
                return Err(anyhow!(
                    "Detected this CLI comes from homebrew, use `brew upgrade cedra` instead"
                ));
            },
        ...
        Update::configure()
            ...
            .target_version_tag(&format!("cedra-cli-v{}", info.target_version))
            .target(target)
