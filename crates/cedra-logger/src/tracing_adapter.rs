...
/// A layer that translates tracing events into cedra-logger events.
pub struct TracingToCedraDataLayer;
...
impl<S> Layer<S> for TracingToCedraDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-cedra-logger] failed to translate event due to unknown level {:?}",
