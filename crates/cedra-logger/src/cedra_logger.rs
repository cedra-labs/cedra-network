...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        #[cfg(node_identity)]
        {
            peer_id = cedra_node_identity::peer_id_as_str();
            chain_id = cedra_node_identity::chain_id().map(|chain_id| chain_id.id());
        }
...
/// A builder for a `CedraData`, configures what, where, and how to write logs.
pub struct CedraDataBuilder {
    channel_size: usize,
...
impl CedraDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<CedraData> {
    ...
    pub fn build(&mut self) -> Arc<CedraData> {
...
impl CedraData {
    pub fn builder() -> CedraDataBuilder {
        CedraDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> CedraDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Cedra Data Builder
...
/// or to a `CedraData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<CedraData>,
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/cedra-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<CedraData>,
    logger_builder: CedraDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<CedraData>, logger_builder: CedraDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{text_format, CedraData, LogEntry};
    use crate::{
        cedra_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        telemetry_log_writer::TelemetryLog,
        trace, warn, CedraDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"cedra_logger\",\"file\":\"crates/cedra-logger/src/cedra_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (CedraDataBuilder, Arc<CedraData>) {
        let mut logger_builder = CedraDataBuilder::new();
    ...
    fn new_text_logger() -> (
        ...
        let logger = CedraDataBuilder::new()
            .level(Level::Debug)
