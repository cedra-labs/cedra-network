...
use cedra_config::config::{Peer, PeerRole, PeerSet};
use cedra_infallible::RwLock;
use cedra_rest_client::Response;
use cedra_types::{
...
impl PeerSetCacheUpdater {
    ...
    async fn update_for_chain(
        ...
        let client = cedra_rest_client::Client::new(Url::parse(url).map_err(|e| {
...
mod tests {
    ...
    use cedra_infallible::RwLock;
    use cedra_rest_client::cedra_api_types::*;
    use cedra_types::{
    ...
    async fn test_validator_cache_updater_with_invalid_address() {
        ...
        let mock = server.mock(|when, then| {
            ...
            then.status(200)
                ...
                .header(X_CEDRA_CHAIN_ID, "25")
                .header(X_CEDRA_EPOCH, "10")
                .header(X_CEDRA_LEDGER_VERSION, "10")
                .header(X_CEDRA_LEDGER_OLDEST_VERSION, "2")
                .header(X_CEDRA_BLOCK_HEIGHT, "25")
                .header(X_CEDRA_OLDEST_BLOCK_HEIGHT, "10")
                .header(X_CEDRA_LEDGER_TIMESTAMP, "10");
        });
    ...
    async fn test_validator_cache_updater_with_valid_address() {
        ...
        let validator_info = ValidatorInfo::new(
            ...
            ValidatorConfig::new(
                ...
                bcs::to_bytes(&vec![NetworkAddress::from_str("/dns/fullnode0.testnet.cedralabs.com/tcp/6182/noise-ik/0xea19ab47ed9191865f15d85d751ed0663205c0b2f0f465714b1947c023715973/handshake/0").unwrap()]).unwrap(),
                2,
            ),
        );
