...
#[clap(name = "Cedra Telemetry Service", author, version)]
pub struct CedraTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl CedraTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(cedra_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(cedra_infallible::RwLock::new(HashMap::new()));
        let peer_locations = Arc::new(cedra_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    CedraTelemetryServiceArgs::command().debug_assert()
}
