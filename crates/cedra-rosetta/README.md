...
[Rosetta](https://en.wikipedia.org/wiki/Rosetta_(software)) works as a sidecar to an Cedra fullnode.  Rosetta then proxies the Rosetta standard
API calls to underlying Cedra REST API calls and builds the appropriate data.  


## Running Rosetta

The `cedra-rosetta` binary can run in three modes:
1. `online` -> This runs a local fullnode and blocks the Cedra REST API from outside access, using it only as a local proxy for Rosetta APIs.
...
### Balances
* Only the native `Cedra` is supported.
...
All transactions are parsed from the events provided by the CedraFramework.  There are a few exceptions to this that use the transaction payload, but only for errors.
...
* Transfers support only Cedra at this moment.
...
file to run the automated checks.  Additionally, the [cedra.ros](./cedra.ros)
file uses the Rosetta CLI DSL to describe the possible operations that
can be run.

Additionally, we have our `cedra-rosetta-cli` crate for local testing.
