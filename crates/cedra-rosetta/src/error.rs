...
use cedra_rest_client::{cedra_api_types::CedraErrorCode, error::RestError};
    ...
    fn from(err: RestError) -> Self {
        match err {
            RestError::Api(err) => match err.error.error_code {
                ...
                CedraErrorCode::ModuleNotFound => ApiError::ModuleNotFound(Some(err.error.message)),
                ...
                CedraErrorCode::BlockNotFound => ApiError::BlockNotFound(Some(err.error.message)),
                CedraErrorCode::StateValueNotFound => {
                    ApiError::StateValueNotFound(Some(err.error.message))
                },
                CedraErrorCode::VersionPruned => ApiError::VersionPruned(Some(err.error.message)),
                CedraErrorCode::BlockPruned => ApiError::BlockPruned(Some(err.error.message)),
                CedraErrorCode::InvalidInput => ApiError::InvalidInput(Some(err.error.message)),
                ...
                CedraErrorCode::VmError => ApiError::VmError(Some(err.error.message)),
                CedraErrorCode::HealthCheckFailed => {
                    ApiError::InternalError(Some(err.error.message))
                },
                CedraErrorCode::MempoolIsFull => ApiError::MempoolIsFull(Some(err.error.message)),
                CedraErrorCode::WebFrameworkError => {
                    ApiError::InternalError(Some(err.error.message))
                },
                CedraErrorCode::BcsNotSupported => ApiError::InvalidInput(Some(err.error.message)),
                CedraErrorCode::InternalError => ApiError::InternalError(Some(err.error.message)),
                CedraErrorCode::ApiDisabled => ApiError::InternalError(Some(err.error.message)),
            },
