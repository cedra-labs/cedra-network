...
use cedra_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
use cedra_logger::warn;
use cedra_rest_client::cedra_api_types::{ResourceGroup, TransactionOnChainData, U64};
use cedra_types::{
    access_path::Path,
...
pub struct Block {
    ...
    pub timestamp: u64,
    /// Transactions associated with the version.  In cedra there should only be one transaction
    pub transactions: Vec<Transaction>,
}

/// A combination of a transaction and the block associated.  In Cedra, this is just the same
...
///
/// This is known as a write set change within Cedra
...
/// Cryptographic signature type used for signing transactions.  Cedra only uses
    ...
    if let TransactionPayload::EntryFunction(inner) = payload {
        match (
            *inner.module().address(),
            ...
            | (AccountAddress::ONE, CEDRA_ACCOUNT_MODULE, TRANSFER_COINS_FUNCTION) => {
            ...
            (AccountAddress::ONE, CEDRA_ACCOUNT_MODULE, TRANSFER_FUNCTION) => {
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<cedra_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<cedra_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
...
impl InternalOperation {
    ...
    pub fn payload(
        &self,
    ) -> ApiResult<(cedra_types::transaction::TransactionPayload, AccountAddress)> {
        Ok(match self {
            InternalOperation::CreateAccount(create_account) => (
                cedra_stdlib::cedra_account_create_account(create_account.new_account),
            ...
            InternalOperation::Transfer(transfer) => {
                ...
                if currency == &native_coin() {
                    return Ok((
                        cedra_stdlib::cedra_account_transfer(transfer.receiver, transfer.amount.0),
                        transfer.sender,
                    ));
                }
                ...
                if let Some(ref metadata) = currency.metadata {
                    match (&metadata.move_type, &metadata.fa_address) {
                        ...
                        (Some(coin_type), Some(_)) | (Some(coin_type), None) => {
                            ...
                            (
                                cedra_stdlib::cedra_account_transfer_coins(
                                    coin_type_tag,
                                    transfer.receiver,
                                    transfer.amount.0,
                                ),
            ...
            InternalOperation::SetVoter(set_voter) => {
                ...
                (
                    cedra_stdlib::staking_contract_update_voter(
            ...
            InternalOperation::InitializeStakePool(init_stake_pool) => (
                cedra_stdlib::staking_contract_create_staking_contract(
            ...
            InternalOperation::ResetLockup(reset_lockup) => (
                cedra_stdlib::staking_contract_reset_lockup(reset_lockup.operator),
                reset_lockup.owner,
            ),
            InternalOperation::UnlockStake(unlock_stake) => (
                cedra_stdlib::staking_contract_unlock_stake(
            ...
            InternalOperation::UpdateCommission(update_commision) => (
                cedra_stdlib::staking_contract_update_commision(
            ...
            InternalOperation::DistributeStakingRewards(distribute_staking_rewards) => (
                cedra_stdlib::staking_contract_distribute(
            ...
            InternalOperation::AddDelegatedStake(add_delegated_stake) => (
                cedra_stdlib::delegation_pool_add_stake(
            ...
            InternalOperation::UnlockDelegatedStake(unlock_delegated_stake) => (
                cedra_stdlib::delegation_pool_unlock(
            ...
            InternalOperation::WithdrawUndelegated(withdraw_undelegated) => (
                cedra_stdlib::delegation_pool_withdraw(
