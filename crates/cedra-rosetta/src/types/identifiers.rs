...
/// Identifier for a "block".  On Cedra, we use a transaction model, so the index
...
pub struct NetworkIdentifier {
    /// Blockchain name, should always be `cedra` and be hardcoded
...
impl From<cedra_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: cedra_crypto::HashValue) -> Self {
        TransactionIdentifier {
