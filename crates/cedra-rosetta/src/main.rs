...
use cedra_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use cedra_logger::prelude::*;
use cedra_node::CedraNodeArgs;
use cedra_rosetta::{bootstrap, common::native_coin, types::Currency};
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("cedra-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        },
        CommandArgs::Online(_) => {
            println!("cedra-rosetta: Starting Rosetta in Online (with local full node) mode")
        },
        CommandArgs::Offline(_) => println!("cedra-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    }) = args
    {
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "cedra-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("cedra-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("cedra-rosetta: Starting rosetta");
    ...
    .expect("cedra-rosetta: Should bootstrap rosetta server");

    println!("cedra-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<cedra_rest_client::Client>;
...
/// Cedra Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on Cedra.
#[derive(Debug, Parser)]
#[clap(name = "cedra-rosetta", author, version, propagate_version = true)]
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the Cedra REST API. e.g. https://devnet.cedra.dev
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<cedra_rest_client::Client> {
        Some(cedra_rest_client::Client::new(self.rest_api_url.clone()))
    }
