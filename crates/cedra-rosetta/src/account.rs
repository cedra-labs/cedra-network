...
use cedra_logger::{debug, trace, warn};
use cedra_rest_client::{
    cedra_api_types::{CedraError, CedraErrorCode, ViewFunction},
    error::{CedraErrorResponse, RestError},
    Client,
};
use cedra_types::{account_address::AccountAddress, account_config::AccountResource};
use move_core_types::{
    ident_str,
    identifier::IdentStr,
...
async fn account_balance(
    ...
    Ok(AccountBalanceResponse {
        ...
        metadata: AccountBalanceMetadata {
            ...
            operators,
            lockup_expiration_time_utc: cedra_rest_client::cedra_api_types::U64(lockup_expiration),
        },
    })
}
...
async fn get_sequence_number(
        ...
        .await
    {
        ...
        Err(RestError::Api(CedraErrorResponse {
            error:
                CedraError {
                    error_code: CedraErrorCode::AccountNotFound,
                    ..
                },
            ..
        }))
        | Err(RestError::Api(CedraErrorResponse {
            error:
                CedraError {
                    error_code: CedraErrorCode::ResourceNotFound,
                    ..
                },
            ..
        })) => {
