...
pub use crate::client_builder::{CedraBaseUrl, ClientBuilder};
use crate::{cedra::CedraVersion, error::RestError};
use anyhow::{anyhow, Result};
pub use cedra_api_types::{
...
use cedra_api_types::{
    ...
    CedraError, CedraErrorCode, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse,
...
use cedra_logger::{debug, info, sample, sample::SampleRate};
use cedra_types::{
...
const X_CEDRA_SDK_HEADER_VALUE: &str = concat!("cedra-rust-sdk/", env!("CARGO_PKG_VERSION"));

type CedraResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(cedra_base_url: CedraBaseUrl) -> ClientBuilder {
        ClientBuilder::new(cedra_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(CedraBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> CedraResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> CedraResult<Url> {
    ...
    pub async fn get_cedra_version(&self) -> CedraResult<Response<CedraVersion>> {
        self.get_resource::<CedraVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    pub async fn lookup_address(
        ...
        must_exist: bool,
    ) -> CedraResult<Response<AccountAddress>> {
        ...
        {
            Ok(inner) => Ok(inner),
            Err(RestError::Api(CedraErrorResponse {
                error:
                    CedraError {
                        error_code: CedraErrorCode::TableItemNotFound,
                        ..
                    },
                ..
            })) => {
    ...
    /// For more details, see: https://cedra.dev/en/build/apis/fullnode-rest-api-reference#tag/accounts/GET/accounts/{address}/balance/{asset_type}
    ...
    pub async fn get_index_bcs(&self) -> CedraResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/cedra-labs/cedra-network/issues/5597.
    pub async fn get_ledger_information(&self) -> CedraResult<Response<State>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> CedraResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> CedraResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> CedraResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> CedraResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> CedraResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> CedraResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> CedraResult<Response<Transaction>> {
    ...
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = CedraResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(cedra_error_response) = error {
                        if let Some(state) = cedra_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if cedra_infallible::duration_since_epoch().as_secs()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CedraResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CedraResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> CedraResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> CedraResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> CedraResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> CedraResult<Response<Option<Resource>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}",
            address.to_hex(),
            resource_type
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> CedraResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> CedraResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address.to_hex()),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> CedraResult<Response<MoveModuleBytecode>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> CedraResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CedraResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_sequence_number(
        ...
        match res {
            ...
            Err(error) => match error {
                RestError::Api(error) => {
                    if matches!(error.error.error_code, CedraErrorCode::AccountNotFound) {
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> CedraResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> CedraResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> CedraResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address.to_hex()))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_transaction_summaries(
        ...
        limit: Option<u16>,
    ) -> CedraResult<Response<Vec<IndexedTransactionSummary>>> {
        let url = self.build_path(&format!(
    ...
    pub async fn estimate_gas_price(&self) -> CedraResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> CedraResult<String> {
    ...
    ) -> CedraResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> CedraResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> CedraResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> CedraResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
        self.check_and_parse_bcs_response(response).await
    }

    pub async fn make_prover_request(&self, req: ProverRequest) -> CedraResult<ZeroKnowledgeSig> {
    ...
    pub async fn make_pepper_request(&self, req: PepperRequest) -> CedraResult<Pepper> {
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<CedraError>) -> bool,
        Fut: Future<Output = CedraResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> CedraResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, cedra_error: Option<CedraError>) -> bool {
    retriable(status_code, cedra_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _cedra_error: Option<CedraError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> CedraResult<State> {
