...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestCedraCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestCedraCrypto {
    TestCedraCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestCedraCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestCedraCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestCedraCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestCedraCrypto>>();
...
/// cedra-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello Cedra!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestCedraCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Cedra <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
