...
//! cedra_channel provides an mpsc channel which has two ends `cedra_channel::Receiver`
//! and `cedra_channel::Sender` similar to existing mpsc data structures.
...
use cedra_infallible::{Mutex, NonZeroUsize};
use cedra_metrics_core::IntCounterVec;
use futures::{
    channel::oneshot,
...
/// The sending end of the cedra_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a cedra_channel. If the element is successfully
...
/// The receiving end of the cedra_channel.
#[derive(Debug)]
...
/// Configuration for a new cedra_channel queue.
#[derive(Clone, Copy)]
pub struct Config {
...
impl Config {
    /// The cedra_channel has a "sub-queue" per key. The `max_capacity` controls
...
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "cedra_channel cannot be of size 0");
