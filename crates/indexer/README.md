...
### Installing fullnode
Please follow standard fullnode installation guide on cedra.dev (https://cedra.dev/nodes/full-node/fullnode-source-code-or-docker)

### Running indexer
```bash
cargo run -p cedra-node --features "indexer" --release -- -f <some_path>/fullnode.yaml
```
...
> - Cedra tests use the `INDEXER_DATABASE_URL` env var. It needs to be set for the relevant tests to run.
