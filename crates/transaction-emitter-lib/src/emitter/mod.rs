...
use cedra_config::config::DEFAULT_MAX_SUBMIT_TRANSACTION_BATCH_SIZE;
use cedra_crypto::ed25519::Ed25519PrivateKey;
use cedra_logger::{sample, sample::SampleRate};
use cedra_rest_client::{cedra_api_types::CedraErrorCode, error::RestError, Client as RestClient};
use cedra_sdk::{
...
use cedra_types::account_config::cedra_test_root_address;
...
impl Default for EmitJobRequest {
    fn default() -> Self {
        Self {
            ...
            max_gas_per_txn: cedra_global_constants::MAX_GAS_AMOUNT,
...
async fn wait_for_accounts_sequence(
    ...
    loop {
        ...
        if cedra_infallible::duration_since_epoch().as_secs() >= txn_expiration_ts_secs + 240 {
            sample!(
    ...
    match &result {
        ...
        Err(e) => {
            ...
            if let RestError::Api(api_error) = e {
                if let CedraErrorCode::AccountNotFound = api_error.error.error_code {
                    return Ok((
                        0,
...
pub fn gen_transfer_txn_request(
    ...
    sender.sign_with_transaction_builder(
        txn_factory.payload(cedra_stdlib::cedra_coin_transfer(*receiver, num_coins)),
    )
}
