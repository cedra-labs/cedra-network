...
use cedra_types::{aggregate_signature::AggregateSignature, transaction::ReplayProtector, PeerId};
use maplit::hashset;
...
async fn test_proof_calculate_remaining_txns_and_proofs() {
    ...
    let batch_store = batch_store_for_test(5 * 1024 * 1024);
    let mut proof_queue = BatchProofQueue::new(my_peer_id, batch_store, 1);
    let now_in_secs = cedra_infallible::duration_since_epoch().as_secs() as u64;
    let now_in_usecs = cedra_infallible::duration_since_epoch().as_micros() as u64;
    let author_0 = PeerId::random();
    let author_1 = PeerId::random();
    let txns = vec![
...
async fn test_proof_queue_pull_full_utilization() {
    ...
    let now_in_secs = cedra_infallible::duration_since_epoch();
