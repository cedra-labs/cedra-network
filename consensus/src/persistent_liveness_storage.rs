...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the cedradb.
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self {
            accu_hash: *cedra_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH,
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, cedra_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, cedra_db }
    }
}
