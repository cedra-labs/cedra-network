...
impl From<cedra_storage_interface::CedraDbError> for DbError {
    fn from(e: cedra_storage_interface::CedraDbError) -> Self {
...
impl From<cedra_executor_types::ExecutorError> for StateSyncError {
    fn from(e: cedra_executor_types::ExecutorError) -> Self {
        StateSyncError { inner: e.into() }
    }
}
...
pub fn error_kind(e: &anyhow::Error) -> &'static str {
    if e.downcast_ref::<cedra_executor_types::ExecutorError>()
        .is_some()
    {
        return "Execution";
    }
    if let Some(e) = e.downcast_ref::<StateSyncError>() {
        if e.inner
            .downcast_ref::<cedra_executor_types::ExecutorError>()
            .is_some()
        {
            return "Execution";
        }
    ...
    if e.downcast_ref::<cedra_safety_rules::Error>().is_some() {
        return "SafetyRules";
    }
...
mod tests {
    ...
    fn conversion_and_downcast() {
        let error = cedra_executor_types::ExecutorError::InternalError {
