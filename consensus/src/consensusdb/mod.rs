...
use anyhow::Result;
use cedra_consensus_types::{block::Block, quorum_cert::QuorumCert};
use cedra_crypto::HashValue;
use cedra_logger::prelude::*;
use cedra_schemadb::{batch::SchemaBatch, schema::Schema, Options, DB, DEFAULT_COLUMN_FAMILY_NAME};
...
impl ConsensusDB {
    ...
    pub fn get_all<S: Schema>(&self) -> Result<Vec<(S::Key, S::Value)>, DbError> {
        ...
        iter.seek_to_first();
        Ok(iter.collect::<Result<Vec<(S::Key, S::Value)>, CedraDbError>>()?)
    }
