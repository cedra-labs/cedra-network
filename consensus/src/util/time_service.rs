...
where
    T: Send + 'static,
{
    sender: Option<cedra_channels::Sender<T>>,
    message: Option<T>,
}
    ...
    T: Send + 'static,
{
    ...
    pub fn make(sender: cedra_channels::Sender<T>, message: T) -> Box<dyn ScheduledTask> {
        Box::new(SendTask {
            sender: Some(sender),
...
async fn test_time_service_abort() {
    ...
    let (tx, mut rx) = cedra_channels::new_test(10);
