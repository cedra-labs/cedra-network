...
def get_release_number_from_release_tag(release_tag: str) -> str:
    """Get the release number from the release tag. The release tag looks like cedra-node-vX.Y.Z"""
...
def main():
    required_envs = ["RELEASE_TAG", "CEDRA_NODE_CARGO_TOML"]
    ...
    CEDRA_NODE_TOML = os.environ.get("CEDRA_NODE_CARGO_TOML")

    new_lines = []  # construct the file again
    with open(CEDRA_NODE_TOML, "r") as f:
        lines = f.readlines()
        for line in lines:
            new_line = line
    ...
    with open(CEDRA_NODE_TOML, "w") as f:
...
class CheckCedraNodeVersionTests(unittest.TestCase):
    def test_envs_not_specified(self):
        with self.assertRaises(SystemExit):
            os.environ["RELEASE_TAG"] = ""
            os.environ["CEDRA_NODE_CARGO_TOML"] = ""
            main()
    ...
    def test_version_match(self):
        ...
        os.environ["CEDRA_NODE_CARGO_TOML"] = tmp.name
