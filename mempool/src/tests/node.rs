...
use cedra_crypto::{x25519::PrivateKey, Uniform};
use cedra_event_notifications::{ReconfigNotification, ReconfigNotificationListener};
use cedra_infallible::{Mutex, MutexGuard, RwLock};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: cedra_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    /// Peer notification sender for sending outgoing messages to other peers
    pub(crate) network_notifs_tx: cedra_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
}
...
fn setup_node_network_interface(
    ...
    let (connection_reqs_tx, _) = cedra_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
