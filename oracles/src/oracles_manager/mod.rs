...
pub struct DKGManager<DKG: DKGTrait> {
    ...
    agg_trx_tx: Option<cedra_channel::Sender<(), DKG::Transcript>>,

    // When we put vtxn in the pool, we also put a copy of this so later pool can notify us.
    pull_notification_tx: cedra_channel::Sender<(), Arc<ValidatorTransaction>>,
    pull_notification_rx: cedra_channel::Receiver<(), Arc<ValidatorTransaction>>,
...
impl<DKG: DKGTrait> DKGManager<DKG> {
    ...
    pub async fn run(
        ...
        mut dkg_start_event_rx: cedra_channel::Receiver<(), DKGStartEvent>,
        mut rpc_msg_rx: cedra_channel::Receiver<
            AccountAddress,
        ...
        let (agg_trx_tx, mut agg_trx_rx) = cedra_channel::new(QueueStyle::KLAST, 1, None);
