...
pub struct HashValue(pub cedra_crypto::hash::HashValue);

impl From<cedra_crypto::hash::HashValue> for HashValue {
    fn from(val: cedra_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for cedra_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<cedra_crypto::hash::HashValue>()?.into())
        } else {
            Ok(s.parse::<cedra_crypto::hash::HashValue>()?.into())
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(cedra_crypto::hash::HashValue::zero())
    }
}
