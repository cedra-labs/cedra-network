...
use cedra_types::vm_status::StatusCode;
use poem_openapi::{Enum, Object};
use serde::{Deserialize, Serialize};
use std::fmt::Formatter;

/// This is the generic struct we use for all API errors, it contains a string
/// message and an Cedra API specific error code.
#[derive(Debug, Clone, Serialize, Deserialize, Object)]
pub struct CedraError {
    /// A message describing the error
    pub message: String,
    pub error_code: CedraErrorCode,
    /// A code providing VM error details when submitting transactions to the VM
    pub vm_error_code: Option<u64>,
}

impl std::fmt::Display for CedraError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "Error({:?}): {:#}", self.error_code, self.message)
    }
}

impl std::error::Error for CedraError {}
...
fn test_serialize_deserialize() {
    let with_code = CedraError::new_with_vm_status(
        "Invalid transaction",
        CedraErrorCode::VmError,
        cedra_types::vm_status::StatusCode::UNKNOWN_MODULE,
    );
    let _: CedraError = bcs::from_bytes(&bcs::to_bytes(&with_code).unwrap()).unwrap();
    let _: CedraError = serde_json::from_str(&serde_json::to_string(&with_code).unwrap()).unwrap();

    let without_code =
        CedraError::new_with_error_code("some message", CedraErrorCode::MempoolIsFull);
    let _: CedraError = bcs::from_bytes(&bcs::to_bytes(&without_code).unwrap()).unwrap();
    let _: CedraError =
