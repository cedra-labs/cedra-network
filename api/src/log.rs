...
static REQUEST_SOURCE_CLIENT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"cedra-[a-zA-Z\-]+/[0-9A-Za-z\.\-]+").unwrap());
...
pub async fn middleware_log<E: Endpoint>(next: E, request: Request) -> Result<Response> {
    ...
    REQUEST_SOURCE_CLIENT
        .with_label_values(&[
            determine_request_source_client(&log.cedra_client),
            operation_id,
            log.status.to_string().as_str(),
        ])
        .inc();
...
// Each of our clients includes a header value called X_CEDRA_CLIENT that identifies
// that client. This string follows a particular format: <identifier>/<version>,
// where <identifier> always starts with `cedra-`. This function ensure this string
...
fn determine_request_source_client(cedra_client: &Option<String>) -> &str {
    // If the header is not set we can't determine the request source.
    let cedra_client = match cedra_client {
        Some(cedra_client) => cedra_client,
    ...
    match REQUEST_SOURCE_CLIENT_REGEX.find_iter(cedra_client).last() {
