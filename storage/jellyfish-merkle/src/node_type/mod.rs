...
use crate::{
    get_hash,
    metrics::{CEDRA_JELLYFISH_INTERNAL_ENCODED_BYTES, CEDRA_JELLYFISH_LEAF_ENCODED_BYTES},
    Key, TreeReader,
};
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        match self {
            Node::Internal(internal_node) => {
                ...
                CEDRA_JELLYFISH_INTERNAL_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Leaf(leaf_node) => {
                out.push(NodeTag::Leaf as u8);
                out.extend(bcs::to_bytes(&leaf_node)?);
                CEDRA_JELLYFISH_LEAF_ENCODED_BYTES.inc_by(out.len() as u64);
            },
            Node::Null => {
