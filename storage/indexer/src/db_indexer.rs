...
use cedra_config::config::internal_indexer_db_config::InternalIndexerDBConfig;
...
use cedra_storage_interface::{
    db_ensure as ensure, db_other_bail as bail, CedraDbError, DbReader, Result,
};
use cedra_types::{
    ...
    pub fn get_account_ordered_transactions_iter(
        ...
        Ok(AccountOrderedTransactionsIter::new(
            ...
            min_seq_num
                ...
                .ok_or(CedraDbError::TooManyRequested(min_seq_num, num_versions))?,
            ledger_version,
        ))
    }
...
impl DBIndexer {
    ...
    pub fn process_a_batch(&self, start_version: Version, end_version: Version) -> Result<Version> {
        let _timer: cedra_metrics_core::HistogramTimer =
