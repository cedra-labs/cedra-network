...
fn open_db(dir: &cedra_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &cedra_temppath::TempPath) -> DB {
    DB::open_cf_readonly(&rocksdb::Options::default(), dir.path(), "test", get_cfds())
        .expect("Failed to open DB.")
}

fn open_db_as_secondary(dir: &cedra_temppath::TempPath, dir_sec: &cedra_temppath::TempPath) -> DB {
...
struct TestDB {
    _tmpdir: cedra_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = cedra_temppath::TempPath::new();
...
fn collect_values<S: Schema>(db: &TestDB) -> Vec<(S::Key, S::Value)> {
    ...
    iter.seek_to_first();
    iter.collect::<Result<Vec<_>, CedraDbError>>().unwrap()
}
...
fn test_open_as_secondary() {
    let tmpdir = cedra_temppath::TempPath::new();
    let tmpdir_sec = cedra_temppath::TempPath::new();
...
fn test_checkpoint() {
    let tmpdir = cedra_temppath::TempPath::new();
    let checkpoint = cedra_temppath::TempPath::new();
    {
        let db = open_db(&tmpdir);
        db.put::<TestSchema1>(&TestField(0), &TestField(0)).unwrap();
