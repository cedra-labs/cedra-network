...
use crate::{
    schema::state_value_by_key_hash::StateValueByKeyHashSchema, state_kv_db::StateKvDb, CedraDB,
};
use cedra_config::config::{RocksdbConfig, StorageDirPaths};
use cedra_crypto::{hash::CryptoHash, HashValue};
use cedra_db_indexer::db_ops::open_internal_indexer_db;
...
use cedra_schemadb::{ReadOptions, DB};
...
pub fn validate_db_data(
    ...
    let cedra_db = CedraDB::new_for_test_with_sharding(db_root_path, 1000000);
    let batch_size = 20_000;
    let start_version = cedra_db.get_first_txn_version()?.unwrap();
    target_ledger_version = std::cmp::min(
        cedra_db.get_synced_version()?.unwrap(),
