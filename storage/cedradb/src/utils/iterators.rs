    ...
    fn expect_continuous_versions(
        ...
        Ok(ContinuousVersionIter {
            ...
            end_version: first_version
                ...
                .ok_or(CedraDbError::TooManyRequested(first_version, limit as u64))?,
...
impl<'a> PrefixedStateValueIterator<'a> {
    pub fn new(
        ...
        // cedra/abc|0
        // cedra/abd|1
        // if we seek('cedra/'), and call next, we may not reach `cedra/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `cedra/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `cedra/abc`.
    ...
    fn next_impl(&mut self) -> Result<Option<Vec<ContractEvent>>> {
        ...
        while let Some(res) = self.inner.peek() {
            let ((version, _index), _event) = res
                .as_ref()
                .map_err(|e| CedraDbError::Other(format!("Hit error iterating events: {}", e)))?;
        ...
        self.expected_next_version = self
            .expected_next_version
            .checked_add(1)
            .ok_or_else(|| CedraDbError::Other("expected version overflowed.".to_string()))?;
        Ok(Some(ret))
    }
}
