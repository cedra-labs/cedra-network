...
use crate::{CedraDB, EventStore, LedgerPrunerManager, PrunerManager};
use cedra_config::config::LedgerPrunerConfig;
use cedra_proptest_helpers::Index;
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &cedra_db.event_store;
    ...
    cedra_db.ledger_db.event_db().write_schemas(batch).unwrap();
