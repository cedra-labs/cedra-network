...
use cedra_config::config::StateMerklePrunerConfig;
use cedra_jellyfish_merkle::StaleNodeIndex;
use cedra_schemadb::schema::KeyCodec;
use cedra_storage_interface::Result;
use cedra_types::transaction::{AtomicVersion, Version};
use std::{
    marker::PhantomData,
    sync::{atomic::Ordering, Arc},
};

/// The `Pruner` is meant to be part of a `CedraDB` instance and runs in the background to prune old
/// data.
///
