...
use cedra_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use cedra_logger::info;
use cedra_metrics_core::TimerHelper;
use cedra_schemadb::batch::{NativeBatch, SchemaBatch, WriteBatch};
...
use cedra_storage_interface::{
    ...
    CedraDbError, DbReader, Result, StateSnapshotReceiver,
};
use cedra_types::{
...
impl StateStore {
    ...
    // TODO: change to async comment once it does like https://github.com/cedra-labs/cedra-network/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<cedra_jellyfish_merkle::node_type::NodeKey>> {
        cedra_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
            .map_err(Into::into)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<cedra_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .metadata_db()
        ...
        let mut keys: Vec<cedra_jellyfish_merkle::node_type::NodeKey> =
...
mod test_only {
    ...
    use cedra_schemadb::batch::SchemaBatch;
