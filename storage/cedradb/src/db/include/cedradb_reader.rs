...
use cedra_storage_interface::state_store::state::State;
use cedra_storage_interface::state_store::state_summary::StateSummary;
use cedra_storage_interface::state_store::state_view::hot_state_view::HotStateView;
use cedra_types::{block_info::BlockHeight, transaction::IndexedTransactionSummary};

impl DbReader for CedraDB {
    ...
    /// Returns the transaction by version, delegates to `CedraDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| CedraDbError::NotFound(format!("TableInfo for {:?}", handle)))
        })
    }
