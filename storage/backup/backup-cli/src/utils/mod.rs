...
use cedra_db_indexer_schemas::metadata::StateSnapshotProgress;
...
use cedra_jellyfish_merkle::{NodeBatch, TreeWriter};
use cedra_logger::info;
use cedra_storage_interface::{CedraDbError, Result};
use cedra_types::{
    state_store::{
...
impl TryFrom<GlobalRestoreOpt> for GlobalRestoreOptions {
    ...
    fn try_from(opt: GlobalRestoreOpt) -> anyhow::Result<Self> {
        ...
        let run_mode = if let Some(db_dir) = &opt.db_dir {
            ...
            let restore_handler = Arc::new(CedraDB::open_kv_only(
...
pub struct ReplayConcurrencyLevelOpt {
    /// CedraVM::set_concurrency_level_once() is called with this
    #[clap(
        long,
...
impl<T: AsRef<Path>> PathToString for T {
    fn path_to_string(&self) -> Result<String> {
        self.as_ref()
            ...
            .into_os_string()
            .into_string()
            .map_err(|s| CedraDbError::Other(format!("into_string failed for OsString '{:?}'", s)))
    }
}
