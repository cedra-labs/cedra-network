...
use cedra_crypto::{hash::CryptoHash, PrivateKey};
...
use cedra_types::{
    account_config::{cedra_test_root_address, AccountResource, CORE_CODE_ADDRESS},
    block_metadata::BlockMetadata,
    on_chain_config::{CedraVersion, OnChainConfig, ValidatorSet},
...
fn test_genesis() {
    ...
    let (cedra_framework_account_resource, state_proof) = db
        .reader
    ...
    state_proof
        .verify(
            ...
            cedra_framework_account_resource.as_ref(),
        )
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    // txn3 = set the cedra version for next epoch
    ...
    assert_eq!(
        CedraVersion::fetch_config(&db_state_view).unwrap().major,
        42
    );
}
