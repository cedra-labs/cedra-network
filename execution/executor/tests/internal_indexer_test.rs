...
use cedra_db_indexer::db_indexer::DBIndexer;
...
use cedra_indexer_grpc_table_info::internal_indexer_db_service::InternalIndexerDBService;
use cedra_sdk::{
...
#[cfg(test)]
pub fn create_test_db() -> (Arc<CedraDB>, LocalAccount) {
    // create test db
    let path = cedra_temppath::TempPath::new();
    let (genesis, validators) = cedra_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    let core_resources_account: LocalAccount = LocalAccount::new(
        cedra_test_root_address(),
        AccountKey::from_private_key(cedra_vm_genesis::GENESIS_KEYPAIR.0.clone()),
        0,
    );
    let (cedra_db, _db, executor, _waypoint) =
    ...
    let signer = cedra_types::validator_signer::ValidatorSigner::new(
    ...
    let txn_factory = TransactionFactory::new(ChainId::test(), false);
    ...
    let reconfig1 = core_resources_account.sign_with_transaction_builder(
        txn_factory.payload(cedra_stdlib::cedra_governance_force_end_epoch_test_only()),
    );
    ...
    (cedra_db, core_resources_account)
}
...
fn test_db_indexer_data() {
    ...
    // create test db
    let (cedra_db, core_account) = create_test_db();
    let total_version = cedra_db.expect_synced_version();
    assert_eq!(total_version, 11);
    let temp_path = TempPath::new();
    let mut node_config = cedra_config::config::NodeConfig::default();
    ...
    let db_indexer = DBIndexer::new(internal_indexer_db.clone(), cedra_db.clone());
    ...
    let expected_code = vec![
        ...
        ident_str!("cedra_coin"),
        ident_str!("cedra_hash"),
    ...
    let expected_resources = vec![
        ...
        (false, "0x1::stake::CedraCoinCapabilities"),
        ...
        (false, "0x1::cedra_governance::VotingRecords"),
        (false, "0x1::state_storage::StateStorageUsage"),
        (false, "0x1::cedra_governance::VotingRecordsV2"),
        ...
        (false, "0x1::cedra_governance::GovernanceConfig"),
        (false, "0x1::cedra_governance::GovernanceEvents"),
        ...
        (false, "0x1::transaction_fee::CedraCoinMintCapability"),
        (false, "0x1::transaction_fee::CedraFABurnCapabilities"),
        (false, "0x1::jwk_consensus_config::JWKConsensusConfig"),
        (false, "0x1::cedra_governance::ApprovedExecutionHashes"),
        (false, "0x1::cedra_governance::GovernanceResponsbility"),
        ...
        (false, "0x1::coin::CoinInfo<0x1::cedra_coin::CedraCoin>"),
        (
            false,
