...
use cedra_executor::db_bootstrapper::{generate_waypoint, maybe_bootstrap};
...
pub(crate) fn bootstrap_with_genesis(
    ...
    let (config, _genesis_key) =
        cedra_genesis::test_utils::test_config_with_custom_onchain(Some(Arc::new(move |config| {
    ...
    let waypoint =
        generate_waypoint::<CedraVMBlockExecutor>(&db_rw, get_genesis_txn(&config).unwrap())
            .unwrap();
    maybe_bootstrap::<CedraVMBlockExecutor>(&db_rw, get_genesis_txn(&config).unwrap(), waypoint)
        .unwrap();
}
