...
use cedra_executor_service::remote_executor_client;
use cedra_experimental_ptx_executor::PtxBlockExecutor;
#[cfg(target_os = "linux")]
use cedra_experimental_runtimes::thread_manager::{ThreadConfigStrategy, ThreadManagerBuilder};
use cedra_metrics_core::{register_int_gauge, IntGauge};
use cedra_profiler::{ProfilerConfig, ProfilerHandler};
    ...
    E: VMBlockExecutor + 'static,
{
    match opt.cmd {
        Command::CreateDb {
            ...
            cedra_executor_benchmark::db_generator::create_db_with_accounts::<E>(
                num_accounts,
                init_account_balance,
                opt.block_size,
                data_dir,
        ...
        Command::RunExecutor {
            ...
            // cedra_types::on_chain_config::hack_enable_default_features_for_genesis(enable_feature);
            // cedra_types::on_chain_config::hack_disable_default_features_for_genesis(
            //     disable_feature,
            // );
            ...
            cedra_executor_benchmark::run_benchmark::<E>(
                opt.block_size,
                blocks,
                workload,
        ...
        Command::AddAccounts {
            ...
            cedra_executor_benchmark::add_accounts::<E>(
                num_new_accounts,
                init_account_balance,
                opt.block_size,
                data_dir,
...
fn main() {
    ...
    CedraVM::set_num_shards_once(execution_shards);
    CedraVM::set_concurrency_level_once(execution_threads_per_shard);
    ...
    match opt.block_executor_type {
        BlockExecutorTypeOpt::CedraVMWithBlockSTM => {
            run::<CedraVMBlockExecutor>(opt);
        },
        BlockExecutorTypeOpt::NativeVMWithBlockSTM => {
            run::<NativeVMBlockExecutor>(opt);
        },
        BlockExecutorTypeOpt::CedraVMParallelUncoordinated => {
            run::<CedraVMParallelUncoordinatedBlockExecutor>(opt);
        },
    ...
    if memory_profiling {
        let _mem_end = memory_profiler.end_profiling("./target/release/cedra-executor-benchmark");
    }
}
