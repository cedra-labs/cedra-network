...
use cedra_config::config::DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD;
use cedra_consensus_types::block::Block;
...
use cedra_types::{
    ...
    transaction::{
        ...
        FaAddress, TransactionListWithProof, TransactionWithProof, WriteSetPayload,
    },
...
pub fn test_execution_with_storage_impl() -> Arc<CedraDB> {
...
pub fn test_execution_with_storage_impl_inner(
    ...
    let (cedra_db, db, executor, waypoint) =
...
pub fn create_db_and_executor<P: AsRef<std::path::Path>>(
    ...
    Arc<CedraDB>,
    DbReaderWriter,
    BlockExecutor<CedraVMBlockExecutor>,
    Waypoint,
) {
    let (db, dbrw) = force_sharding
        .then(|| {
            DbReaderWriter::wrap(CedraDB::new_for_test_with_sharding(
                &path,
                DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD,
            ))
        })
        .unwrap_or_else(|| DbReaderWriter::wrap(CedraDB::new_for_test(&path)));
    let waypoint = bootstrap_genesis::<CedraVMBlockExecutor>(&dbrw, genesis).unwrap();
