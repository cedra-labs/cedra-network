...
pub trait TransactionValidation: Send + Sync + Clone {
    type ValidationInstance: cedra_vm::VMValidator;
...
struct ValidationState<S> {
    ...
    module_cache: UnsyncModuleCache<ModuleId, CompiledModule, Module, CedraModuleExtension, usize>,
}
...
impl<S: StateView> ValidationState<S> {
    ...
    fn new(state_view: S) -> Self {
        ...
        let environment = CedraEnvironment::new(&state_view);
        Self {
            state_view,
            environment,
    ...
    fn reset_all(&mut self, state_view: S) {
        self.state_view = state_view;
        self.environment = CedraEnvironment::new(&self.state_view);
...
impl<S: StateView> ModuleCache for ValidationState<S> {
    ...
    type Extension = CedraModuleExtension;
    type Key = ModuleId;
    ...
    > {
        ...
        Ok(if module.extension().hash() == &hash {
            ...
            let extension = Arc::new(CedraModuleExtension::new(state_value));
...
impl TransactionValidation for PooledVMValidator {
    type ValidationInstance = CedraVM;
...
mod tests {
    use super::*;
    use cedra_types::state_store::{state_value::StateValue, MockStateView};
