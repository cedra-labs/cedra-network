...
use cedra_types::{
    ...
    transaction::{FaAddress, Script, TransactionPayload},
...
struct TestValidator {
    ...
    _db_path: cedra_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = cedra_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(CedraDB::new_for_test(_db_path.path()));
        cedra_executor_test_helpers::bootstrap_genesis::<CedraVMBlockExecutor>(
            &db_rw,
            &cedra_vm_genesis::test_genesis_transaction(),
        )
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/cedra_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::cedra_test_root_address();
    let program = cedra_stdlib::cedra_coin_mint(address, 100);
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::cedra_test_root_address();
    let program = cedra_stdlib::cedra_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::cedra_test_root_address();
