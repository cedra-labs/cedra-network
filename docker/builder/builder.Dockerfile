...
RUN rm -f /etc/apt/apt.conf.d/docker-clean; echo 'Binary::apt::Cedra::Keep-Downloaded-Packages "true";' > /etc/apt/apt.conf.d/keep-cache
...
FROM builder-base as cedra-node-builder
