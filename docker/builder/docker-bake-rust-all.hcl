...
variable "CI" {
  # whether this build runs in cedra-labs' CI environment which makes certain assumptions about certain registries being available to push to cache layers.
...
variable "ecr_base" {
  default = "${AWS_ECR_ACCOUNT_NUM}.dkr.ecr.us-west-2.amazonaws.com/cedra"
}
...
target "_common" {
  contexts = {
    ...
    node-builder    = "target:cedra-node-builder"
