...
variable "chain_id" {
  description = "Cedra chain ID. If var.enable_forge set, defaults to 4"
  type        = number
  default     = 4
}
...
variable "chain_name" {
  description = "Cedra chain name. If unset, defaults to using the workspace name"
  type        = string
  default     = ""
}

variable "image_tag" {
  description = "Docker image tag for all Cedra workloads, including validators, fullnodes, backup, restore, genesis, and other tooling"
  type        = string
  default     = "devnet"
}
...
variable "cedra_node_helm_values" {
  description = "Map of values to pass to cedra-node helm chart"
  type        = any
  default     = {}
}
