...
module "validator" {
  ...
  helm_release_name_override = var.enable_forge ? "cedra-node" : ""
  ...
  helm_values         = var.cedra_node_helm_values
...
locals {
  cedra_node_helm_prefix = var.enable_forge ? "cedra-node" : "${module.validator.helm_release_name}-cedra-node"
}
