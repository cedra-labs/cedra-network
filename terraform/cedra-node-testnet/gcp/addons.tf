...
resource "helm_release" "chaos-mesh" {
  ...
  values = [
    jsonencode({
      chaos-mesh = {
        images = {
          registry = "us-docker.pkg.dev/cedra-registry/docker/ghcr.io"
          tag      = "cedra-patch" // Same as the patched chart in helm/chaos
        },
        chaosDaemon = {
          runtime    = "containerd"
  ...
  values = [
    jsonencode({
      ...
      extraArgs = [
        ...
        "--txt-owner-id=cedra-${local.workspace_name}",
        # "--txt-prefix=cedra-",
      ]
    })
  ]
...
resource "google_compute_global_address" "testnet-addons-ingress" {
  count   = var.zone_name != "" ? 1 : 0
  project = var.project
  name    = "cedra-${local.workspace_name}-testnet-addons-ingress"
}
...
resource "helm_release" "testnet-addons" {
  ...
  values = [
    jsonencode({
      ...
      genesis = {
        era             = var.era
        username_prefix = local.cedra_node_helm_prefix
      ...
      ingress = {
        gce_static_ip           = "cedra-${local.workspace_name}-testnet-addons-ingress"
        gce_managed_certificate = var.create_google_managed_ssl_certificate ? "cedra-${local.workspace_name}-${var.zone_name}-testnet-addons" : null
      }
    }),
