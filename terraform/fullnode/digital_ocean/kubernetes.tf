provider "kubernetes" {
  host                   = digitalocean_kubernetes_cluster.cedra.endpoint
  cluster_ca_certificate = base64decode(digitalocean_kubernetes_cluster.cedra.kube_config[0].cluster_ca_certificate)
  token                  = digitalocean_kubernetes_cluster.cedra.kube_config[0].token
}
...
resource "helm_release" "fullnode" {
  ...
  values = [
    jsonencode({
      ...
      nodeSelector = {
        "doks.digitalocean.com/node-pool" = digitalocean_kubernetes_cluster.cedra.node_pool[0].name
      }
      storageClass = {
