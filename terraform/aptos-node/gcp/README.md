...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  For the full customization options, see the variables file [here](https://github.com/cedra-labs/cedra/blob/main/terraform/cedra-node/gcp/variables.tf), and the [helm values](https://github.com/cedra-labs/cedra/blob/main/terraform/helm/cedra-node/values.yaml).
...
13. Download CedraFramework Move bytecode into a folder named `framework`. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Cedra Labs**
    ...
    Download the Cedra Framework from the release page: https://github.com/cedra-labs/cedra/releases/tag/cedra-framework-v0.1.0
