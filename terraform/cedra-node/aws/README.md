...
Install pre-requisites if needed:
   ...
   * Cedra CLI: https://github.com/cedra-labs/cedra-network/releases/tag/cedra-cli-v0.1.0-alpha
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  terraform {
    ...
    backend "s3" {
      bucket = "terraform.cedra-node"
      key    = "state/cedra-node"
      region = var.region
    }
  }
...
For the full customization options, see the variables file [here](https://github.com/cedra-labs/cedra-network/blob/main/terraform/cedra-node/aws/variables.tf), and the [helm values](https://github.com/cedra-labs/cedra-network/blob/main/terraform/helm/cedra-node/values.yaml).
...
8. Once terraform apply finishes, you can check if those resources are created:
    ...
    - `aws eks update-kubeconfig --name cedra-$WORKSPACE` to configure access for your k8s cluster.
...
9. Get your node IP info:
    ...
    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc ${WORKSPACE}-cedra-node-validator-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc ${WORKSPACE}-cedra-node-fullnode-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
    ```
...
11. Configure validator information.
    ...
    ```
    $ cedra genesis set-validator-configuration --keys-dir ~/$WORKSPACE --local-repository-dir ~/$WORKSPACE --username <pick a username for your node> --validator-host $VALIDATOR_ADDRESS:6180 --full-node-host $FULLNODE_ADDRESS:6182

    ```

    This will create a YAML file in your working directory with your username, e.g. `cedrabot.yaml`, it should looks like below:
...
13. Download CedraFramework Move bytecodes into a folder named `framework`.
