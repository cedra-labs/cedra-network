provider "kubernetes" {
  host                   = aws_eks_cluster.cedra.endpoint
  cluster_ca_certificate = base64decode(aws_eks_cluster.cedra.certificate_authority[0].data)
  token                  = data.aws_eks_cluster_auth.cedra.token
}
...
resource "null_resource" "delete-gp2" {
  provisioner "local-exec" {
    command = <<-EOT
      aws --region ${var.region} eks update-kubeconfig --name ${aws_eks_cluster.cedra.name} --kubeconfig ${local.kubeconfig} &&
