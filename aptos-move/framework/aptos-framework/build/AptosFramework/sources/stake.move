...
module aptos_framework::stake {
    ...
    public(friend) fun store_cedra_coin_mint_cap(aptos_framework: &signer, mint_cap: MintCapability<CedraCoin>) {
        system_addresses::assert_aptos_framework(aptos_framework);
        move_to(aptos_framework, CedraCoinCapabilities { mint_cap })
    }
    ...
    fun distribute_rewards(
        ...
        if (rewards_amount > 0) {
            let mint_cap = &borrow_global<CedraCoinCapabilities>(@aptos_framework).mint_cap;
    ...
    public fun initialize_for_test_custom(
        ...
        if (!exists<CedraCoinCapabilities>(@aptos_framework)) {
            let (burn_cap, mint_cap) = cedra_coin::initialize_for_test(aptos_framework);
            store_cedra_coin_mint_cap(aptos_framework, mint_cap);
    ...
    public entry fun test_allowed_validators(
        ...
        let (burn, mint) = cedra_coin::initialize_for_test(aptos_framework);
