spec aptos_framework::transaction_fee {
    ...
    spec module {
        ...
        /// [high-level-req-1]
        invariant [suspendable] chain_status::is_operating() ==> exists<CedraCoinCapabilities>(@aptos_framework) || exists<AptosFABurnCapabilities>(@aptos_framework);
    }
    ...
    spec burn_fee(account: address, fee: u64) {
        ...
        aborts_if !exists<CedraCoinCapabilities>(@aptos_framework);
    ...
    spec mint_and_refund(account: address, refund: u64) {
        ...
        aborts_if (refund != 0) && !exists<CoinInfo<CedraCoin>>(aptos_addr);
        ...
        aborts_if !exists<CedraCoinMintCapability>(@aptos_framework);
    ...
    spec store_cedra_coin_burn_cap(aptos_framework: &signer, burn_cap: BurnCapability<CedraCoin>) {
        use std::signer;
        ...
        ensures exists<AptosFABurnCapabilities>(addr) || exists<CedraCoinCapabilities>(addr);
    }

    /// Ensure caller is admin.
    /// Aborts if `CedraCoinMintCapability` already exists.
    spec store_cedra_coin_mint_cap(aptos_framework: &signer, mint_cap: MintCapability<CedraCoin>) {
        use std::signer;
        let addr = signer::address_of(aptos_framework);
        aborts_if !system_addresses::is_aptos_framework_address(addr);
