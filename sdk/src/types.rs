...
use cedra_crypto::{ed25519::Ed25519Signature, secp256r1_ecdsa, HashValue, PrivateKey, SigningKey};
use cedra_ledger::CedraLedgerError;
use cedra_rest_client::{cedra_api_types::MoveStructTag, Client, PepperRequest, ProverRequest};
pub use cedra_types::*;
use cedra_types::{
    event::EventKey,
...
lazy_static! {
    pub static ref CEDRA_METADATA_ADDRESS: AccountAddress = {
        let mut addr = [0u8; 32];
        addr[31] = 10u8;
...
/// LocalAccount represents an account on the Cedra blockchain. Internally it
...
pub fn get_paired_fa_primary_store_address(
    ...
    bytes.push(0xFC);
    AccountAddress::from_bytes(cedra_crypto::hash::HashValue::sha3_256_of(&bytes).to_vec()).unwrap()
}
...
pub fn get_paired_fa_metadata_address(coin_type_name: &MoveStructTag) -> AccountAddress {
    ...
    if coin_type_name == CEDRA_COIN_TYPE_STR {
        *CEDRA_METADATA_ADDRESS
    } else {
        let mut preimage = CEDRA_METADATA_ADDRESS.to_vec();
...
impl LocalAccount {
    ...
    /// does not actually create an account on the Cedra blockchain, just a
    ...
    /// create an account on the Cedra blockchain, it just generates a new
    /// account locally.
...
impl HardwareWalletAccount {
    ...
    /// This requires the Ledger device to be connected, unlocked and the Cedra app to be opened
    pub fn from_ledger(
        derivation_path: String,
        sequence_number: u64,
    ) -> Result<Self, CedraLedgerError> {
        let public_key = cedra_ledger::get_public_key(&derivation_path, false)?;
    ...
    pub fn sign_arbitrary_message(
        &self,
        message: &[u8],
    ) -> Result<Ed25519Signature, CedraLedgerError> {
        cedra_ledger::sign_message(&self.derivation_path, message)
    }
}
...
mod tests {
    ...
    use cedra_rest_client::{CedraBaseUrl, FaucetClient};
    use reqwest::Url;

    #[test]
    fn test_recover_account_from_derive_path() {
        // Same constants in test cases of TypeScript
        // https://github.com/cedra-labs/cedra-network/blob/main/ecosystem/typescript/sdk/src/cedra_account.test.ts
    ...
    async fn test_derive_keyless_account() {
        let cedra_rest_client = Client::builder(CedraBaseUrl::Devnet).build();
        // This JWT is taken from https://github.com/cedra-labs/cedra-ts-sdk/blob/f644e61beb70e69dfd489e75287c67b527385135/tests/e2e/api/keyless.test.ts#L11
        ...
        let mut account = derive_keyless_account(&cedra_rest_client, jwt, ephemeral_key_pair, None)
            .await
            .unwrap();
        ...
        let coin_client = CoinClient::new(&cedra_rest_client);
