...
const TARGETED_UNIT_TEST_PACKAGES_TO_IGNORE: [&str; 3] =
    ["cedra-testcases", "smoke-test", "cedra-keyless-circuit"];
...
impl CedraCargoCommand {
    fn command(&self) -> &'static str {
        match self {
            CedraCargoCommand::Check(_) => "check",
            CedraCargoCommand::Xclippy(_) => "clippy",
            CedraCargoCommand::Fmt(_) => "fmt",
            CedraCargoCommand::Nextest(_) => "nextest",
            CedraCargoCommand::Test(_) => "test",
    ...
    fn command_args(&self) -> &CommonArgs {
        match self {
            CedraCargoCommand::AffectedPackages(args) => args,
            CedraCargoCommand::ChangedFiles(args) => args,
            CedraCargoCommand::Check(args) => args,
            CedraCargoCommand::CheckMergeBase(args) => args,
            CedraCargoCommand::Xclippy(args) => args,
            CedraCargoCommand::Fmt(args) => args,
            CedraCargoCommand::Nextest(args) => args,
            CedraCargoCommand::TargetedCLITests(args) => args,
            CedraCargoCommand::TargetedCompilerV2Tests(args) => args,
            CedraCargoCommand::TargetedExecutionPerformanceTests(args) => args,
            CedraCargoCommand::TargetedFrameworkUpgradeTests(args) => args,
            CedraCargoCommand::TargetedUnitTests(args) => args,
            CedraCargoCommand::Test(args) => args,
        }
    }

    fn extra_opts(&self) -> Option<&[&str]> {
        match self {
            CedraCargoCommand::Xclippy(_) => Some(&[
                "-Dwarnings",
                "-Wclippy::all",
    ...
    pub fn execute(&self, package_args: &SelectedPackageArgs) -> anyhow::Result<()> {
        match self {
            CedraCargoCommand::AffectedPackages(_) => {
            ...
            CedraCargoCommand::ChangedFiles(_) => {
            ...
            CedraCargoCommand::TargetedCLITests(_) => {
                ...
                for package_path in affected_package_paths {
                    ...
                    // Check if the package is the Cedra CLI
                    if package_name == CEDRA_CLI_PACKAGE_NAME {
                        cli_affected = true; // The Cedra CLI was affected
                        break;
                    }
                }

                // If the Cedra CLI is affected, run the targeted CLI tests
                if cli_affected {
                ...
                println!("Skipping CLI tests as the Cedra CLI package was not affected!");
                Ok(())
            },
            CedraCargoCommand::TargetedCompilerV2Tests(_) => {
            ...
            CedraCargoCommand::TargetedExecutionPerformanceTests(_) => {
            ...
            CedraCargoCommand::TargetedFrameworkUpgradeTests(_) => {
            ...
            CedraCargoCommand::TargetedUnitTests(_) => {
...
fn run_targeted_cli_tests() -> anyhow::Result<()> {
    ...
    command.args(["-p", CEDRA_CLI_PACKAGE_NAME]);
    command.run(false);
...
mod tests {
    ...
    fn test_detect_relevant_changes() {
        ...
        let relevant_file_paths = vec![".github/actions/", "cedra-move/", "Cargo.lock", "crates/"];
        let relevant_package_names = vec!["cedra-node", "e2e-move-tests"];
