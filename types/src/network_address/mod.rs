...
/// ## Overview
///
/// Cedra `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a CedraNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the CedraNet concept of a
...
///
/// use cedra_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use cedra_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use cedra_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical CedraNet address with
    /// associated protocols.
    ///
    /// "typical" CedraNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_cedranet_addr());
    /// ```
    pub fn is_cedranet_addr(&self) -> bool {
        parse_cedranet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_cedranet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_cedranet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_cedranet_protos).prop_map(
        |(mut transport_protos, mut cedranet_protos)| {
            transport_protos.append(&mut cedranet_protos);
...
/// parse canonical cedranet protocols
///
/// See: [`NetworkAddress::is_cedranet_addr`]
fn parse_cedranet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_cedranet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_cedranet_addr()) {
        ...
        #[test]
        fn test_is_cedranet_addr(addr in arb_cedranet_addr()) {
            assert!(addr.is_cedranet_addr(), "addr.is_cedranet_addr() = false; addr: '{}'", addr);
        }
    }
}
