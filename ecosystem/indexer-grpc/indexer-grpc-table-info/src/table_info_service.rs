...
use cedra_api::context::Context as ApiContext;
use cedra_api_types::TransactionOnChainData;
use cedra_db_indexer::db_v2::IndexerAsyncV2;
use cedra_indexer_grpc_fullnode::stream_coordinator::{
    IndexerStreamCoordinator, TransactionBatchInfo,
};
use cedra_indexer_grpc_utils::counters::{log_grpc_step, IndexerGrpcStep};
use cedra_logger::{debug, error, info, sample, sample::SampleRate};
use cedra_types::write_set::WriteSet;
...
impl TableInfoService {
        ...
        let backup_is_enabled = match self.backup_restore_operator.clone() {
            Some(backup_restore_operator) => {
                ...
                let _task = tokio::spawn(async move {
                    loop {
                        cedra_logger::info!("[Table Info] Checking for snapshots to backup.");
    ...
    async fn backup_snapshot_if_present(
        ...
        if epochs_to_backup.is_empty() {
            ...
            cedra_logger::info!("[Table Info] No snapshot to backup. Skipping the backup.");
            return;
        }
        cedra_logger::info!(
