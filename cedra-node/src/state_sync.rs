...
use cedra_config::config::{NodeConfig, StateSyncConfig};
use cedra_consensus_notifications::ConsensusNotifier;
use cedra_data_client::{client::CedraDataClient, poller};
...
use cedra_storage_service_server::{
...
use cedra_types::waypoint::Waypoint;
...
pub fn create_event_subscription_service(
    ...
    let oracles_subscriptions = if node_config.base.role.is_validator() {
        ...
        let oracles_updated_events = event_subscription_service
            .subscribe_to_events(
                vec![],
                vec!["0x1::prices_storage::PriceUpdated".to_string()],
            )
...
pub fn start_state_sync_and_get_notification_handles(
    ...
    let (cedra_data_client, cedra_data_client_runtime) =
        setup_cedra_data_client(node_config, network_client, db_rw.reader.clone())?;

    // Start the data streaming service
    let state_sync_config = node_config.state_sync;
    let (streaming_service_client, streaming_service_runtime) =
        setup_data_streaming_service(state_sync_config, cedra_data_client.clone())?;
    ...
    let (mempool_notifier, mempool_listener) =
        cedra_mempool_notifications::new_mempool_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
                .max_pending_mempool_notifications,
        );
    let (consensus_notifier, consensus_listener) =
        cedra_consensus_notifications::new_consensus_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
...
fn setup_data_streaming_service(
    ...
    let data_streaming_service = DataStreamingService::new(
        state_sync_config.cedra_data_client,
    ...
    // Start the data streaming service
    let streaming_service_runtime = cedra_runtimes::spawn_named_runtime("stream-serv".into(), None);
...
/// Sets up the cedra data client runtime
fn setup_cedra_data_client(
    node_config: &NodeConfig,
    network_client: NetworkClient<StorageServiceMessage>,
    storage: Arc<dyn DbReader>,
) -> anyhow::Result<(CedraDataClient, Runtime)> {
    ...
    let cedra_data_client_runtime = cedra_runtimes::spawn_named_runtime("data-client".into(), None);

    // Create the data client and spawn the data poller
    let (cedra_data_client, data_summary_poller) = CedraDataClient::new(
        node_config.state_sync.cedra_data_client,
        node_config.base.clone(),
        TimeService::real(),
        storage,
        storage_service_client,
        Some(cedra_data_client_runtime.handle().clone()),
    );
    cedra_data_client_runtime.spawn(poller::start_poller(data_summary_poller));

    Ok((cedra_data_client, cedra_data_client_runtime))
}
...
fn setup_state_sync_storage_service(
    ...
    let storage_service_runtime = cedra_runtimes::spawn_named_runtime("stor-server".into(), None);
