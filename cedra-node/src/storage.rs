...
use cedra_config::{config::NodeConfig, utils::get_genesis_txn};
use cedra_db::{fast_sync_storage_wrapper::FastSyncStorageWrapper, CedraDB};
use cedra_db_indexer::db_indexer::InternalIndexerDB;
use cedra_executor::db_bootstrapper::maybe_bootstrap;
...
pub(crate) fn maybe_apply_genesis(
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        let ledger_info_opt =
            maybe_bootstrap::<CedraVMBlockExecutor>(db_rw, genesis, genesis_waypoint)
...
pub(crate) fn bootstrap_db(
    ...
    let (cedra_db_reader, db_rw, backup_service) = match FastSyncStorageWrapper::initialize_dbs(
        node_config,
...
/// In consensus-only mode, return a in-memory based [FakeCedraDB] and
/// do not run the backup service.
#[cfg(feature = "consensus-only-perf-test")]
pub(crate) fn bootstrap_db(
    node_config: &NodeConfig,
) -> Result<(Arc<dyn DbReader>, DbReaderWriter, Option<Runtime>)> {
    use cedra_db::db::fake_cedradb::FakeCedraDB;

    let cedra_db = CedraDB::open(
    ...
    let (cedra_db, db_rw) = DbReaderWriter::wrap(FakeCedraDB::new(cedra_db));
    maybe_apply_genesis(&db_rw, node_config)?;
    Ok((cedra_db, db_rw, None))
}
...
fn create_rocksdb_checkpoint_and_change_working_dir(
    ...
    CedraDB::create_checkpoint(
        &source_dir,
        &checkpoint_dir,
        node_config.storage.rocksdb_configs.enable_storage_sharding,
    )
    .expect("CedraDB checkpoint creation failed.");

    // Create a consensus db checkpoint
    cedra_consensus::create_checkpoint(&source_dir, &checkpoint_dir)
        .expect("ConsensusDB checkpoint creation failed.");

    // Create a state sync db checkpoint
    let state_sync_db =
        cedra_state_sync_driver::metadata_storage::PersistentMetadataStorage::new(&source_dir);
    state_sync_db
...
pub fn initialize_database_and_checkpoints(
    ...
    let (_cedra_db, db_rw, backup_service, indexer_db_opt, update_receiver) =
