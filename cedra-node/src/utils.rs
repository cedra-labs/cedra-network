...
use anyhow::anyhow;
use cedra_config::config::{NodeConfig, DEFAULT_EXECUTION_CONCURRENCY_LEVEL};
#[cfg(unix)]
...
cedra-node was compiled with feature flags that shouldn't be enabled.
...
PLEASE RECOMPILE CEDRA-NODE SEPARATELY using the following command:
    cargo build --package cedra-node
...
pub fn fetch_chain_id(db: &DbReaderWriter) -> anyhow::Result<ChainId> {
    let db_state_view = db
        ...
        .map_err(|err| anyhow!("[cedra-node] failed to create db state view {}", err))?;
    Ok(ChainIdResource::fetch_config(&db_state_view)
        .expect("[cedra-node] missing chain ID resource")
        .chain_id())
}

/// Sets the Cedra VM configuration based on the node configurations
pub fn set_cedra_vm_configurations(node_config: &NodeConfig) {
    ...
    CedraVM::set_concurrency_level_once(effective_concurrency_level as usize);
    CedraVM::set_discard_failed_blocks(node_config.execution.discard_failed_blocks);
    CedraVM::set_num_proof_reading_threads_once(
