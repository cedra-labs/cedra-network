...
use cedra_api::bootstrap as bootstrap_api;
use cedra_build_info::build_information;
use cedra_config::config::{merge_node_config, NodeConfig, PersistableConfig};
...
/// Runs an Cedra validator or fullnode
#[derive(Clone, Debug, Parser)]
#[clap(name = "Cedra Node", author, version)]
pub struct CedraNodeArgs {
    ...
    /// Paths to the Cedra framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
impl CedraNodeArgs {
    /// Runs an Cedra node based on the given command line arguments and config flags
    pub fn run(self) {
        #[cfg(target_os = "linux")]
...
pub fn start_and_report_ports(
    ...
    // Setup panic handler
    cedra_crash_handler::setup_panic_handler();

    // Create global rayon thread pool
    utils::create_global_rayon_pool(create_global_rayon_pool);

    // Initialize the global cedra-node-identity
    cedra_node_identity::init(config.get_peer_id())?;
...
pub fn start_test_environment_node(
    ...
    let cedra_root_key_path = test_dir.join("mint.key");
    ...
    println!("\tCedra root key path: {:?}", cedra_root_key_path);
    ...
    println!(
        "\tCedranet fullnode network endpoint: {}",
    ...
    println!("\nCedra is running, press ctrl-c to exit\n");
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| cedra_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let builder = cedra_genesis::builder::Builder::new(test_dir, framework.clone())?
        ...
        .with_init_genesis_config(Some(Arc::new(|genesis_config| {
            ...
            match env::var("ENABLE_KEYLESS_DEFAULT") {
                Ok(val) if val.as_str() == "1" => {
                    let response = ureq::get("https://api.devnet.cedralabs.com/v1/accounts/0x1/resource/0x1::keyless_account::Groth16VerificationKey").call();
    ...
    let mut key_file = fs::File::create(cedra_root_key_path)?;
    ...
    cedra_config::config::sanitize_node_config(validators[0].config.override_config_mut())?;
...
pub fn setup_environment_and_start_node(
    ...
    admin_service.set_cedra_db(db_rw.clone().into());

    // Set the Cedra VM configurations
    utils::set_cedra_vm_configurations(&node_config);

    // Obtain the chain_id from the DB
    let chain_id = utils::fetch_chain_id(&db_rw)?;

    // Set the chain_id in global CedraNodeIdentity
    cedra_node_identity::set_chain_id(chain_id)?;
    ...
    let (cedra_data_client, state_sync_runtimes, mempool_listener, consensus_notifier) =
...
fn verify_tool() {
    ...
    CedraNodeArgs::command().debug_assert()
}
