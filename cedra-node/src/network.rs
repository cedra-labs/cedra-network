...
use cedra_mempool::network::MempoolSyncMsg;
use cedra_network::{
    application::{
...
use cedra_network_benchmark::NetbenchMessage;
use cedra_network_builder::builder::NetworkBuilder;
use cedra_oracles_runtime::types::OraclesMessage;
use cedra_peer_monitoring_service_types::PeerMonitoringServiceMessage;
use cedra_storage_service_types::StorageServiceMessage;
...
pub fn consensus_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        cedra_consensus::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = cedra_consensus::network_interface::RPC.into();
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        rpc_protocols,
        cedra_channel::Config::new(node_config.consensus.max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(&cedra_consensus::counters::PENDING_CONSENSUS_NETWORK_EVENTS),
    );
...
pub fn dkg_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        cedra_dkg_runtime::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = cedra_dkg_runtime::network_interface::RPC.into();
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        cedra_channel::Config::new(node_config.dkg.max_network_channel_size)
            .queue_style(QueueStyle::FIFO),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn jwk_consensus_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        cedra_jwk_consensus::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = cedra_jwk_consensus::network_interface::RPC.into();
...
/// Returns the network application config for the Oracles client and service
pub fn oracles_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
...
pub fn mempool_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        cedra_channel::Config::new(node_config.mempool.max_network_channel_size)
            .queue_style(QueueStyle::KLAST) // TODO: why is this not FIFO?
            .counters(&cedra_mempool::counters::PENDING_MEMPOOL_NETWORK_EVENTS),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn peer_monitoring_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        cedra_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &cedra_peer_monitoring_service_server::metrics::PENDING_PEER_MONITORING_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn storage_service_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        cedra_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &cedra_storage_service_server::metrics::PENDING_STORAGE_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn netbench_network_configuration(
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        cedra_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(&cedra_network_benchmark::PENDING_NETBENCH_NETWORK_EVENTS),
    );
...
pub fn setup_networks_and_get_interfaces(
    ...
    Option<ApplicationNetworkInterfaces<OraclesMessage>>,
    ...
    let mut oracles_network_handle = None;
    ...
    for network_config in network_configs.into_iter() {
        ...
        if network_id.is_validator_network() {
            ...
            if oracles_network_handle.is_some() {
                panic!("There can be at most one validator network!");
            } else {
                let network_handle = register_client_and_service_with_network(
                    &mut network_builder,
                    network_id,
                    &network_config,
                    oracles_network_configuration(node_config),
                    true,
                );
                oracles_network_handle = Some(network_handle);
            }
        }
    ...
    if !netbench_handles.is_empty() {
        ...
        let netbench_runtime =
            cedra_runtimes::spawn_named_runtime("benchmark".into(), netbench_service_threads);
...
fn create_network_runtime(network_config: &NetworkConfig) -> Runtime {
    ...
    cedra_runtimes::spawn_named_runtime(thread_name, network_config.runtime_threads)
}
...
fn transform_network_handles_into_interfaces(
    ...
    oracles_network_handle: Option<ApplicationNetworkHandle<OraclesMessage>>,
    ...
    let oracles_interfaces = oracles_network_handle.map(|handle| {
        create_network_interfaces(
            vec![handle],
            oracles_network_configuration(node_config),
            peers_and_metadata.clone(),
