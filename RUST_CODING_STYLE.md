# Coding Guidelines for Cedra Core

This document describes the coding guidelines for the [CEdra Core](https://github.com/cedra-labs/cedra) Rust codebase. For the Move language, see the [Move Coding Conventions](https://cedra.dev/move/book/SUMMARY).
Secure coding guidance is provided in the [Cedra Rust Secure Coding Guidelines](./RUST_SECURE_CODING.md).
...
The Cedra codebase uses inclusive terminology (similar to other projects such as [the Linux kernel](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=49decddd39e5f6132ccd7d9fdc3d7c470b0061bb)). The terms below are recommended when appropriate.
...
Each major component of Cedra Core needs to have a `README.md` file. Major components are:

- top-level directories (e.g. `cedra-network/network`, `cedra-network/language`)
...
Refer to the Cedra Project contributing guide [LINK].

## License

Refer to the Cedra Project License [LINK].
```

A good example of README.md is `cedra-network/network/README.md` that describes the networking crate.
...
- `duration_since_epoch()` - to obtain the Unix time, call the function provided by `cedra-infallible`.
- `RwLock` and `Mutex` - Instead of calling `unwrap()` on the standard library implementations of these functions, use the infallible equivalent types that we provide in `cedra-infallible`.
...
Production crates are defined as the set of crates that create externally published artifacts, e.g. the Cedra validator,
...
feature activation. In the Cedra codebase, we therefore recommend that
