...
use cedra_types::{
    ...
    transaction::{
        EntryFunction, ExecutionStatus, FaAddress, Script, TransactionPayload, TransactionStatus,
    },
...
fn test_existing_account_with_fee_payer() {
    ...
    let alice_start = h.read_cedra_balance(alice.address());
    ...
    let alice_after = h.read_cedra_balance(alice.address());
    let bob_after = h.read_cedra_balance(bob.address());
...
fn test_account_not_exist_with_fee_payer() {
    ...
    let alice_start = h.read_resource::<CoinStoreResource<CedraCoinType>>(
        alice.address(),
    ...
    let alice_after = h.read_resource::<CoinStoreResource<CedraCoinType>>(
        alice.address(),
        CoinStoreResource::<CedraCoinType>::struct_tag(),
    );
...
fn test_account_not_exist_move_abort_with_fee_payer_out_of_gas() {
    ...
    let transaction = TransactionBuilder::new(new_alice.clone(), FaAddress::native_cedra())
        .fee_payer(cafe.clone())
        .payload(payload)
        .sequence_number(0)
