...
use cedra_vm::testing::{testing_only::inject_error_once, InjectedError};
...
fn assert_result(
    ...
    let start_balance = h.read_cedra_balance(account.address());
    ...
    let end_balance = h.read_cedra_balance(account.address());
