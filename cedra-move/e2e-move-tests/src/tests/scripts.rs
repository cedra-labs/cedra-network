...
use cedra_language_e2e_tests::account::TransactionBuilder;
use cedra_types::{
...
fn test_script_with_type_parameter() {
    ...
    let package = build_package(
        ...
        cedra_framework::BuildOptions::default(),
    )
    .expect("building package must succeed");
...
fn test_script_with_signer_parameter() {
    ...
    assert_eq!(
        ...
        TransactionStatus::Keep(
            cedra_types::transaction::ExecutionStatus::MiscellaneousError(Some(
                cedra_types::vm_status::StatusCode::INVALID_MAIN_FUNCTION_SIGNATURE
            ))
        )
    );
}
...
fn test_two_to_two_transfer() {
    ...
    let build_options = cedra_framework::BuildOptions {
        with_srcs: false,
        with_abis: false,
