...
use cedra_cached_packages::cedra_stdlib::{cedra_account_batch_transfer, cedra_account_transfer};
...
fn test_basic_fungible_token() {
    ...
    let root = h.cedra_framework_account();

    let mut build_options = cedra_framework::BuildOptions::default();
    build_options
        .named_addresses
...
fn test_coin_to_fungible_asset_migration() {
    let mut h = MoveHarness::new_with_features(vec![], vec![
        FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_CEDRA_STORE,
        FeatureFlag::OPERATIONS_DEFAULT_TO_FA_CEDRA_STORE,
...
fn test_prologue_speculation() {
    ...
    let sink_txn = harness.create_transaction_payload(
        &independent_account,
        cedra_account_batch_transfer(vec![AccountAddress::random(); 50], vec![10_000_000_000; 50]),
    );
    ...
    let transfer_1_txn =
        harness.create_transaction_payload(&dst_1, cedra_account_transfer(*dst_2.address(), 1));
    let transfer_2_txn =
        harness.create_transaction_payload(&dst_2, cedra_account_transfer(*dst_3.address(), 1));
    let transfer_3_txn =
        harness.create_transaction_payload(&dst_3, cedra_account_transfer(*dst_1.address(), 1));
