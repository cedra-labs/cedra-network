...
use crate::{assert_success, build_package, CedraPackageHooks};
use cedra_cached_packages::cedra_stdlib;
use cedra_framework::{natives::code::PackageMetadata, BuildOptions, BuiltPackage};
use cedra_gas_profiling::TransactionGasLog;
use cedra_gas_schedule::{
    CedraGasParameters, FromOnChainGasSchedule, InitialGasSchedule, ToOnChainGasSchedule,
};
...
use cedra_transaction_simulation::SimulationStateStore;
use cedra_types::{
...
impl MoveHarness {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(CedraPackageHooks {}));
        Self {
    ...
    /// Creates a [`MoveHarness`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn new_with_remote_state(network_url: CedraBaseUrl, txn_id: u64) -> Self {
    ...
    /// Gets the account where the Cedra framework is installed (0x1).
    pub fn cedra_framework_account(&mut self) -> Account {
    ...
    pub fn create_publish_built_package(
        ...
        self.create_transaction_payload(
            account,
            cedra_stdlib::code_publish_package_txn(
    ...
    pub fn create_object_code_deployment_built_package(
        ...
        self.create_transaction_payload(
            account,
            cedra_stdlib::object_code_deployment_publish(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
            ),
        )
    ...
    pub fn create_object_code_upgrade_built_package(
        ...
        self.create_transaction_payload(
            account,
            cedra_stdlib::object_code_deployment_upgrade(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
                code_object,
            ),
    ...
    pub fn object_code_freeze_code_object(
        ...
        let txn = self.create_transaction_payload(
            account,
            cedra_stdlib::object_code_deployment_freeze_code_object(code_object),
        );
        self.run(txn)
    }
    ...
    pub fn read_cedra_balance(&self, addr: &AccountAddress) -> u64 {
        ...
        .unwrap_or(0)
            + self
                .read_resource_from_resource_group::<FungibleStoreResource>(
                    &cedra_types::account_config::fungible_store::primary_cedra_store(*addr),
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.cedra_framework_account();
    ...
    fn override_one_gas_param(&mut self, param: &str, param_value: u64) {
        // TODO: The CedraGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = CedraGasParameters::initial()
            .to_on_chain_gas_schedule(cedra_gas_schedule::LATEST_GAS_FEATURE_VERSION);
        let entries = entries
            .into_iter()
            .map(|(name, val)| {
                if name == param {
        ...
        let gas_schedule = GasScheduleV2 {
            feature_version: cedra_gas_schedule::LATEST_GAS_FEATURE_VERSION,
            entries,
        };
        ...
        self.executor
            .exec("cedra_governance", "force_end_epoch", vec![], vec![
                core_signer_arg,
            ]);
    }
    ...
    pub fn modify_gas_schedule(&mut self, modify: impl FnOnce(&mut CedraGasParameters)) {
    ...
    pub fn get_gas_params(&self) -> (u64, CedraGasParameters) {
        let gas_schedule: GasScheduleV2 = self.get_gas_schedule();
        let feature_version = gas_schedule.feature_version;
        let params = CedraGasParameters::from_on_chain_gas_schedule(
            &gas_schedule.into_btree_map(),
            feature_version,
        )
        .unwrap();
...
macro_rules! enable_golden {
    ($h:expr) => {
        $h.internal_set_golden(
            std::file!(),
            cedra_language_e2e_tests::current_function_name!(),
        )
    };
}
...
macro_rules! assert_success {
    ($s:expr $(,)?) => {{
        assert_eq!($s, cedra_types::transaction::TransactionStatus::Keep(
            cedra_types::transaction::ExecutionStatus::Success))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
            cedra_types::transaction::TransactionStatus::Keep(
                cedra_types::transaction::ExecutionStatus::Success),
            $($arg)+
        )
    }};
...
macro_rules! assert_out_of_gas {
    ($s:expr $(,)?) => {{
        assert_eq!($s, cedra_types::transaction::TransactionStatus::Keep(
            cedra_types::transaction::ExecutionStatus::OutOfGas))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
...
macro_rules! assert_abort {
    ...
    ($s:expr, $c:ident $(,)?) => {{
        assert!(matches!(
            $s,
            cedra_types::transaction::TransactionStatus::Keep(
                cedra_types::transaction::ExecutionStatus::MoveAbort { code, .. }
            )
            if code == $c,
        ));
    }};
    ($s:expr, $c:pat $(,)?) => {{
        assert!(matches!(
            $s,
            cedra_types::transaction::TransactionStatus::Keep(
                cedra_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
