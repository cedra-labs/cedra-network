...
use cedra_framework::{ReleaseBundle, ReleasePackage};
use cedra_gas_schedule::{
    CedraGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use cedra_types::{
    account_config::{self, cedra_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS},
    chain_id::ChainId,
...
use cedra_vm::{
    ...
    move_vm_ext::{
        convert_modules_into_write_ops, CedraMoveResolver, GenesisMoveVm, GenesisRuntimeBuilder,
        SessionExt,
    },
};
use cedra_vm_types::{
...
const GOVERNANCE_MODULE_NAME: &str = "cedra_governance";
...
pub struct GenesisConfiguration {
    ...
    /// Per-network address of the published CedraOracle package.
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        ...
        entries: CedraGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_cedra_mainnet_genesis_transaction(
    ...
    let module_storage = state_view.as_cedra_code_storage(&genesis_runtime_environment);
...
pub fn encode_genesis_transaction(
    cedra_root_key: Ed25519PublicKey,
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(CEDRA_MAX_KNOWN_VERSION.major),
            ...
            MoveValue::Address(genesis_config.oracle_address),
        ]),
    );
}
...
fn allow_core_resources_to_set_version(
    ...
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(cedra_test_root_address())]),
    );
}
...
fn code_to_writes_for_publishing(
    ...
    let module_storage = genesis_state_view.as_cedra_code_storage(genesis_runtime_environment);
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => cedra_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => cedra_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            cedra_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => cedra_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => cedra_framework::testnet_release_bundle(),
    };
...
pub fn test_genesis_change_set_and_validators(
    ...
    generate_test_genesis(cedra_cached_packages::head_release_bundle(), count)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Validator {
    /// The Cedra account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The Cedra account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
pub fn generate_test_genesis(
    ...
    let genesis = encode_genesis_change_set(
        ...
        &GenesisConfiguration {
            ...
            // 1M CEDRA coins (with 8 decimals).
...
pub fn test_mainnet_end_to_end() {
    ...
    let transaction = encode_cedra_mainnet_genesis_transaction(
        &accounts,
        &employees,
        &validators,
        cedra_cached_packages::head_release_bundle(),
