...
use cedra_validator_interface::{
    CedraValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use cedra_vm::{
    cedra_vm::CedraVMBlockExecutor, data_cache::AsMoveResolver, CedraVM, VMBlockExecutor,
};
use cedra_vm_environment::environment::CedraEnvironment;
use cedra_vm_logging::log_schema::AdapterLogSchema;
use cedra_vm_types::{module_and_script_storage::AsCedraCodeStorage, output::VMOutput};
use itertools::Itertools;
use std::{path::Path, sync::Arc, time::Instant};

pub struct CedraDebugger {
    debugger: Arc<dyn CedraValidatorInterface + Send>,
}

impl CedraDebugger {
    pub fn new(debugger: Arc<dyn CedraValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
...
fn execute_block_no_limit(
    ...
    let executor = CedraVMBlockExecutor::new();
    executor
