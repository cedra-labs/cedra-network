...
use crate::cedra_debugger::CedraDebugger;
use anyhow::Result;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        ...
        let debugger = CedraDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?;
        ...
        println!(
            "Overview: https://cedrascan.com/txn/{:?}/userTxnOverview?network={}",
            version, network
        );
        println!(
            "Payload: https://cedrascan.com/txn/{:?}/payload?network={}",
            version, network
        );
