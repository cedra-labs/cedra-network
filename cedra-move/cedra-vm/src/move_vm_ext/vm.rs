...
use crate::move_vm_ext::{CedraMoveResolver, SessionExt, SessionId};
...
use cedra_vm_environment::{
    environment::CedraEnvironment,
    natives::cedra_natives_with_builder,
    prod_configs::{cedra_default_ty_builder, cedra_prod_vm_config},
};
use cedra_vm_types::storage::change_set_configs::ChangeSetConfigs;
...
impl GenesisRuntimeBuilder {
    ...
    pub fn new(chain_id: ChainId) -> Self {
        ...
        let vm_config = cedra_prod_vm_config(
        ...
        let natives = cedra_natives_with_builder(&mut native_builder, false);
...
impl GenesisMoveVm {
    ...
    pub fn new_genesis_session<'r, R: CedraMoveResolver>(
        &self,
        resolver: &'r R,
...
pub struct MoveVmExt {
    pub(crate) env: CedraEnvironment,
}

impl MoveVmExt {
    pub fn new(env: &CedraEnvironment) -> Self {
        Self { env: env.clone() }
    }
