...
use crate::cedra_vm::{serialized_signer, SerializedSigners};
use crate::CedraVM;
#[cfg(any(test, feature = "testing"))]
use crate::{
...
#[cfg(any(test, feature = "testing"))]
use cedra_types::{state_store::StateView, transaction::SignedTransaction};
#[cfg(any(test, feature = "testing"))]
use cedra_vm_logging::log_schema::AdapterLogSchema;
#[cfg(any(test, feature = "testing"))]
use cedra_vm_types::{
    module_and_script_storage::AsCedraCodeStorage, output::VMOutput,
...
impl CedraVM {
    ...
    pub fn test_failed_transaction_cleanup(
        ...
        let module_storage = state_view.as_cedra_code_storage(self.runtime_environment());
