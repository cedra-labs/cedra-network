...
use crate::{
    block_executor::{CedraTransactionOutput, CedraVMBlockExecutorWrapper},
    counters::*,
    ...
    move_vm_ext::{
        ...
        CedraMoveResolver, MoveVmExt, SessionExt, SessionId, UserTransactionContext,
    },
...
use cedra_framework::natives::code::PublishRequest;
use cedra_gas_algebra::{Gas, GasQuantity, NumBytes, Octa};
use cedra_gas_meter::{CedraGasMeter, GasAlgebra};
use cedra_gas_schedule::{
    gas_feature_versions::{RELEASE_V1_10, RELEASE_V1_27},
    CedraGasParameters, VMGasParameters,
};
use cedra_logger::{enabled, prelude::*, Level};
...
use cedra_types::{
    ...
    }, oracles::{GLOBAL_ORCLE_PRICES}, randomness::Randomness, state_store::{StateView, TStateView}, transaction::{
...
use cedra_vm_types::{
    ...
    module_and_script_storage::{
        code_storage::CedraCodeStorage, module_storage::CedraModuleStorage, AsCedraCodeStorage,
    },
...
pub(crate) fn get_system_transaction_output(
    session: SessionExt<impl CedraMoveResolver>,
...
impl CedraVM {
    ...
    pub fn new(env: &CedraEnvironment, state_view: &impl StateView) -> Self {
        let resolver = state_view.as_move_resolver();
        let module_storage = state_view.as_cedra_code_storage(env);
    ...
    pub(crate) fn gas_params(
        &self,
        log_context: &AdapterLogSchema,
    ) -> Result<&CedraGasParameters, VMStatus> {
    ...
    #[inline(always)]
    pub fn environment(&self) -> CedraEnvironment {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn gas_params_for_test(&self) -> Result<&CedraGasParameters, VMStatus> {
    ...
    fn custom_fee_statement_from_gas_meter(
        ...
        let stablecoin: String = txn_data.fa_address.clone().to_string();

        let gas_used = Self::convert_fee_price(stablecoin.clone(), Self::gas_used(txn_data.max_gas_amount(), gas_meter));
        CustomFeeStatement::new(
            gas_used,
            Self::convert_fee_price(stablecoin.clone(), u64::from(gas_meter.execution_gas_used())),
            Self::convert_fee_price(stablecoin.clone(), u64::from(gas_meter.io_gas_used())),
            Self::convert_fee_price(stablecoin.clone(), u64::from(gas_meter.storage_fee_used())),
            Self::convert_fee_price(stablecoin.clone(), storage_fee_refund),
        )
    ...
    fn convert_fee_price(stablecoin: String, cedra_amount: u64) -> u64 {
        if cedra_amount == 0 {
            return  0
        }

        let prices = GLOBAL_ORCLE_PRICES.clone();

        let price_list = prices.read().unwrap();
        let cedra_price_info = price_list.iter().find(|p| p.fa_address == CedraCoinType::type_tag().to_string()).unwrap().clone();
        let stablecoin_price_info = price_list.iter().find(|p| p.fa_address == stablecoin).unwrap().clone();
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[cedra-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[cedra-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
    ...
    fn validate_and_execute_script<'a>(
        ...
        // Note: cannot use CedraGasMeter because it is not implemented for
        //       UnmeteredGasMeter.
    ...
    fn execute_script_or_entry_function<'a, 'r>(
        ...
        fail_point!("cedra_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_signed_transaction(
        ...
        let keyless_authenticators = cedra_types::keyless::get_authenticators(transaction)
    ...
    fn execute_user_transaction_impl(
        ...
        let _timer = VM_TIMER.timer_with_label("CedraVM::execute_user_transaction_impl");
        ...
        let should_create_account_resource_timer =
            VM_TIMER.timer_with_label("CedraVM::create_account_resource_lazily");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("CedraVM::execute_user_transaction_impl [payload]");
    ...
    where
        C: CedraCodeStorage + BlockSynchronizationKillSwitch,
        G: CedraGasMeter,
    ...
    pub fn execute_user_transaction_with_modified_gas_meter<'a, G, F>(
        &self,
        resolver: &'a impl CedraMoveResolver,
        code_storage: &'a (impl CedraCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        modify_gas_meter: F,
    ) -> Result<(VMStatus, VMOutput, G), VMStatus>
    ...
    pub fn execute_user_transaction(
        &self,
        resolver: &impl CedraMoveResolver,
        code_storage: &(impl CedraCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
    ) -> (VMStatus, VMOutput) {
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[cedra_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[cedra_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    fn gas_used(max_gas_amount: Gas, gas_meter: &impl CedraGasMeter) -> u64 {
        max_gas_amount
            .checked_sub(gas_meter.balance())
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("cedra_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("cedra_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("cedra_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[cedra_vm] Transaction breaking invariant violation. txn: {:?}, status: {:?}",
                        ...
                        if vm_status.sub_status()
                            == Some(move_core_types::vm_status::sub_status::unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[cedra_vm] Transaction breaking paranoid mode. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct CedraVMBlockExecutor {
    ...
    module_cache_manager: CedraModuleCacheManager,
}

impl CedraVMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        fail_point!("cedra_vm_block_executor::execute_block_with_config", |_| {
            Err(VMStatus::error(
                StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
                None,
            ))
        ...
        let result = CedraVMBlockExecutorWrapper::execute_block::<
            _,
...
impl VMBlockExecutor for CedraVMBlockExecutor {
    fn new() -> Self {
        Self {
            module_cache_manager: CedraModuleCacheManager::new(),
        }
    }
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                concurrency_level: CedraVM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: CedraVM::get_discard_failed_blocks(),
...
impl VMValidator for CedraVM {
    ...
    fn validate_transaction(
        ...
        {
            if let cedra_types::transaction::authenticator::TransactionAuthenticator::SingleSender{ .. } = transaction.authenticator_ref() {
...
// Ensure encapsulation of CedraVM APIs by using a wrapper.
pub struct CedraSimulationVM;

impl CedraSimulationVM {
    ...
    pub fn create_vm_and_simulate_signed_transaction(
        ...
        let env = CedraEnvironment::new(state_view);
        let mut vm = CedraVM::new(&env, state_view);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, CedraVM};
    use cedra_types::{
        account_address::AccountAddress,
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(CedraVM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(CedraVM::should_restart_execution(&[(
