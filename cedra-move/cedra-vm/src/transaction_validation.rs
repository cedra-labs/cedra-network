...
pub static CEDRA_TRANSACTION_VALIDATION: Lazy<TransactionValidation> =
        ...
        || features.is_derivable_account_abstraction_enabled()
    {
            ...
            if let (Some(_fee_payer), Some(fee_payer_auth_key)) = (
                ...
                (
                    if features.is_transaction_payload_v2_enabled() {
                        &CEDRA_TRANSACTION_VALIDATION.unified_prologue_fee_payer_v2_name
                    } else {
                        &CEDRA_TRANSACTION_VALIDATION.unified_prologue_fee_payer_name
                    },
                    serialized_args,
                )
            } else {
                ...
                (
                    if features.is_transaction_payload_v2_enabled() {
                        &CEDRA_TRANSACTION_VALIDATION.unified_prologue_v2_name
                    } else {
                        &CEDRA_TRANSACTION_VALIDATION.unified_prologue_name
                    },
                    serialized_args,
                )
            };
        ...
        let (prologue_function_name, args) = if let (Some(fee_payer), Some(fee_payer_auth_key)) = (
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CEDRA_TRANSACTION_VALIDATION.fee_payer_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&CEDRA_TRANSACTION_VALIDATION.fee_payer_prologue_name, args)
            }
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CEDRA_TRANSACTION_VALIDATION.multi_agent_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (
                    &CEDRA_TRANSACTION_VALIDATION.multi_agent_prologue_name,
                    args,
                )
            }
        } else {
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CEDRA_TRANSACTION_VALIDATION.script_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&CEDRA_TRANSACTION_VALIDATION.script_prologue_name, args)
            }
        };
...
fn run_epilogue(
    ...
    if txn_data.v2_fee_event && features.is_emit_custom_fee_statement_enabled(){
...
pub(crate) fn run_failure_epilogue(
    ...
    .or_else(|err| {
        expect_only_successful_execution(
            err,
            CEDRA_TRANSACTION_VALIDATION.user_epilogue_name.as_str(),
            log_context,
        )
    })
}
