...
use cedra_gas_algebra::{FeePerGasUnit, Gas, GasExpression};
use cedra_gas_schedule::{CedraGasParameters, InitialGasSchedule, LATEST_GAS_FEATURE_VERSION};
...
impl AUTransactionGen for InsufficientBalanceGen {
    fn apply(
        ...
        let gas_params = CedraGasParameters::initial();
...
pub struct InvalidAuthkeyGen {
    sender: Index,
    #[proptest(
        strategy = "cedra_crypto::test_utils::uniform_keypair_strategy_with_perturbation(1)"
    )]
