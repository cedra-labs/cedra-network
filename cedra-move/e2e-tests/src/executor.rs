...
use cedra_abstract_gas_usage::CalibrationAlgebra;
use cedra_bitvec::BitVec;
use cedra_block_executor::{
    code_cache_global_manager::CedraModuleCacheManager, txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::default::DefaultTxnProvider,
};
use cedra_crypto::HashValue;
use cedra_framework::ReleaseBundle;
use cedra_gas_algebra::DynamicExpression;
use cedra_gas_meter::{CedraGasMeter, GasAlgebra, StandardGasAlgebra, StandardGasMeter};
use cedra_gas_profiling::{GasProfiler, TransactionGasLog};
use cedra_keygen::KeyGen;
use cedra_rest_client::CedraBaseUrl;
...
use cedra_types::{
    ...
    move_utils::MemberId,
    on_chain_config::{
        CedraVersion, CurrentTimeMicroseconds, FeatureFlag, Features, OnChainConfig, ValidatorSet,
    },
...
use cedra_validator_interface::{DebuggerStateView, RestDebuggerInterface};
use cedra_vm::{
    block_executor::{CedraTransactionOutput, CedraVMBlockExecutorWrapper},
    data_cache::AsMoveResolver,
    gas::make_prod_gas_meter,
    move_vm_ext::{CedraMoveResolver, MoveVmExt, SessionExt, SessionId},
    CedraVM, VMValidator,
};
use cedra_vm_environment::environment::CedraEnvironment;
use cedra_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
use cedra_vm_logging::log_schema::AdapterLogSchema;
use cedra_vm_types::{
    module_and_script_storage::{module_storage::CedraModuleStorage, AsCedraCodeStorage},
...
///
/// This struct is a mock in-memory implementation of the Cedra executor.
...
impl FakeExecutor {
    ...
    fn from_remote_state_impl(
        network_url: CedraBaseUrl,
        txn_id: u64,
        api_key: Option<&str>,
    ) -> Self {
        let mut builder = cedra_rest_client::Client::builder(network_url);
    ...
    ///
    /// If you hit rate limits, you can create a free Cedra Build account and generate an API key:
    /// - https://build.cedralabs.com/docs/start#api-quick-start
    ...
    /// Creates a [`FakeExecutor`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn from_remote_state(network_url: CedraBaseUrl, txn_id: u64) -> Self {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let cedra_version =
                CedraVersion::fetch_config(&self.state_store).map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, cedra_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn stdlib_only_genesis() -> Self {
        ...
        for (bytes, module) in
            cedra_cached_packages::head_release_bundle().code_and_compiled_modules()
        {
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = cedra_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn new_account_data_at(&mut self, addr: AccountAddress) -> AccountData {
        ...
        // Mint the account 10M Cedra coins (with 8 decimals).
    ...
    pub fn store_and_fund_account(
        ...
        let use_fa_balance = features.is_enabled(FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_CEDRA_STORE);
    ...
    fn execute_transaction_block_impl_with_state_view(
        ...
        CedraVMBlockExecutorWrapper::execute_block_on_thread_pool::<
            _,
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = CedraEnvironment::new(&self.state_store);
        let vm = CedraVM::new(&env, self.get_state_view());

        let resolver = self.state_store.as_move_resolver();
        let code_storage = self.get_state_view().as_cedra_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.state_store,
            &self.state_store.as_cedra_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        // cached.
        let module_storage = self.state_store.as_cedra_code_storage(&env);
    ...
    pub fn exec_abstract_usage(
        ...
        let (write_set, _events) = {
            let env = CedraEnvironment::new_with_gas_hook(
                &self.state_store,
...
/// responsibility of the adapter, i.e., [CedraVM]).
