...
use cedra_crypto::{ed25519::Ed25519PrivateKey, ValidCryptoMaterialStringExt};
use cedra_framework::natives::code::PackageRegistry;
use cedra_gas_schedule::LATEST_GAS_FEATURE_VERSION;
use cedra_release_builder::{
    components::fetch_config,
    initialize_cedra_core_path,
...
pub struct Argument {
    ...
    cmd: Commands,
    #[clap(long)]
    cedra_core_path: Option<PathBuf>,
}

// TODO(vgao1996): unify with `ReplayNetworkSelection` in the `cedra` crate.
#[derive(Clone, Debug)]
...
pub enum InputOptions {
    FromDirectory {
        /// Path to the localnet folder. If you are running localnet via cli, it should be `.cedra/testnet`.
        #[clap(short, long)]
        test_dir: PathBuf,
    },
    FromArgs {
...
async fn main() -> anyhow::Result<()> {
    ...
    initialize_cedra_core_path(args.cedra_core_path.clone());
    ...
    match args.cmd {
        Commands::GenerateProposals {
            ...
            cedra_release_builder::ReleaseConfig::load_config(release_config.as_path())
        ...
        Commands::WriteDefault { output_path } => {
            cedra_release_builder::ReleaseConfig::default().save_config(output_path.as_path())
        },
        Commands::ValidateProposals {
            ...
            let mut network_config = match input_option {
                ...
                InputOptions::FromArgs {
                    ...
                    cedra_release_builder::validate::NetworkConfig {
                        root_key_path,
                        validator_account,
                        validator_key,
            ...
            if mint_to_validator {
                let chain_id = cedra_rest_client::Client::new(endpoint)
            ...
            network_config
                ...
                .await?;
            cedra_release_builder::validate::validate_config_and_generate_release(
                config,
        ...
        Commands::GenerateGasSchedule {
            ...
            let gas_schedule = cedra_gas_schedule_updator::current_gas_schedule(version);
        ...
        Commands::PrintConfigs {
            ...
            let client = cedra_rest_client::Client::new(endpoint);
            ...
            print_configs!(OnChainConsensusConfig, OnChainExecutionConfig, CedraVersion);
            ...
            println!(
                ...
                serde_yaml::to_string(
                    &cedra_release_builder::components::feature_flags::Features::from(&features)
                )?
            );
