...
use cedra_vm::{data_cache::AsMoveResolver, move_vm_ext::SessionId, CedraVM};
use cedra_vm_environment::environment::CedraEnvironment;
use cedra_vm_logging::log_schema::AdapterLogSchema;
use cedra_vm_types::{
    module_and_script_storage::AsCedraCodeStorage, module_write_set::ModuleWriteSet,
 ...
 *
 **************************************************************************************************/
static MODULE_ID_CEDRA_GOVERNANCE: Lazy<ModuleId> = Lazy::new(|| {
    ModuleId::new(
        AccountAddress::ONE,
        Identifier::new("cedra_governance").unwrap(),
    )
});
...
/// Patches `cedra_framework::cedra_governance::resolve_multi_step_proposal` so that
/// it returns the requested signer directly, skipping the governance process altogether.
fn patch_cedra_governance(
    state_view: &impl SimulationStateStore,
    forbid_next_execution_hash: bool,
) -> Result<()> {
    use Bytecode::*;

    patch_module(state_view, &MODULE_ID_CEDRA_GOVERNANCE, |m| {
...
pub async fn simulate_multistep_proposal(
    ...
    for path in proposal_scripts {
        let framework_package_args = FrameworkPackageArgs::try_parse_from([
            ...
            "--framework-local-dir",
            &cedra_framework_path().to_string_lossy(),
    ...
    let debugger =
        CedraDebugger::rest_client(client.clone()).context("failed to create CedraDebugger")?;
    ...
    std::io::stdout().flush()?;
    let mut rng = cedra_keygen::KeyGen::from_seed([0; 32]);
    let balance = 100 * 1_0000_0000; // 100 Cedra
        ...
        proposal_scripts.iter().zip(compiled_scripts).enumerate()
    {
        ...
        patch_cedra_governance(&state_view, forbid_next_execution_hash)
