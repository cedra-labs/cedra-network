...
use crate::{cedra_core_path, components::get_execution_hash};
use anyhow::Result;
use cedra_crypto::HashValue;
use cedra_framework::{BuildOptions, BuiltPackage, ReleasePackage};
...
pub struct FrameworkReleaseConfig {
    ...
    /// If set to None, we will use the cedra framework under current repo.
...
pub fn generate_upgrade_proposals(
    ...
    let mut package_path_list = [
        ("0x1", "cedra-move/framework/move-stdlib"),
        ("0x1", "cedra-move/framework/cedra-stdlib"),
        ("0x1", "cedra-move/framework/cedra-framework"),
        ("0x3", "cedra-move/framework/cedra-token"),
        ("0x4", "cedra-move/framework/cedra-token-objects"),
    ];
    ...
    let commit_info = if let Some(revision) = &config.git_hash {
        ...
        let repository = Repository::clone(CEDRA_GIT_PATH, temp_root_path.path())?;
    ...
    for (publish_addr, relative_package_path) in package_path_list.iter() {
        ...
        // 0-move-stdlib.move	2-cedra-framework.move	4-gas-schedule.move	6-features.move
        // 1-cedra-stdlib.move	3-cedra-token.move	5-version.move		7-consensus-config.move
        // The first framework file being generated is 3-cedra-token.move. It's using the next_execution_hash being passed in (so in this case, the hash of 4-gas-schedule.move being passed in mod.rs).
        // The second framework file being generated would be 2-cedra-framework.move, and it's using the hash of 3-cedra-token.move (which would be result.last()).
