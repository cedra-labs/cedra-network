...
use cedra_types::on_chain_config::GasScheduleV2;
use clap::Parser;
...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use cedra_framework::cedra_governance;");
    emitln!(writer, "use cedra_framework::gas_schedule;");
    emitln!(writer);
    ...
    emitln!(
        writer,
        "let framework_signer = cedra_governance::resolve(proposal_id, @{});\n",
        AccountAddress::ONE,
    );
...
fn cedra_framework_path() -> PathBuf {
    Path::join(
        Path::new(env!("CARGO_MANIFEST_DIR")),
        "../framework/cedra-framework",
    )
}
...
pub fn current_gas_schedule(feature_version: u64) -> GasScheduleV2 {
    GasScheduleV2 {
        feature_version,
        entries: CedraGasParameters::initial().to_on_chain_gas_schedule(feature_version),
    }
}
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("CedraFramework", &cedra_framework_path().to_string_lossy());
