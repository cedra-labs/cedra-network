...
use cedra_transaction_workloads_lib::{EntryPoints, LoopType, MapType, OrderBookState};
use cedra_types::{account_address::AccountAddress, transaction::TransactionPayload};
use clap::Parser;
...
fn get_parsed_calibration_values() -> HashMap<String, CalibrationInfo> {
    let calibration_values =
        fs::read_to_string("cedra-move/e2e-benchmark/data/calibration_values.tsv")
...
fn main() {
    ...
    for (index, (flow, entry_point)) in entry_points.into_iter().enumerate() {
        ...
        json_lines.push(json!({
            "grep": "grep_json_cedra_move_vm_perf",
    ...
    assert_eq!(
        0,
        cedra_logger::ERROR_LOG_COUNT.get(),
