...
use crate::{
    ...
    natives::cedra_natives_with_builder,
    prod_configs::{
        cedra_default_ty_builder, cedra_prod_ty_builder, cedra_prod_vm_config,
        get_timed_feature_override,
    },
};
use cedra_gas_algebra::DynamicExpression;
use cedra_gas_schedule::{CedraGasParameters, MiscGasParameters, NativeGasParameters};
use cedra_native_interface::SafeNativeBuilder;
use cedra_types::{
    chain_id::ChainId,
    on_chain_config::{
...
pub struct CedraEnvironment(Arc<Environment>);

impl CedraEnvironment {
    ...
    /// on-chain.
    #[inline]
    pub fn gas_params(&self) -> &Result<CedraGasParameters, String> {
        &self.0.gas_params
    }
...
impl Clone for CedraEnvironment {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

impl PartialEq for CedraEnvironment {
    fn eq(&self, other: &Self) -> bool {
        self.0.hash == other.0.hash
    }
}

impl Eq for CedraEnvironment {}

impl WithRuntimeEnvironment for CedraEnvironment {
...
struct Environment {
    ...
    /// on-chain.
    gas_params: Result<CedraGasParameters, String>,
...
impl Environment {
    fn new(
        ...
        let (native_gas_params, misc_gas_params, ty_builder) = match &gas_params {
            Ok(gas_params) => {
                let ty_builder = cedra_prod_ty_builder(gas_feature_version, gas_params);
                (
            ...
            Err(_) => {
                let ty_builder = cedra_default_ty_builder();
                (
        ...
        let natives = cedra_natives_with_builder(&mut builder, inject_create_signer_for_gov_sim);
        let vm_config =
            cedra_prod_vm_config(gas_feature_version, &features, &timed_features, ty_builder);
...
pub mod tests {
    ...
    fn test_environment_eq() {
        let state_view = MockStateView::empty();
        let environment_1 = CedraEnvironment::new(&state_view);
        let environment_2 = CedraEnvironment::new(&state_view);
    ...
    fn test_environment_ne() {
        ...
        for i in 0..state_views.len() {
            for j in 0..state_views.len() {
                if i != j {
                    let environment_1 = CedraEnvironment::new(&state_views[i]);
                    let environment_2 = CedraEnvironment::new(&state_views[j]);
                    assert!(environment_1 != environment_2);
                }
            }
        }
    ...
    fn test_environment_with_injected_create_signer_for_gov_sim() {
        ...
        let not_injected_envs = [
            CedraEnvironment::new(&state_view),
            CedraEnvironment::new_with_gas_hook(&state_view, Arc::new(|_| {})),
        ...
        // Injected.
        let env = CedraEnvironment::new_with_injected_create_signer_for_gov_sim(&state_view);
        #[allow(deprecated)]
