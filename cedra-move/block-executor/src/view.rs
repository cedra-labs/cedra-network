...
pub(crate) struct ParallelState<'a, T: Transaction> {
    ...
    pub(crate) captured_reads:
        RefCell<CapturedReads<T, ModuleId, CompiledModule, Module, CedraModuleExtension>>,
}

fn get_delayed_field_value_impl<T: Transaction>(
    captured_reads: &RefCell<
        CapturedReads<T, ModuleId, CompiledModule, Module, CedraModuleExtension>,
    >,
...
pub(crate) struct LatestView<'a, T: Transaction, S: TStateView<Key = T::Key>> {
    ...
    pub(crate) global_module_cache:
        &'a GlobalModuleCache<ModuleId, CompiledModule, Module, CedraModuleExtension>,
...
mod test {
    ...
    use cedra_vm_types::resolver::TResourceView;
