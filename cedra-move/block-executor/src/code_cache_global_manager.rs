...
macro_rules! alert_or_println {
    ($($arg:tt)*) => {
        if cfg!(any(test, feature = "testing")) {
            ...
            use cedra_vm_logging::{alert, prelude::CRITICAL_ERRORS};
...
pub struct ModuleCacheManager<K, D, V, E> {
    ...
    environment: Option<CedraEnvironment>,
    ...
    E: WithSize,
{
    ...
    fn check_ready(
        &mut self,
        storage_environment: CedraEnvironment,
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<(), VMStatus> {
...
/// Module cache manager used by Cedra block executor. Ensures that only one thread has exclusive
/// access to it at a time.
pub struct CedraModuleCacheManager {
    inner: Mutex<ModuleCacheManager<ModuleId, CompiledModule, Module, CedraModuleExtension>>,
}

impl CedraModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [CedraModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        Ok(match self.inner.try_lock() {
            ...
            None => {
                ...
                CedraModuleCacheManagerGuard::None {
    ...
    /// Tries to lock the manager using [CedraModuleCacheManager::try_lock_inner]. Additionally, if
    /// the module cache is empty, can prefetch Cedra framework into it.
    pub fn try_lock(
        &self,
        state_view: &impl StateView,
        config: &BlockExecutorModuleCacheLocalConfig,
        ...
        if guard.module_cache().num_modules() == 0 && config.prefetch_framework_code {
            prefetch_cedra_framework(state_view, &mut guard).map_err(|err| {
                alert_or_println!("Failed to load Cedra framework to module cache: {:?}", err);
...
/// A guard that can be acquired from [CedraModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum CedraModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [CedraModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
            ModuleCacheManager<ModuleId, CompiledModule, Module, CedraModuleExtension>,
        >,
    },
    /// Either there is no [CedraModuleCacheManager], or acquiring the lock for it failed.
    None {
        environment: CedraEnvironment,
        module_cache: GlobalModuleCache<ModuleId, CompiledModule, Module, CedraModuleExtension>,
    },
}

impl<'a> CedraModuleCacheManagerGuard<'a> {
    /// Returns the references to the environment. If environment is not set, panics.
    pub fn environment(&self) -> &CedraEnvironment {
        use CedraModuleCacheManagerGuard::*;
        match self {
            Guard { guard } => guard
                .environment
                .as_ref()
    ...
    pub fn module_cache(
        &self,
    ) -> &GlobalModuleCache<ModuleId, CompiledModule, Module, CedraModuleExtension> {
        use CedraModuleCacheManagerGuard::*;
        match self {
    ...
    pub fn module_cache_mut(
        &mut self,
    ) -> &mut GlobalModuleCache<ModuleId, CompiledModule, Module, CedraModuleExtension> {
        use CedraModuleCacheManagerGuard::*;
        match self {
    ...
    /// A guard in [CedraModuleCacheManagerGuard::None] state with empty module cache and default
    /// environment. Use for testing only.
    #[cfg(test)]
    pub(crate) fn none() -> Self {
        use cedra_types::state_store::MockStateView;
        Self::none_for_state_view(&MockStateView::empty())
    }

    /// A guard in [CedraModuleCacheManagerGuard::None] state with empty module cache and the
    ...
    pub(crate) fn none_for_state_view(state_view: &impl StateView) -> Self {
        CedraModuleCacheManagerGuard::None {
            environment: CedraEnvironment::new(state_view),
            module_cache: GlobalModuleCache::empty(),
        }
    }
}

/// If Cedra framework exists, loads "transaction_validation.move" and all its transitive
/// dependencies from storage into provided module cache. If loading fails for any reason, a panic
/// error is returned.
fn prefetch_cedra_framework(
    state_view: &impl StateView,
    guard: &mut CedraModuleCacheManagerGuard,
) -> Result<(), PanicError> {
    let code_storage = state_view.as_cedra_code_storage(guard.environment());
    ...
    let maybe_loaded = code_storage
        ...
        .map_err(|err| {
            ...
            PanicError::CodeInvariantError(format!("Unable to fetch Cedra framework: {:?}", err))
        })?;
...
mod test {
    use super::*;
    use cedra_transaction_simulation::InMemoryStateStore;
    use cedra_types::{
    ...
    #[test]
    fn test_prefetch_existing_cedra_framework() {
        let state_view = InMemoryStateStore::from_head_genesis();

        let mut guard = CedraModuleCacheManagerGuard::none_for_state_view(&state_view);
        assert_eq!(guard.module_cache().num_modules(), 0);

        let result = prefetch_cedra_framework(&state_view, &mut guard);
        assert!(result.is_ok());
        assert!(guard.module_cache().num_modules() > 0);
    }

    #[test]
    fn test_prefetch_non_existing_cedra_framework() {
    ...
    fn test_check_ready() {
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_1));
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_2));
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_3));
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_4));
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_5));
        ...
        assert_ok!(manager.check_ready(CedraEnvironment::new(&state_view), &config, metadata_6));
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = CedraModuleCacheManager::new();
        ...
        assert!(matches!(guard, CedraModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(CedraModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, CedraModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }
