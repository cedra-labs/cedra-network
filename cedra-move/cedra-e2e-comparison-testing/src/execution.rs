...
use crate::{
    check_cedra_packages_availability, compile_cedra_packages, compile_package,
    data_state_view::DataStateView, generate_compiled_blob, is_cedra_package, CompilationCache,
    DataManager, IndexReader, PackageInfo, TxnIndex, CEDRA_COMMONS,
};
use anyhow::Result;
use cedra_framework::CEDRA_PACKAGES;
use cedra_language_e2e_tests::executor::FakeExecutor;
use cedra_transaction_simulation::{InMemoryStateStore, SimulationStateStore};
use cedra_types::{
...
use cedra_validator_interface::CedraValidatorInterface;
use clap::ValueEnum;
...
fn add_cedra_packages_to_state_store(
    state_store: &impl SimulationStateStore,
...
impl Execution {
    ...
    pub async fn execute_txns(&self, begin: Version, num_txns_to_execute: u64) -> Result<()> {
        let cedra_commons_path = self.input_path.join(CEDRA_COMMONS);
        if !check_cedra_packages_availability(cedra_commons_path.clone()) {
            return Err(anyhow::Error::msg("cedra packages are missing"));
        }

        let mut compiled_cache = CompilationCache::default();
        if self.execution_mode.is_v1_or_compare() {
            compile_cedra_packages(
                &cedra_commons_path,
    ...
    fn execute_one_txn(
        ...
        if let Some(txn_idx) = data_manager.get_txn_index(cur_version) {
            ...
            if txn_idx.package_info.is_compilable()
                && !is_cedra_package(&txn_idx.package_info.package_name)
            {
    ...
    pub(crate) fn execute_and_compare(
        ...
        debugger: Option<Arc<dyn CedraValidatorInterface + Send>>,
    ) {
    ...
    fn execute_code(
        ...
        txn: &Transaction,
        compiled_package_cache: &HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
        debugger_opt: Option<Arc<dyn CedraValidatorInterface + Send>>,
        v2_flag: bool,
    ) -> Result<(WriteSet, Vec<ContractEvent>), VMStatus> {
        // Always add Cedra (0x1) packages.
        add_cedra_packages_to_state_store(&state, compiled_package_cache);
