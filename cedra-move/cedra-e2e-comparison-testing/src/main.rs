...
use anyhow::Result;
use cedra_comparison_testing::{
    prepare_cedra_packages, DataCollection, Execution, ExecutionMode, OnlineExecutor, CEDRA_COMMONS,
};
...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.cedralabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.cedralabs.com/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.cedralabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.cedralabs.com/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Dump {
            ...
            if !skip_source_code {
                prepare_cedra_packages(output.join(CEDRA_COMMONS)).await;
            }
        ...
        Cmd::Execute {
            ...
            prepare_cedra_packages(input.join(CEDRA_COMMONS)).await;
