...
use cedra_framework::{
    natives::code::PackageMetadata, unzip_metadata_str, BuiltPackage, CEDRA_PACKAGES,
};
...
const CEDRA_PACKAGES_DIR_NAMES: [&str; 5] = [
    "cedra-framework",
    "move-stdlib",
    "cedra-stdlib",
    "cedra-token",
...
pub const CEDRA_COMMONS: &str = "cedra-commons";
...
fn is_cedra_package(package_name: &str) -> bool {
    CEDRA_PACKAGES.contains(&package_name)
}

fn get_cedra_dir(package_name: &str) -> Option<&str> {
    if is_cedra_package(package_name) {
        for i in 0..CEDRA_PACKAGES.len() {
            if CEDRA_PACKAGES[i] == package_name {
                return Some(CEDRA_PACKAGES_DIR_NAMES[i]);
            }
        }
    }
    None
...
async fn download_cedra_packages(path: &Path) -> anyhow::Result<()> {
    let git_url = "https://github.com/cedra-labs/cedra";
    let tmp_dir = TempDir::new()?;
    Command::new("git")
        .args(["clone", git_url, tmp_dir.path().to_str().unwrap()])
        .output()
        .map_err(|_| anyhow::anyhow!("Failed to clone Git repository"))?;
    let source_framework_path = PathBuf::from(tmp_dir.path()).join("cedra-move/framework");
    for package_name in CEDRA_PACKAGES {
        let source_framework_path =
            source_framework_path.join(get_cedra_dir(package_name).unwrap());
        let target_framework_path = PathBuf::from(path).join(get_cedra_dir(package_name).unwrap());
        Command::new("cp")
            .arg("-r")
...
fn check_cedra_packages_availability(path: PathBuf) -> bool {
    if !path.exists() {
        return false;
    }
    for package in CEDRA_PACKAGES {
        if !path.join(get_cedra_dir(package).unwrap()).exists() {
            return false;
        }
    }
    true
...
pub async fn prepare_cedra_packages(path: PathBuf) {
    ...
    if success {
        ...
        download_cedra_packages(&path).await.unwrap();
    }
}
...
fn compile_cedra_packages(
    cedra_commons_path: &Path,
    compiled_package_map: &mut HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
    v2_flag: bool,
) -> anyhow::Result<()> {
    for package in CEDRA_PACKAGES {
        let root_package_dir = cedra_commons_path.join(get_cedra_dir(package).unwrap());
        let compiler_version = if v2_flag {
            Some(CompilerVersion::latest_stable())
        } else {
            Some(CompilerVersion::V1)
        };
        // For simplicity, all packages including cedra token are stored under 0x1 in the map
...
fn compile_package(
    ...
    let mut build_options = cedra_framework::BuildOptions {
    ...
    for manifest_dep in manifest_deps {
        ...
        for pack_dep in &root_package_metadata.deps {
            ...
            if pack_dep_name == manifest_dep_name {
                if is_cedra_package(&pack_dep_name) {
                    fix_manifest_dep(
                        dep,
                        &format!(
                            "{}/{}",
                            CEDRA_COMMONS,
                            get_cedra_dir(&pack_dep_name).unwrap()
                        ),
                    );
                    break;
                }
