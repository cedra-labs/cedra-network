...
use crate::{CedraValidatorInterface, FilterCondition};
use anyhow::{anyhow, Result};
use cedra_api_types::{CedraError, CedraErrorCode};
use cedra_framework::{
    natives::code::{PackageMetadata, PackageRegistry},
    CEDRA_PACKAGES,
};
use cedra_rest_client::{
    error::{CedraErrorResponse, RestError},
    Client,
};
use cedra_types::{
...
async fn check_and_obtain_source_code(
    ...
    if let Some(target_package) = target_package_opt {
        ...
        if CEDRA_PACKAGES.contains(&target_package.name.as_str()) {
            package_cache.insert(
                m.clone(),
                (
                    AccountAddress::ONE,
                    target_package.name.clone(), // all cedra packages are stored under 0x1
                    HashMap::new(),
                ),
            );
            txns.push((
                ...
                Some((
                    AccountAddress::ONE,
                    target_package.name, // all cedra packages are stored under 0x1
                    HashMap::new(),
                )), // do not need to store the package registry for cedra packages
            ));
...
impl CedraValidatorInterface for RestDebuggerInterface {
    async fn get_state_value_by_version(
        ...
        match self.0.get_raw_state_value(state_key, version).await {
            ...
            Err(err) => match err {
                RestError::Api(CedraErrorResponse {
                    error:
                        CedraError {
                            error_code:
                                CedraErrorCode::StateValueNotFound | CedraErrorCode::TableItemNotFound, /* bug in pre 1.9 nodes */
                            ..
                        },
                    ..
                }) => Ok(None),
