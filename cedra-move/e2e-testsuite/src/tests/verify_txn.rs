...
use cedra_types::{
    ...
    transaction::{
        ExecutionStatus, FaAddress, Script, TransactionArgument, TransactionStatus,
    },
...
fn verify_signature() {
    ...
    let program = cedra_stdlib::cedra_coin_transfer(*sender.address(), 100);
...
fn good_module_uses_bad(
    ...
    let framework_modules = cedra_cached_packages::head_release_bundle().compiled_modules();
