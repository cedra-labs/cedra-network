...
//! Test infrastructure for modeling Cedra accounts.
//! Ported from the old `e2e-tests` crate.

use cedra_crypto::ed25519::*;
 ...
 *
 **************************************************************************************************/
/// Details about a Cedra account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Cedra account.
///
...
impl Account {
    ...
    /// not automatically get added to the Cedra store. To add an account to the store, use
    ...
    /// Creates a new account representing the cedra root account in memory.
    ///
    /// The address will be [`cedra_test_root_address`][account_config::cedra_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][static@GENESIS_KEYPAIR] as its keypair.
    pub fn new_cedra_root() -> Self {
        Self::new_genesis_account(account_config::cedra_test_root_address())
    }
...
impl CoinStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let coin_store = CoinStoreResource::<CedraCoinType>::new(
            self.coin,
            self.frozen,
...
impl AccountData {
    ...
    pub fn to_writeset(&self) -> WriteSet {
        ...
        if let Some(coin_store) = &self.coin_store {
            write_set.push((
                StateKey::resource_typed::<CoinStoreResource<CedraCoinType>>(self.address())
                    .unwrap(),
