...
//! This library runs and manages a set of services that makes up a local Cedra network.
//! - node
//!     - node API
//!     - indexer grpc
//! - faucet
...
use cedra_localnet::docker::get_docker;
use clap::Parser;
...
async fn run_all_services(timeout: u64) -> Result<()> {
    ...
    // Docker Network
    let docker_network_name = "cedra-workspace".to_string();
