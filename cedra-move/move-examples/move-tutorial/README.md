---
title: "Cedra Move Tutorial"
slug: "move-tutorial"
---

# Move Tutorial

Welcome to the Cedra Move Tutorial! This is the original Move language and tools tutorial, independent of a particular network, adapted to use Cedra tooling. Here you will learn basic usage of the Move language and tools for compiling, testing, and verifying Move.

This tutorial does not teach you how to use the existing [Cedra Move frameworks](https://cedra.dev/reference/move) or how to run code on the Cedra network. See the [Cedra developer documentation](https://cedra.dev/tutorials/) for those instructions.
...
You also need a recent version of the [Cedra CLI](https://cedra.dev/tools/cedra-cli/install-cli/). This tutorial is written using the following version:
...
If you want to use an IDE for working through the examples, we recommend CLion/IntelliJ, which has good support for Cedra Move.
...
[module](https://cedra.dev/move/book/modules-and-scripts/). Modules are the
...
[struct](https://cedra.dev/move/book/structs-and-resources)
...
* It takes a [`&signer`](https://cedra.dev/move/book/signer) reference ('`&`') -- an
...
Let us make sure it builds! This can be done with the `cedra move compile` command from within the package directory ([`step_1/basic_coin`](./step_1/basic_coin/)):
...
* You can create an empty Move package by calling:
    ```bash
    cedra move init --name <pkg_name>
    ```
* Move code can also live in a number of other places. See the [Move
  book](https://cedra.dev/move/book/packages) for more information on the
  Move package system.
* More information on the `Move.toml` file can also be found in the [Package](https://cedra.dev/move/book/packages#movetoml) section of the Move book.
* Move also supports the idea of [named
  addresses](https://cedra.dev/move/book/address#named-addresses); Named
  ...
  that you can deploy, depending on what address(es) you control. They are used quite frequently, and can be defined in the `Move.toml` file in the `[addresses]` section, like so:
    ...
    ```
* [Structures](https://cedra.dev/move/book/structs-and-resources) in Move
  can be given different
  [abilities](https://cedra.dev/move/book/abilities) that describe what
...
* [Functions](https://cedra.dev/move/book/functions) are default
    private, and can also be `public`,
    [`public(friend)`](https://cedra.dev/move/book/friends).
    A function marked as `entry` can be called as a transaction.
* `move_to` is one of the [five different global storage
  operators](https://cedra.dev/move/book/global-storage-operators).
</details>
...
You can run the tests with the `cedra move test` command:
...
* There are a number of [test-related annotations](https://cedra.dev/move/book/unit-testing#testing-annotations-their-meaning-and-usage) that are worth exploring.
  You'll see some of these used in Step 5.

#### Exercises
* Change the assertion to `11` so that the test fails. Find a flag that you can
  pass to the `cedra move test` command that will dump the global state when
...
* Find a flag that allows you to gather test coverage information, and
  then experiment with using the `cedra move coverage` command to examine
...
> NOTE: The coin and balance interfaces are for illustration of Move concepts only. Cedra uses a different, richer [coin type](https://cedra.dev/reference/move?branch=mainnet&page=cedra-framework/doc/coin.md) contained in the Cedra framework.
...
```
Read more on Move function [visibilities](https://cedra.dev/move/book/functions#visibility).
</details>
<details>
...
if an [abort](https://cedra.dev/move/book/abort-and-assert) is raised no unwinding of state
needs to be performed, as no changes from that transaction will be persisted to the blockchain.

[`error` module]: https://github.com/cedra-labs/cedra-network/blob/main/cedra-move/framework/move-stdlib/sources/error.move
...
to get a mutable reference to the global storage, and `&mut` is used to create a mutable [reference](https://cedra.dev/move/book/references) to a field of a
...
To get started, run the `cedra move test` command in the [`step_5/basic_coin`](./step_5/basic_coin) directory:
...
In Move, we can use
[generics](https://cedra.dev/move/book/generics)
...
Read more about [phantom type](https://cedra.dev/move/book/generics#phantom-type-parameters) parameters in the Cedra Move Book.
</details>

## Step 7:  Use the Move Prover
    
> NOTE: Before running the Move Prover, ensure that the [Move Prover](https://cedra.dev/tools/cedra-cli/install-cli/install-move-prover) and associated tools are installed.
...
[Move Prover](https://cedra.dev/move/prover/move-prover)
