 ...
 * 2. Sponsors add locked Cedras for custom expiration time + amount for recipients. Each lockup is called a "lock".
...
module defi::locked_coins {
    ...
    #[test_only]
    use cedra_framework::coin::BurnCapability;
    #[test_only]
    ...
    #[test_only]
    fun setup(cedra_framework: &signer, sponsor: &signer): BurnCapability<CedraCoin> {
    ...
    #[test(cedra_framework = @0x1, sponsor = @0x123, recipient = @0x234)]
    public entry fun test_recipient_can_claim_coins(
        cedra_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks {
        let burn_cap = setup(cedra_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        cedra_account::create_account(recipient_addr);
        let sponsor_address = signer::address_of(sponsor);
    ...
    #[test(cedra_framework = @0x1, sponsor = @0x123, recipient_1 = @0x234, recipient_2 = @0x345)]
    public entry fun test_sponsor_can_batch_update_lockup(
        cedra_framework: &signer, sponsor: &signer, recipient_1: &signer, recipient_2: &signer) acquires Locks {
        let burn_cap = setup(cedra_framework, sponsor);
        ...
        cedra_account::create_account(recipient_1_addr);
        cedra_account::create_account(recipient_2_addr);
        let sponsor_address = signer::address_of(sponsor);
        initialize_sponsor<CedraCoin>(sponsor, sponsor_address);
    ...
    #[test(cedra_framework = @0x1, sponsor = @0x123, recipient = @0x234, withdrawal = @0x345)]
    public entry fun test_sponsor_can_cancel_lockup(
        cedra_framework: &signer, sponsor: &signer, recipient: &signer, withdrawal: &signer) acquires Locks {
        let burn_cap = setup(cedra_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        let withdrawal_addr = signer::address_of(withdrawal);
        cedra_account::create_account(withdrawal_addr);
        cedra_account::create_account(recipient_addr);
        let sponsor_address = signer::address_of(sponsor);
    ...
    #[test(cedra_framework = @0x1, sponsor = @0x123, recipient_1 = @0x234, recipient_2 = @0x345, withdrawal = @0x456)]
    ...
    #[test(cedra_framework = @0x1, sponsor = @0x123, recipient = @0x234, withdrawal = @0x456)]
