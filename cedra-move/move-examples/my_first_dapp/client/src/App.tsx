...
import { useWallet, InputTransactionData } from "@cedra-labs/wallet-adapter-react";
...
function App() {
  ...
  const fetchList = async () => {
    if (!account) return [];
    try {
      const todoListResource = await cedra.getAccountResource(
      ...
      while (counter <= taskCounter) {
        ...
        const task = await cedra.getTableItem<Task>({handle:tableHandle, data:tableItem});
        tasks.push(task);
        counter++;
      }
                  ...
                  renderItem={(task: Task) => (
                    ...
                    >
                      <List.Item.Meta
                        ...
                        description={
                          <a
                            href={`https://cedrascan.com/account/${task.address}/`}
                            target="_blank"
