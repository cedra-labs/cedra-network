module raffle::raffle_test {
    ...
    #[test_only]
    use cedra_framework::coin::MintCapability;
    ...
    #[test_only]
    use cedra_std::crypto_algebra::enable_cryptography_algebra_natives;
    #[test_only]
    use cedra_framework::randomness;

    #[test_only]
    fun give_coins(mint_cap: &MintCapability<CedraCoin>, to: &signer) {
        let to_addr = signer::address_of(to);
        if (!account::exists_at(to_addr)) {
            account::create_account_for_test(to_addr);
        };
        coin::register<CedraCoin>(to);
    ...
    fun test_raffle(
        ...
        let (burn_cap, mint_cap) = cedra_coin::initialize_for_test(&fx);

        // Create fake coins for users participating in raffle & initialize cedra_framework
        ...
        while (i < num_players) {
            ...
            if (player == winner) {
                assert!(coin::balance<CedraCoin>(player) == raffle::get_ticket_price() * num_players, 1);
            } else {
                assert!(coin::balance<CedraCoin>(player) == 0, 1);
            };
