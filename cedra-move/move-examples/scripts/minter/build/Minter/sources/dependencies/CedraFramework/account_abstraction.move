module cedra_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@cedra_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    entry fun initialize(cedra_framework: &signer) {
        system_addresses::assert_cedra_framework(cedra_framework);
        move_to(
            cedra_framework,
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @cedra_fungible_asset)
    }
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@cedra_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@cedra_framework].auth_functions
   }
    ...
    fun authenticate(
        ...
        // Returned signer MUST represent the same account address. Otherwise, it may break the invariant of Cedra blockchain!
        assert!(
