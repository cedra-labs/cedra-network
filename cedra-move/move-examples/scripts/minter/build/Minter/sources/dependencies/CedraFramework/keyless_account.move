...
module cedra_framework::keyless_account {
    ...
    use std::vector;
    use cedra_std::crypto_algebra;
    use cedra_std::ed25519;
    use cedra_framework::chain_status;
    use cedra_framework::system_addresses;

    // The `cedra_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = cedra_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@cedra_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@cedra_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@cedra_framework)) {
                *borrow_global_mut<Configuration>(@cedra_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
