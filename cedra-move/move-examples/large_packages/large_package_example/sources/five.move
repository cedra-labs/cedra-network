...
///
/// # Integrate with the Cedra Blockchain
///
/// If you provide blockchain services to your customers and wish to add the Cedra blockchain to your platform, then this guide is for you. This system integrators guide will walk you through all you need to integrate the Cedra blockchain into your platform.
///
/// ## Overview
///
/// This document will guide you through the following tasks to integrate with Cedra:
...
///
/// See [Cedra Blockchain Networks](../nodes/networks.md) for full details on each environment.
///
/// ### Run a localnet
///
...
///       <li>Indexer API: <a href="https://api.devnet.cedralabs.com/v1/graphql">https://api.devnet.cedralabs.com/v1/graphql</a></li>
///       <li>Faucet API: <a href="https://faucet-api.cedra.dev">https://faucet-api.cedra.dev</a></li>
///       <li><a href="https://cloud.hasura.io/public/graphiql?endpoint=https://api.devnet.cedralabs.com/v1/graphql">Indexer GraphQL</a></li>
///     </ul>
///   </TabItem>
...
///       <li>Indexer API: <a href="https://api.mainnet.cedralabs.com/v1/graphql">https://api.mainnet.cedralabs.com/v1/graphql</a></li>
///       <li>Faucet: N/A</li>
///       <li><a href="https://cloud.hasura.io/public/graphiql?endpoint=https://api.mainnet.cedralabs.com/v1/graphql">Indexer GraphQL</a></li>
///     </ul>
///   </TabItem>
/// </Tabs>
///
/// ### SDKs and tools
///
/// Cedra currently provides three SDKs:
/// 1. [Typescript](../sdks/ts-sdk/index.md)
/// 2. [Python](../sdks/python-sdk.md)
/// 3. [Rust](../sdks/rust-sdk.md)
///
/// Almost all developers will benefit from exploring the CLI. [Using the CLI](../tools/cedra-cli-tool/use-cedra-cli.md) demonstrates how the CLI can be used to which includes creating accounts, transferring coins, and publishing modules.
///
/// ## Accounts on Cedra
///
///
...
///
/// Read more about [Accounts](../concepts/accounts.md) and [set one up](../tools/cedra-cli-tool/use-cedra-cli#initialize-local-configuration-and-create-an-account).
///
/// ## Transactions
///
///
...
///
/// Cedra supports two methods for constructing transactions:
///
/// - Using the Cedra client libraries to generate native BCS transactions.
...
/// - A script (payload)
///
/// Both [Python](https://github.com/cedra-labs/cedra-network/blob/3973311dac6bb9348bfc81cf983c2a1be11f1b48/ecosystem/python/sdk/cedra_sdk/client.py#L256) and [TypeScript](https://github.com/cedra-labs/cedra-network/blob/3973311dac6bb9348bfc81cf983c2a1be11f1b48/ecosystem/typescript/sdk/src/cedra_client.test.ts#L93) support the generation of transactions that target entry points. This guide points out many of those entry points, such as `cedra_account::transfer` and `cedra_account::create_account`.
///
/// Most basic operations on the Cedra blockchain should be available via entry point calls. While one could submit multiple transactions calling entry points in series, such operations benefit from being called atomically from a single transaction. A script payload transaction can call any public (entry) function defined within any module. Here's an example [Move script](https://github.com/cedra-labs/cedra-network/tree/main/cedra-move/move-examples/scripts/two_by_two_transfer) that uses a MultiAgent transaction to extract funds from two accounts and deposit them into two other accounts. This is a [Python example](https://github.com/cedra-labs/cedra-network/blob/main/ecosystem/python/sdk/examples/transfer-two-by-two.py) that uses the bytecode generated by compiling that script. Currently there is limited support for script payloads in TypeScript.
///
...
///
/// To facilitate evaluation of transactions as well as gas estimation, Cedra supports a simulation API that does not require and should not contain valid signatures on transactions.
///
///
/// Both the [Typescript SDK](https://github.com/cedra-labs/cedra-network/blob/9b85d41ed8ef4a61a9cd64f9de511654fcc02024/ecosystem/typescript/sdk/src/cedra_client.ts#L413) and [Python SDK](https://github.com/cedra-labs/cedra-network/blob/main/ecosystem/python/sdk/examples/simulate-transfer-coin.py) support the simulation API. Note the output and gas used may change based upon the state of the account. For gas estimations, we recommend that the maximum gas amount be larger than the amount quoted by this API.
///
/// ## Viewing current and historical state
///
/// Most integrations into the Cedra blockchain benefit from a holistic and comprehensive overview of the current and historical state of the blockchain. Cedra provides historical transactions, state, and events, all the result of transaction execution.
///
...
///
/// Events and transactions pruning can be disabled via setting the [`enable_ledger_pruner`](https://github.com/cedra-labs/cedra-network/blob/cf0bc2e4031a843cdc0c04e70b3f7cd92666afcf/config/src/config/storage_config.rs#L141) to `false`. This is default behavior in Mainnet. In the near future, Cedra will provide indexers that mitigate the need to directly query from a node.
///
...
///
/// Cedra has a standard [Coin type](https://github.com/cedra-labs/cedra-network/blob/main/cedra-move/framework/cedra-framework/sources/coin.move). Different types of coins can be represented in this type through the use of distinct structs that represent the type parameter or generic for `Coin<T>`.
///
/// Coins are stored within an account under the resource `CoinStore<T>`. At account creation, each user has the resource `CoinStore<0x1::cedra_coin::CedraCoin>` or `CoinStore<CedraCoin>`, for short. Within this resource is the Cedra coin: `Coin<CedraCoin>`.
///
/// ### Transferring coins between users
///
/// Coins can be transferred between users via the [`coin::transfer`](https://github.com/cedra-labs/cedra-network/blob/36a7c00b29a457469264187d8e44070b2d5391fe/cedra-move/framework/cedra-framework/sources/coin.move#L307) function for all coins and [`cedra_account::transfer`](https://github.com/cedra-labs/cedra-network/blob/88c9aab3982c246f8aa75eb2caf8c8ab1dcab491/cedra-move/framework/cedra-framework/sources/cedra_account.move#L18) for Cedra coins. The advantage of the latter function is that it creates the destination account if it does not exist.
///
/// :::caution
...
///
/// The current balance for a `Coin<T>` where `T` is the Cedra coin is available at the account resources URL: `https://{rest_api_server}/accounts/{address}/resource/0x1::coin::CoinStore<0x1::cedra_coin::CedraCoin>`. The balance is stored within `coin::amount`. The resource also contains the total number of deposit and withdraw events, and the `counter` value within `deposit_events` and `withdraw_events`, respectively.
///
/// ```
/// {
///   "type": "0x1::coin::CoinStore<0x1::cedra_coin::CedraCoin>",
///   "data": {
///     "coin": {
///       "value": "3927"
///     },
...
///
/// In Cedra, each transaction is committed as a distinct version to the blockchain. This allows for the convenience of sharing committed transactions by their version number; to do so, query: `https://{rest_server_api}/transactions/by_version/{version}`
///
...
/// :::tip
///
/// When tracking full movement of coins, normally events are sufficient. `0x1::cedra_coin::CedraCoin`, however, requires considering `gas_used` for each transaction sent from the given account since it represents gas in Cedra. To reduce unnecessary overhead, extracting gas fees due to transactions does not emit an event. All transactions for an account can be retrieved from this API: `https://{rest_server_api}/accounts/{address}/transactions`
///
/// :::
///
...
/// 3. A gas fee `2200` of `0x1::cedra_coin::CedraCoin` from the sending account `0x810026ca8291dd88b5b30a1d3ca2edd683d33d06c4a7f7c451d96f6d47bc5e8b`
///
...
/// 2. Gas is always: `0x1::cedra_coin::CedraCoin`
///
...
///
/// This tutorial is for SDK and wallet developers who want to integrate with the [Cedra Faucet](https://github.com/cedra-labs/cedra-network/tree/main/crates/cedra-faucet). If you are a dapp developer, you should access the faucet through an existing [SDK](../tutorials/first-transaction.md) or [CLI](../tools/cedra-cli-tool/use-cedra-cli#initialize-local-configuration-and-create-an-account) instead.
///
...
/// If you are building a client in JavaScript or TypeScript, you should make use of the [@cedra-labs/cedra-faucet-client](https://www.npmjs.com/package/@cedra-labs/cedra-faucet-client) package. This client is generated based on the OpenAPI spec published by the faucet service.
///
/// Example use:
/// ```typescript
/// import {
///   CedraFaucetClient,
///   FundRequest,
/// } from "@cedra-labs/cedra-faucet-client";
///
/// async function callFaucet(amount: number, address: string): Promise<string[]> {
///   const faucetClient = new CedraFaucetClient({BASE: "https://faucet-api.cedra.dev"});
...
/// 1. Generate a client from the [OpenAPI spec](https://github.com/cedra-labs/cedra-network/blob/main/crates/cedra-faucet/doc/spec.yaml).
