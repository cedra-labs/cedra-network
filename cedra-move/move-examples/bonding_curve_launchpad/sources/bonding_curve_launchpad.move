module bonding_curve_launchpad::bonding_curve_launchpad {
    ...
    use cedra_framework::fungible_asset::{Self, FungibleAsset, Metadata, TransferRef};
    ...
    /// Each created FA has transfers disabled, until the Cedra reserves minimum threshold on
    ...
    /// Optionally, the participant can immediately perform an initial swap from Cedra to FA.
    public entry fun create_fa_pair(
        ...
        // block the transfer of Cedra to the creator.
        let transfer_ref = &borrow_global<FAController>(fa_address).transfer_ref;
        // Create the liquidity pair between Cedra and the new FA. Include the initial creator swap, if needed.
    ...
    /// Swap from FA to Cedra, or vice versa, through `liquidity_pair`.
