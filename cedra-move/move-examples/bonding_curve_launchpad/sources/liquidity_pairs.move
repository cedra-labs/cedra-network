module bonding_curve_launchpad::liquidity_pairs {
    ...
    use cedra_framework::fungible_asset::{Metadata, TransferRef, FungibleAsset, FungibleStore};
    ...
    /// Liquidity pair (Cedra/FA) being created already exists.
    ...
    //---------------------------Liquidity Pair---------------------------
    /// Creates a unique liquidity pair between a given FA and Cedra.
    ...
    public(friend) fun register_liquidity_pair(
        ...
        // Only allow for creation of new Cedra-FA pairs.
        let does_already_exist = object::is_object(get_pair_obj_address(name, symbol));
        ...
        // generator signers from, for when Cedra or the FA needs to be transferred to and from the liquidity pair.
        ...
        // Initial Cedra reserves are virtual liquidity, for less extreme initial swaps (avoiding early adopter's
    ...
    public(friend) fun swap_fa_to_apt(
        ...
        // Determine the amount received of Cedra, when given swapper-supplied amount_in of FA.
        ...
        // Perform the swap.
        // Swapper sends FA to the liquidity pair object. The liquidity pair object sends Cedra to the swapper, in return.
    ...
    public(friend) fun swap_apt_to_fa(
        ...
        let liquidity_pair = borrow_global_mut<LiquidityPair>(get_pair_obj_address(name, symbol));
        assert!(liquidity_pair.is_enabled, ELIQUIDITY_PAIR_DISABLED);
        // Determine the amount received of FA, when given swapper-supplied amount_in of Cedra.
        ...
        // Perform the swap.
        // Swapper sends Cedra to the liquidity pair object. The liquidity pair object sends FA to the swapper, in return.
        ...
        cedra_account::transfer(swapper_account, liquidity_pair_address, apt_given);
    ...
    fun graduate(
        ...
        router::create_pool_coin<CedraCoin>(fa_object_metadata, false);
        let liquidity_pair_signer = object::generate_signer_for_extending(&liquidity_pair.extend_ref);
        add_liquidity_coin_entry_transfer_ref<CedraCoin>(
            transfer_ref,
            &liquidity_pair_signer,
        ...
        let apt_coin_wrapped = coin_wrapper::get_wrapper<CedraCoin>();
    ...
    fun add_liquidity_coin_entry_transfer_ref<CoinType>(
        ...
        // Wrap Cedra into a FA. Then, determine the optimal amounts for providing liquidity to the given FA - Cedra pair.
        ...
        // Retrieve the Cedra and FA from the liquidity provider.
        ...
        // Place the Cedra and FA into the liquidity pair.
