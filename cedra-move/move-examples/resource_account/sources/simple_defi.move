...
/// (2) when exchanging coins, we call `exchange_to` to swap `CedraCoin` to `ChloesCoin`, and `exchange_from` to swap `CedraCoin` from `ChloesCoin`
module resource_account::simple_defi {
    use std::signer;
    use std::string;

    use cedra_framework::account;
    use cedra_framework::coin::{Self, Coin, MintCapability, BurnCapability};
    ...
    /// Exchange CedraCoin to ChloesCoin
    public fun exchange_to(a_coin: Coin<CedraCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    /// Exchange ChloesCoin to CedraCoin
    public fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<CedraCoin> acquires ModuleData {
        ...
        coin::withdraw<CedraCoin>(&resource_signer, amount)
    }
    ...
    public entry fun exchange_to_entry(account: &signer, amount: u64) acquires ModuleData {
        let a_coin = coin::withdraw<CedraCoin>(account, amount);
    ...
    #[test(origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, framework = @cedra_framework)]
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let (cedra_coin_burn_cap, cedra_coin_mint_cap) = cedra_coin::initialize_for_test(&framework);
        set_up_test(&origin_account, &resource_account);

        // exchange from 5 cedra coins to 5 chloe's coins & assert the results are expected
        let five_a_coins = coin::mint(5, &cedra_coin_mint_cap);
        let c_coins = exchange_to(five_a_coins);
        assert!(coin::value(&c_coins) == 5, 0);
        assert!(coin::balance<CedraCoin>(signer::address_of(&resource_account)) == 5, 1);
        assert!(coin::balance<ChloesCoin>(signer::address_of(&resource_account)) == 0, 2);

        // exchange from 5 chloe's coins to 5 cedra coins & assert the results are expected
        let a_coins = exchange_from(c_coins);
        assert!(coin::value(&a_coins) == 5, 0);
        assert!(coin::balance<CedraCoin>(signer::address_of(&resource_account)) == 0, 3);
        ...
        coin::burn(a_coins, &cedra_coin_burn_cap);
        coin::destroy_mint_cap<CedraCoin>(cedra_coin_mint_cap);
        coin::destroy_burn_cap<CedraCoin>(cedra_coin_burn_cap);
    }
}
