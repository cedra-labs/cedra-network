...
module dao_platform::nft_dao {
    use cedra_framework::account::{SignerCapability, create_signer_with_capability};
    ...
    use cedra_std::table;
    use cedra_token::property_map::PropertyMap;
    use cedra_token::property_map;
    use cedra_token::token::{Self, TokenId, create_token_id_raw};
    ...
    #[test_only]
    use cedra_token::token::create_token_script;
    #[test_only]
    use cedra_framework::cedra_account::transfer_coins;
    #[test_only]
    use cedra_framework::cedra_coin;
    use cedra_token::token_transfers;
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_e2e_scenario(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
    ...
    #[test(cedra_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(cedra_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(cedra_framework);
    ...
    #[test(cedra_framework = @0x1, admin = @0xdeaf)]
    public fun test_transferring_ownership_to_zero_address(cedra_framework: &signer, admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(cedra_framework);
        let old_addr = signer::address_of(admin);
        account::create_account_for_test(@0x1);
    ...
    public fun test_double_vote(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    public fun test_resolve_earlier_than_ending_time(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(cedra_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(cedra_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(cedra_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(cedra_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(cedra_framework);
        let new_addr = signer::address_of(new_admin);
        let old_addr = signer::address_of(admin);
    ...
    #[test(cedra_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(cedra_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(cedra_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
