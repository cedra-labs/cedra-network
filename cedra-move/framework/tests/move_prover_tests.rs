...
use cedra_framework::{extended_checks, prover::ProverOptions};
...
pub fn run_prover_for_pkg(
    ...
    if no_tools {
        ...
        See https://github.com/cedra-labs/cedra-network/blob/main/cedra-move/framework/FRAMEWORK-PROVER-GUIDE.md \
        for instructions, or \
...
fn move_framework_prover_tests_shard1() {
    run_prover_for_pkg("cedra-framework", 5, Some(1));
}

#[test]
fn move_framework_prover_tests_shard2() {
    run_prover_for_pkg("cedra-framework", 5, Some(2));
}

#[test]
fn move_framework_prover_tests_shard3() {
    run_prover_for_pkg("cedra-framework", 5, Some(3));
}

#[test]
fn move_framework_prover_tests_shard4() {
    run_prover_for_pkg("cedra-framework", 5, Some(4));
}

#[test]
fn move_framework_prover_tests_shard5() {
    run_prover_for_pkg("cedra-framework", 5, Some(5));
}

#[test]
fn move_token_prover_tests() {
    run_prover_for_pkg("cedra-token", 1, None);
}

#[test]
fn move_cedra_stdlib_prover_tests() {
    run_prover_for_pkg("cedra-stdlib", 1, None);
}
