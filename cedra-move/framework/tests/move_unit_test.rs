...
use cedra_framework::{extended_checks, path_in_crate, BuildOptions};
use cedra_gas_schedule::{MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION};
use cedra_types::on_chain_config::{
    cedra_test_feature_flags_genesis, Features, TimedFeaturesBuilder,
};
...
/// TODO: per @vgao1996:
/// - There should be only one ground truth of `cedra_test_natives`.
...
/// More discussion: https://github.com/cedra-labs/cedra-network/pull/15997#discussion_r1994469668
pub fn cedra_test_natives() -> NativeFunctionTable {
...
fn move_framework_unit_tests() {
    run_tests_for_pkg("cedra-framework", false);
}

#[test]
fn move_cedra_stdlib_unit_tests() {
    run_tests_for_pkg("cedra-stdlib", false);
}
...
fn move_token_unit_tests() {
    run_tests_for_pkg("cedra-token", false);
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("cedra-token-objects", false);
}

#[test]
fn move_experimental_unit_tests() {
    run_tests_for_pkg("cedra-experimental", true);
}
