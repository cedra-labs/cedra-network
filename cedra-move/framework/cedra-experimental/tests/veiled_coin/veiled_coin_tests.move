...
module cedra_experimental::veiled_coin_tests {
    #[test_only]
    use std::features;
    #[test_only]
    use std::signer;
    ...
    #[test_only]
    use cedra_std::ristretto255_bulletproofs as bulletproofs;
    #[test_only]
    use cedra_std::debug::print;
    #[test_only]
    use cedra_std::ristretto255_elgamal as elgamal;
    #[test_only]
    use cedra_std::ristretto255;
    #[test_only]
    use cedra_std::ristretto255_pedersen as pedersen;
    ...
    #[test_only]
    use cedra_experimental::veiled_coin;
    #[test_only]
    use cedra_experimental::helpers::generate_elgamal_keypair;
    #[test_only]
    use cedra_experimental::sigma_protos::{serialize_withdrawal_subproof, prove_withdrawal};
    #[test_only]
    use cedra_experimental::sigma_protos;
    ...
    fun set_up_for_veiled_coin_test(
        ...
        assert!(signer::address_of(&cedra_fx) != signer::address_of(sender), 1);
        assert!(signer::address_of(&cedra_fx) != signer::address_of(recipient), 2);
        ...
        features::change_feature_flags_for_testing(&cedra_fx, vector[features::get_bulletproofs_feature()], vector[]);
        println(b"Enabled feature flags.");

        // Set up an account for the framework address
        account::create_account_for_test(signer::address_of(&cedra_fx)); // needed in `coin::create_fake_money`
        ...
        // Create `amount` of `FakeCoin` coins at the Cedra 0x1 address (must do) and register a `FakeCoin` coin
        // store for the `sender`.
        coin::create_fake_money(
            &cedra_fx,
            sender,
            veiled_coin::cast_u32_to_u64_amount(sender_amount + recipient_amount));
        println(b"Created fake money inside @cedra_framework");
    ...
    #[test(veiled_coin = @cedra_experimental, cedra_fx = @cedra_framework, sender = @0xc0ffee, recipient = @0x1337)]
    fun veil_test(
        veiled_coin: signer,
        cedra_fx: signer,
        sender: signer,
        ...
        print(&@cedra_experimental);
        println(b"@cedra_framework:");
        print(&@cedra_framework);

        // Split 500 and 500 between `sender` and `recipient`
        set_up_for_veiled_coin_test(
            &veiled_coin, cedra_fx, &sender, &recipient, 500u32, 500u32);
    ...
    #[test(veiled_coin = @cedra_experimental, cedra_fx = @cedra_framework, sender = @0x1337)]
    fun unveil_test(
        veiled_coin: signer,
        cedra_fx: signer,
        sender: signer,
        ...
        set_up_for_veiled_coin_test(
            &veiled_coin, cedra_fx, &sender, &sender, 500, 0);
    ...
    fun basic_viability_test(
        ...
        set_up_for_veiled_coin_test(&veiled_coin, cedra_fx, &sender, &recipient, 500, 500);
