#[test_only]
module cedra_experimental::confidential_asset_tests {
    use std::features;
    use std::option;
    use std::signer;
    use std::string::utf8;
    ...
    use cedra_experimental::ristretto255_twisted_elgamal::{Self as twisted_elgamal, generate_twisted_elgamal_keypair};
    ...
    {
        chain_id::initialize_for_test(cedra_fx, 4);
    ...
    fun success_deposit_test(
        ...
        bob: signer)
    {
        let token = set_up_for_confidential_asset_test(&confidential_asset, &cedra_fx, &fa, &alice, &bob, 500, 500);
    ...
    fun fail_register_if_token_disallowed(
        ...
        alice: signer)
    {
        let token = set_up_for_confidential_asset_test(&confidential_asset, &cedra_fx, &fa, &alice, &alice, 500, 500);

        confidential_asset::enable_allow_list(&cedra_fx);
        ...
        alice: signer)
    {
        ...
        confidential_asset::enable_allow_list(&cedra_fx);
        confidential_asset::enable_token(&cedra_fx, token);
        ...
        alice: signer)
    {
        chain_id::initialize_for_test(&cedra_fx, 4);
        confidential_asset::init_module_for_testing(&confidential_asset);
        coin::create_coin_conversion_map(&cedra_fx);
        ...
        coin::create_pairing<MockCoin>(&cedra_fx);
