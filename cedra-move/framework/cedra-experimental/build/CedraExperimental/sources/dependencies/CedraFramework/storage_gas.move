...
module cedra_framework::storage_gas {
    ...
    public(friend) fun set_config(cedra_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_cedra_framework(cedra_framework);
        *borrow_global_mut<StorageGasConfig>(@cedra_framework) = config;
    }
    ...
    public fun initialize(cedra_framework: &signer) {
        system_addresses::assert_cedra_framework(cedra_framework);
        assert!(
            !exists<StorageGasConfig>(@cedra_framework),
        ...
        move_to(cedra_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@cedra_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(cedra_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@cedra_framework);
        let gas = borrow_global_mut<StorageGas>(@cedra_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@cedra_framework);
