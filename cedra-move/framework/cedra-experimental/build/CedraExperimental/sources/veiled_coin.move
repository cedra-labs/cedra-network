...
///
/// This module allows users to "register" a veiled account for any pre-existing `cedra_framework::Coin` type `T` via
...
/// Cedra framework. This is because coin amounts must be encrypted with an *efficient*, additively-homomorphic encryption
...
/// ```
///
/// Recall that: A coin has a *decimal precision* $d$ (e.g., for `CedraCoin`, $d = 8$; see `initialize` in
/// `cedra_coin.move`). This precision $d$ is used when displaying a `u64` amount, by dividing the amount by $10^d$.
...
module cedra_experimental::veiled_coin {
    use std::error;
    ...
    const VEILED_COIN_BULLETPROOFS_DST: vector<u8> = b"CedraVeiledCoin/BulletproofRangeProof";
    ...
    /// represent normal `cedra_framework::coin::Coin` values.
    ...
    fun get_resource_account_signer(): signer acquires VeiledCoinMinter {
        account::create_signer_with_capability(&borrow_global<VeiledCoinMinter>(@cedra_experimental).signer_cap)
    }
