...
module cedra_experimental::confidential_asset {
    use std::bcs;
    use std::error;
    use std::option::Option;
    use std::signer;
    use std::vector;
    use cedra_std::ristretto255::Self;
    use cedra_std::ristretto255_bulletproofs::Self as bulletproofs;
    ...
    use cedra_experimental::confidential_balance;
    use cedra_experimental::confidential_proof::{
    ...
    public fun enable_allow_list(cedra_framework: &signer) acquires FAController {
        system_addresses::assert_cedra_framework(cedra_framework);

        let fa_controller = borrow_global_mut<FAController>(@cedra_experimental);
    ...
    public fun disable_allow_list(cedra_framework: &signer) acquires FAController {
        system_addresses::assert_cedra_framework(cedra_framework);
    ...
    public fun enable_token(cedra_framework: &signer, token: Object<Metadata>) acquires FAConfig, FAController {
        system_addresses::assert_cedra_framework(cedra_framework);
    ...
    public fun disable_token(cedra_framework: &signer, token: Object<Metadata>) acquires FAConfig, FAController {
        system_addresses::assert_cedra_framework(cedra_framework);
    ...
    public fun is_allow_list_enabled(): bool acquires FAController {
        borrow_global<FAController>(@cedra_experimental).allow_list_enabled
    }
    ...
    fun get_fa_store_signer(): signer acquires FAController {
        object::generate_signer_for_extending(&borrow_global<FAController>(@cedra_experimental).extend_ref)
    }

    /// Returns the address that handles all the FA primary stores.
    fun get_fa_store_address(): address acquires FAController {
        object::address_from_extend_ref(&borrow_global<FAController>(@cedra_experimental).extend_ref)
    }
    ...
    fun get_fa_config_signer(token: Object<Metadata>): signer acquires FAController {
        let fa_ext = &borrow_global<FAController>(@cedra_experimental).extend_ref;
