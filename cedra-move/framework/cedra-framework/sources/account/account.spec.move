spec cedra_framework::account {
    ...
    /// to the OriginatingAddress under the cedra_framework account.
    ...
    /// Only the address `@cedra_framework` can call.
    /// OriginatingAddress does not exist under `@cedra_framework` before the call.
    spec initialize(cedra_framework: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(cedra_addr);
        ensures exists<OriginatingAddress>(cedra_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @cedra_framework || new_address == @cedra_token;
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@cedra_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@cedra_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@cedra_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@cedra_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@cedra_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@cedra_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
