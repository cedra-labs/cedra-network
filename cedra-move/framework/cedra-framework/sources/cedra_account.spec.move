spec cedra_framework::cedra_account {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the creation of an Cedra account the following rules should hold: (1) the authentication key
    /// should be 32 bytes in length, (2) an Cedra account should not already exist for that authentication key, and (3)
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Cedra account, the account should become registered to receive CedraCoin.
    /// Criticality: Critical
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @cedra_framework || auth_key == @cedra_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        /// [high-level-req-5]
        ensures exists<cedra_framework::account::Account>(to);
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @cedra_framework || recipients[i] == @cedra_token);
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @cedra_framework || to == @cedra_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<cedra_framework::account::Account>(to);
