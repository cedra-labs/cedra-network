spec cedra_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<CedraCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<CedraCoin> { account: staker };
        let balance = global<coin::CoinStore<CedraCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<CedraCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@cedra_framework);
        requires exists<stake::ValidatorSet>(@cedra_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @cedra_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<cedra_framework::timestamp::CurrentTimeMicroseconds>(@cedra_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@cedra_framework);
        aborts_if exists<stake::AllowedValidators>(@cedra_framework) && !contains(allowed.accounts, resource_addr);
