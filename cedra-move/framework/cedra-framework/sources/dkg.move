...
module cedra_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use cedra_framework::event::emit;
    use cedra_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@cedra_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@cedra_framework)) {
            borrow_global<DKGState>(@cedra_framework).in_progress
        } else {
            option::none()
        }
    }
