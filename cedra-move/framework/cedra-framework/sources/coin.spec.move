spec cedra_framework::coin {
    ...
    /// Can only be updated by `@cedra_framework`.
    spec allow_supply_upgrades(_cedra_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@cedra_framework)) {
            let map = global<CoinConversionMap>(@cedra_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@cedra_framework`.
    ...
    // `account` must be `@cedra_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@cedra_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@cedra_framework);
