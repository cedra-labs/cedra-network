...
use cedra_framework::fungible_asset::{Self, MintRef, TransferRef, BurnRef, Metadata, FungibleAsset};
    ...
    public fun get_metadata(symbol: vector<u8>): Object<Metadata> {
        let asset_address = object::create_object_address(&@cedra_framework, symbol);
    ...
    public entry fun transfer(admin: &signer, from: address, to: address, amount: u64, symbol: vector<u8>) acquires ManagedFungibleAsset, State {
        let asset = get_metadata(symbol);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let from_wallet = primary_fungible_store::primary_store(from, asset);
        let to_wallet = primary_fungible_store::ensure_primary_store_exists(to, asset);
        let fa = withdraw(from_wallet, amount, transfer_ref, symbol);
        deposit(to_wallet, fa, transfer_ref,symbol);
    }

    /// Burn fungible assets as the owner of metadata object.
    public entry fun burn(admin: &signer, from: address, amount: u64, symbol: vector<u8>) acquires ManagedFungibleAsset {
        let asset = get_metadata(symbol);
    ...
    public entry fun freeze_account(admin: &signer, account: address, symbol: vector<u8>) acquires ManagedFungibleAsset {
        let asset = get_metadata(symbol);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
    ...
    public entry fun unfreeze_account(admin: &signer, account: address, symbol: vector<u8>) acquires ManagedFungibleAsset {
        let asset = get_metadata(symbol);
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let wallet = primary_fungible_store::ensure_primary_store_exists(account, asset);
    ...
    public entry fun set_pause(pauser: &signer, paused: bool, symbol: vector<u8>) acquires State {
        let asset = get_metadata(symbol);
        assert!(object::is_owner(asset, signer::address_of(pauser)), error::permission_denied(ENOT_OWNER));
        let state = borrow_global_mut<State>(object::create_object_address(&@cedra_framework,symbol));
    ...
    /// OPTIONAL
    fun assert_not_paused(symbol: vector<u8>) acquires State {
        let state = borrow_global<State>(object::create_object_address(&@cedra_framework, symbol));
