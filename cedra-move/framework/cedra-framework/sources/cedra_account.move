module cedra_framework::cedra_account {
    ...
    use cedra_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    public fun deposit_coins<CoinType>(
        ...
        if (!account::exists_at(to)) {
            ...
            spec {
                ...
                // assume cedra_std::type_info::type_of<CoinType>() == cedra_std::type_info::type_of<CedraCoin>() ==>
    ...
    public fun assert_account_is_registered_for_cedra(addr: address) {
        ...
        assert!(
            coin::is_account_registered<CedraCoin>(addr),
            error::not_found(EACCOUNT_NOT_REGISTERED_FOR_Cedra)
        );
    }
        ...
        if (fungible_asset::store_exists(store_addr)) {
            ...
            object::object_address(
                &primary_fungible_store::create_primary_store(
                    owner, object::address_to_object<Metadata>(@cedra_fungible_asset)
                )
            )
        }
    }
    ...
    inline fun primary_fungible_store_address(account: address): address {
        object::create_user_derived_object_address(account, @cedra_fungible_asset)
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = cedra_framework::cedra_coin::initialize_for_test(core);
