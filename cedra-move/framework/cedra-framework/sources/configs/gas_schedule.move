...
module cedra_framework::gas_schedule {
    use std::bcs;
    use std::error;
    ...
    use cedra_framework::util::from_bytes;
    use cedra_framework::storage_gas::StorageGasConfig;
    ...
    public(friend) fun initialize(cedra_framework: &signer, gas_schedule_blob: vector<u8>) {
        ...
        move_to<GasScheduleV2>(cedra_framework, gas_schedule);
    }
    ...
    public fun set_gas_schedule(cedra_framework: &signer, gas_schedule_blob: vector<u8>) acquires GasSchedule, GasScheduleV2 {
        ...
        if (exists<GasScheduleV2>(@cedra_framework)) {
            ...
            *gas_schedule = new_gas_schedule;
            ...
            let new_gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
            // TODO(Gas): check if gas schedule is consistent
            move_to<GasScheduleV2>(cedra_framework, new_gas_schedule);
        };
    ...
    public fun set_for_next_epoch_check_hash(
        ...
        if (exists<GasScheduleV2>(@cedra_framework)) {
            ...
            let cur_gas_schedule_hash = cedra_hash::sha3_512(cur_gas_schedule_bytes);
            assert!(
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires GasScheduleV2 {
        ...
        if (config_buffer::does_exist<GasScheduleV2>()) {
            ...
            if (exists<GasScheduleV2>(@cedra_framework)) {
                *borrow_global_mut<GasScheduleV2>(@cedra_framework) = new_gas_schedule;
            } else {
