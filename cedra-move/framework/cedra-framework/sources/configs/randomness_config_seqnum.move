...
module cedra_framework::randomness_config_seqnum {
    ...
    public fun initialize(framework: &signer) {
        system_addresses::assert_cedra_framework(framework);
        if (!exists<RandomnessConfigSeqNum>(@cedra_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfigSeqNum {
        ...
        if (config_buffer::does_exist<RandomnessConfigSeqNum>()) {
            ...
            if (exists<RandomnessConfigSeqNum>(@cedra_framework)) {
                *borrow_global_mut<RandomnessConfigSeqNum>(@cedra_framework) = new_config;
            } else {
