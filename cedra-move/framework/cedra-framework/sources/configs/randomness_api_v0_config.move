module cedra_framework::randomness_api_v0_config {
    ...
    public fun on_new_epoch(framework: &signer) acquires RequiredGasDeposit, AllowCustomMaxGasFlag {
        ...
        if (config_buffer::does_exist<RequiredGasDeposit>()) {
            ...
            if (exists<RequiredGasDeposit>(@cedra_framework)) {
                *borrow_global_mut<RequiredGasDeposit>(@cedra_framework) = new_config;
            } else {
        ...
        if (config_buffer::does_exist<AllowCustomMaxGasFlag>()) {
            ...
            if (exists<AllowCustomMaxGasFlag>(@cedra_framework)) {
                *borrow_global_mut<AllowCustomMaxGasFlag>(@cedra_framework) = new_config;
            } else {
                move_to(framework, new_config);
            }
        }
