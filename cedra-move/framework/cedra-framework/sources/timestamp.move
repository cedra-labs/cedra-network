...
module cedra_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(cedra_framework: &signer) {
        system_addresses::assert_cedra_framework(cedra_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(cedra_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by CedraVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@cedra_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@cedra_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@cedra_framework).microseconds
    }
