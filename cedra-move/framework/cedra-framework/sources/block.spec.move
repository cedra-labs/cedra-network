spec cedra_framework::block {
    ...
    /// the module's context, and moved under the Cedra framework account, (2) the block height is set to zero during
    /// initialization, and (3) the epoch interval is greater than zero.
    /// Enforcement: Formally Verified via [high-level-req-1](Initialize).
    ///
    /// No.: 2
    /// Requirement: Only the Cedra framework address may execute the following functionalities: (1) initialize
    /// BlockResource, and (2) update the epoch interval.
    /// Criticality: Critical
    /// Implementation: The initialize and  update_epoch_interval_microsecs functions ensure that only cedra_framework
    /// can call them.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@cedra_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@cedra_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@cedra_framework).new_block_events) == 0;
    ...
    spec initialize(cedra_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@cedra_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@cedra_framework);
        aborts_if !exists<reconfiguration::Configuration>(@cedra_framework);
