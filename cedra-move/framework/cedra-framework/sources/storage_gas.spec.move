spec cedra_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@cedra_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@cedra_framework);
    }
    ...
    spec set_config(cedra_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(cedra_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@cedra_framework);
        aborts_if exists<StorageGas>(@cedra_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@cedra_framework);
        ensures exists<StorageGas>(@cedra_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@cedra_framework);
        aborts_if !exists<StorageGas>(@cedra_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@cedra_framework);
    }
