spec cedra_framework::stake {
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (CedraCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@cedra_framework) ==> validator_set_is_valid();
        // After genesis, `CedraCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CedraCoinCapabilities>(@cedra_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@cedra_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@cedra_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@cedra_framework))) ==>
            old(global<ValidatorSet>(@cedra_framework)) == global<ValidatorSet>(@cedra_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@cedra_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@cedra_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@cedra_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(cedra_framework: &signer) {
        ...
        let cedra_addr = signer::address_of(cedra_framework);
        aborts_if !system_addresses::is_cedra_framework_address(cedra_addr);
        aborts_if exists<ValidatorSet>(cedra_addr);
        aborts_if exists<ValidatorPerformance>(cedra_addr);
        ensures exists<ValidatorSet>(cedra_addr);
        ensures global<ValidatorSet>(cedra_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(cedra_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@cedra_framework);
        aborts_if !exists<ValidatorSet>(@cedra_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@cedra_framework);
        let post p_validator_set = global<ValidatorSet>(@cedra_framework);
    ...
    )
    {
        ...
        let bool_find_validator = !option::spec_is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::spec_is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::spec_is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@cedra_framework);
        ...
        aborts_if !exists<coin::CoinStore<CedraCoin>>(addr);
        include coin::DepositAbortsIf<CedraCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<CedraCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<CedraCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<CedraCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@cedra_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<CedraCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@cedra_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = cedra_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@cedra_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = cedra_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include cedra_framework::cedra_coin::ExistsCedraCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@cedra_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@cedra_framework);
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<CedraCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<CedraCoin>>(addr);
        modifies global<coin::CoinInfo<CedraCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<CedraCoin> { amount: amount };
    }
    ...
    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@cedra_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let cedra = @cedra_framework;
        let pre_validator_set = global<ValidatorSet>(cedra);
        let post validator_set = global<ValidatorSet>(cedra);
        let staking_config = global<staking_config::StakingConfig>(cedra);
    ...
    spec configure_allowed_validators(cedra_framework: &signer, accounts: vector<address>) {
        let cedra_framework_address = signer::address_of(cedra_framework);
        aborts_if !system_addresses::is_cedra_framework_address(cedra_framework_address);
        let post allowed = global<AllowedValidators>(cedra_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@cedra_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<CedraCoinCapabilities>(@cedra_framework);
        requires exists<ValidatorPerformance>(@cedra_framework);
        requires exists<ValidatorSet>(@cedra_framework);
        requires exists<StakingConfig>(@cedra_framework);
        requires exists<StakingRewardsConfig>(@cedra_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@cedra_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@cedra_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = cedra_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = cedra_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate
