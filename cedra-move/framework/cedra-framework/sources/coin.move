...
module cedra_framework::coin {
    ...
    use cedra_framework::optional_aggregator::{Self, OptionalAggregator};
    use cedra_framework::permissioned_signer;
    use cedra_framework::system_addresses;

    use cedra_framework::fungible_asset::{Self, FungibleAsset, Metadata, MintRef, TransferRef, BurnRef};
    use cedra_framework::object::{Self, Object, object_address};
    use cedra_framework::primary_fungible_store;
    use cedra_std::type_info::{Self, TypeInfo};
    use cedra_framework::create_signer;

    friend cedra_framework::cedra_coin;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@cedra_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@cedra_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(cedra_framework: &signer) {
        system_addresses::assert_cedra_framework(cedra_framework);
        if (!exists<CoinConversionMap>(@cedra_framework)) {
            move_to(cedra_framework, CoinConversionMap {
    ...
    inline fun is_cedra<CoinType>(): bool {
        type_info::type_name<CoinType>() == string::utf8(b"0x1::cedra_coin::CedraCoin")
    }

    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_cedra_creation: bool): Object<Metadata> {
        assert!(
            features::coin_to_fungible_asset_migration_feature_enabled(),
            error::invalid_state(EMIGRATION_FRAMEWORK_NOT_ENABLED)
        );
        assert!(exists<CoinConversionMap>(@cedra_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
        let map = borrow_global_mut<CoinConversionMap>(@cedra_framework);
        let type = type_info::type_of<CoinType>();
        if (!table::contains(&map.coin_to_fungible_asset_map, type)) {
            let is_cedra = is_cedra<CoinType>();
            assert!(!is_cedra || allow_cedra_creation, error::invalid_state(ECEDRA_PAIRING_IS_NOT_ENABLED));
            let metadata_object_cref =
                if (is_cedra) {
                    object::create_sticky_object_at_address(@cedra_framework, @cedra_fungible_asset)
                } else {
                    object::create_named_object(
                        &create_signer::create_signer(@cedra_fungible_asset),
    ...
    public fun allow_supply_upgrades(_cedra_framework: &signer, _allowed: bool) {
    ...
    public entry fun migrate_coin_store_to_fungible_store<CoinType>(
        ...
        if (features::new_accounts_default_to_fa_store_enabled() || features::new_accounts_default_to_fa_cedra_store_enabled()) {
    ...
    inline fun can_receive_paired_fungible_asset(
        ...
        features::new_accounts_default_to_fa_store_enabled() || (features::new_accounts_default_to_fa_cedra_store_enabled() && object::object_address(&metadata) == @0xa) || {
    ...
    #[test(source = @0x2, framework = @cedra_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = cedra_framework::fungible_asset)]
    ...
    #[test(framework = @cedra_framework, other = @0x123)]
    ...
    #[test(framework = @cedra_framework)]
    ...
    #[test(framework = @cedra_framework)]
    #[expected_failure(abort_code = 0x20001, location = cedra_framework::aggregator)]
    ...
    #[test(account = @cedra_framework, aaron = @0xcafe)]
    ...
    #[test(account = @cedra_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    fun test_is_account_registered(
        ...
        account::create_account_for_test(aaron_addr);
        account::create_account_for_test(bob_addr);
        let apt_fa_feature = features::get_new_accounts_default_to_fa_cedra_store_feature();
    ...
    #[test(account = @cedra_framework)]
    #[expected_failure(abort_code = 0x50024, location = cedra_framework::fungible_asset)]
