spec cedra_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Cedra Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_cedra_framework function ensures that the provided signer belongs to the
    /// @cedra_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotCedraFramework).
    ///
    /// No.: 3
    ...
    spec assert_cedra_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotCedraFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @cedra_framework with error::PERMISSION_DENIED;
    }
