#[test_only]
module cedra_framework::delegation_pool_integration_tests {
    use std::features;
    use std::signer;

    use cedra_std::bls12381;
    use cedra_std::stake;
    use cedra_std::vector;

    use cedra_framework::account;
    use cedra_framework::cedra_coin::CedraCoin;
    use cedra_framework::coin;
    use cedra_framework::reconfiguration;
    use cedra_framework::delegation_pool as dp;
    ...
    public fun initialize_for_test_custom(
        ...
        features::change_feature_flags_for_testing(cedra_framework, vector[DELEGATION_POOLS, MODULE_EVENT], vector[]);
    }
    ...
    #[expected_failure(abort_code = 0x10007, location = cedra_framework::stake)]
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_Cedra, false, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator, 9900 * ONE_Cedra + 1);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_Cedra, true, true);
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, true, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator_2, 9900 * ONE_Cedra + 1);
    }
    ...
    public entry fun test_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_Cedra, true, true);
    ...
    public entry fun test_active_validator_with_pending_inactive_stake_cannot_add_stake_if_exceeding_max_allowed(
        ...
        dp::unlock(validator, pool_address, 50 * ONE_Cedra);
        stake::assert_validator_state(pool_address, 50 * ONE_Cedra, 0, 0, 50 * ONE_Cedra, 0);
    ...
    public entry fun test_pending_inactive_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_Cedra, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, true, true);
        ...
        // Add 9900 Cedra + 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 Cedra + 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator_1, 9900 * ONE_Cedra + 1);
    }
    ...
    public entry fun test_end_to_end(
        ...
        stake::mint(validator, 900 * ONE_Cedra);
        dp::add_stake(validator, pool_address, 100 * ONE_Cedra);
        assert!(coin::balance<CedraCoin>(validator_address) == 800 * ONE_Cedra, 2);
        stake::assert_validator_state(pool_address, 100 * ONE_Cedra, 0, 100 * ONE_Cedra, 0, 0);
        ...
        stake::assert_validator_state(pool_address, 201 * ONE_Cedra, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_Cedra);
        ...
        dp::withdraw(validator, pool_address, 50 * ONE_Cedra);
        ...
        dp::withdraw(validator, pool_address, 51 * ONE_Cedra);
    ...
    #[test(cedra_framework = @cedra_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x1000D, location = cedra_framework::stake)]
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, false, false);
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_Cedra, false, true);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, false, false);
        ...
        mint_and_add_stake(validator_2, 100 * ONE_Cedra);
        stake::assert_validator_state(validator_2_address, 200 * ONE_Cedra, 0, 0, 0, 0);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator_2, ONE_Cedra);
    }
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_Cedra, true, false);
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit_in_multiple_epochs(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(validator_address, 100 * ONE_Cedra, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 110 * ONE_Cedra, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 121 * ONE_Cedra, 0, 0, 0, 0);
        // Add more than 50% limit. The following line should fail.
        mint_and_add_stake(validator, 99 * ONE_Cedra);
    }
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator, 50 * ONE_Cedra + 1);
    }
    ...
    public entry fun test_active_validator_unlock_partial_stake(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_Cedra);
        stake::assert_validator_state(validator_address, 50 * ONE_Cedra, 0, 0, 50 * ONE_Cedra, 0);
        ...
        stake::assert_validator_state(validator_address, 55 * ONE_Cedra, 55 * ONE_Cedra, 0, 0, 0);
    ...
    public entry fun test_active_validator_can_withdraw_all_stake_and_rewards_at_once(
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_Cedra, 0, 0, 0, 0);
        ...
        dp::unlock(validator, validator_address, 101 * ONE_Cedra);
        stake::assert_validator_state(validator_address, 0, 0, 0, 101 * ONE_Cedra, 0);
    ...
    #[test(cedra_framework = @cedra_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x10006, location = cedra_framework::delegation_pool)]
    public entry fun test_active_validator_unlocking_more_than_available_stake_should_cap(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 200 * ONE_Cedra);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 100 * ONE_Cedra);
        ...
        dp::withdraw(validator, validator_address, 200 * ONE_Cedra);
    ...
    public entry fun test_active_validator_can_reactivate_pending_inactive_stake(
        ...
        dp::reactivate_stake(validator, validator_address, 50 * ONE_Cedra);
        stake::assert_validator_state(validator_address, 100 * ONE_Cedra, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_reactivate_more_than_available_pending_inactive_stake_should_cap(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_Cedra);
        stake::assert_validator_state(validator_address, 50 * ONE_Cedra, 0, 0, 50 * ONE_Cedra, 0);
        dp::reactivate_stake(validator, validator_address, 51 * ONE_Cedra);
        stake::assert_validator_state(validator_address, 100 * ONE_Cedra, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_leaves_staking_but_still_has_a_lockup(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator, 100 * ONE_Cedra, true, false);
        ...
        stake::assert_validator_state(validator_address, 100 * ONE_Cedra, 0, 0, 0, 1);
        end_epoch();
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_Cedra, 0, 0, 0, 1);
    ...
    public entry fun test_pending_inactive_validator_does_not_count_in_increase_limit(
        ...
        mint_and_add_stake(validator_1, 51 * ONE_Cedra);
    }
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_Cedra, false, false);
        ...
        stake::assert_validator_state(validator_1_address, 100 * ONE_Cedra, 0, 0, 0, 0);
        stake::assert_validator_state(validator_2_address, 100 * ONE_Cedra, 0, 0, 0, 1);
        ...
        stake::assert_validator_state(validator_1_address, 101 * ONE_Cedra, 0, 0, 0, 0);
        ...
        stake::assert_validator_state(validator_2_address, 101 * ONE_Cedra, 0, 0, 0, 1);
        assert!(stake::get_validator_state(validator_3_address) == VALIDATOR_STATUS_ACTIVE, 10);
        stake::assert_validator_state(validator_3_address, 100 * ONE_Cedra, 0, 0, 0, 1);

        // Validators without enough stake will be removed.
        dp::unlock(validator_1, validator_1_address, 50 * ONE_Cedra);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        mint_and_add_stake(validator, 100 * ONE_Cedra);
        let pool_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(pool_address, 100 * ONE_Cedra, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_Cedra);
        stake::assert_validator_state(pool_address, 0, 0, 0, 100 * ONE_Cedra, 0);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();

        // Withdraw stake + rewards.
        stake::assert_validator_state(pool_address, 0, 101 * ONE_Cedra, 0, 0, 0);
        dp::withdraw(validator, pool_address, 101 * ONE_Cedra);
        assert!(coin::balance<CedraCoin>(signer::address_of(validator)) == 101 * ONE_Cedra, 1);
    ...
    #[test(cedra_framework = @cedra_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000A, location = cedra_framework::stake)]
    ...
    #[test(cedra_framework = @cedra_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000E, location = cedra_framework::stake)]
    ...
    public entry fun test_staking_validator_index(
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_4, &pop_4, validator_4, 100 * ONE_Cedra, false, false);
        initialize_test_validator(&pk_5, &pop_5, validator_5, 100 * ONE_Cedra, false, false);
    ...
    #[test(cedra_framework = @cedra_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x1000B, location = cedra_framework::stake)]
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        stake::remove_validators(cedra_framework, &vector[validator_to_remove]);
