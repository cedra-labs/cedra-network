...
module cedra_framework::cedra_coin_tests {
    ...
    public fun mint_apt_fa_to_for_test<T: key>(store: Object<T>, amount: u64) {
        fungible_asset::deposit(store, cedra_coin::mint_apt_fa_for_test(amount));
    }
    ...
    public fun mint_apt_fa_to_primary_fungible_store_for_test(
        ...
        primary_fungible_store::deposit(owner, cedra_coin::mint_apt_fa_for_test(amount));
    }

    #[test(cedra_framework = @cedra_framework)]
    fun test_apt_setup_and_mint(cedra_framework: &signer) {
    ...
    fun test_fa_helpers_for_test() {
        ...
        assert!(object::object_exists<Metadata>(@cedra_fungible_asset), 0);
        mint_apt_fa_to_primary_fungible_store_for_test(@cedra_framework, 100);
