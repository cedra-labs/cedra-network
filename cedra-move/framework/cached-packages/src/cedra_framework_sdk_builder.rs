    ...
    pub fn encode(self) -> TransactionPayload {
        ...
        match self {
            ...
            CedraAccountBatchTransfer {
                recipients,
                amounts,
            } => cedra_account_batch_transfer(recipients, amounts),
            CedraAccountBatchTransferCoins {
                coin_type,
                recipients,
                amounts,
            } => cedra_account_batch_transfer_coins(coin_type, recipients, amounts),
            CedraAccountCreateAccount { auth_key } => cedra_account_create_account(auth_key),
            CedraAccountFungibleTransferOnly { to, amount } => {
                cedra_account_fungible_transfer_only(to, amount)
            },
            CedraAccountSetAllowDirectCoinTransfers { allow } => {
                cedra_account_set_allow_direct_coin_transfers(allow)
            },
            ...
            CedraAccountTransferCoins {
                ...
                to,
                amount,
            } => cedra_account_transfer_coins(coin_type, to, amount),
            ...
            CedraGovernanceAddApprovedScriptHashScript { proposal_id } => {
                cedra_governance_add_approved_script_hash_script(proposal_id)
            },
            CedraGovernanceBatchPartialVote {
                stake_pools,
                proposal_id,
                voting_power,
                should_pass,
            } => cedra_governance_batch_partial_vote(
                stake_pools,
                proposal_id,
                voting_power,
                should_pass,
            ...
            CedraGovernanceBatchVote {
                ...
                proposal_id,
                should_pass,
            } => cedra_governance_batch_vote(stake_pools, proposal_id, should_pass),
            CedraGovernanceCreateProposal {
                stake_pool,
                execution_hash,
                metadata_location,
                metadata_hash,
            } => cedra_governance_create_proposal(
                stake_pool,
                execution_hash,
                metadata_location,
                metadata_hash,
            ...
            } => cedra_governance_create_proposal_v2(
                stake_pool,
                execution_hash,
                metadata_location,
                metadata_hash,
            ...
            CedraGovernancePartialVote {
                ...
                voting_power,
                should_pass,
            } => cedra_governance_partial_vote(stake_pool, proposal_id, voting_power, should_pass),
            CedraGovernanceReconfigure {} => cedra_governance_reconfigure(),
            CedraGovernanceVote {
                stake_pool,
                proposal_id,
                should_pass,
            } => cedra_governance_vote(stake_pool, proposal_id, should_pass),
            CodePublishPackageTxn {
                metadata_serialized,
                code,
...
/// Batch version of Cedra transfer.
pub fn cedra_account_batch_transfer(
    recipients: Vec<AccountAddress>,
    amounts: Vec<u64>,
) -> TransactionPayload {
    TransactionPayload::EntryFunction(EntryFunction::new(
...
/// Batch version of transfer_coins.
pub fn cedra_account_batch_transfer_coins(
    coin_type: TypeTag,
    recipients: Vec<AccountAddress>,
    amounts: Vec<u64>,
) -> TransactionPayload {
...
/// Convenient function to transfer a custom CoinType to a recipient account that might not exist.
/// This would create the recipient account first and register it to receive the CoinType, before transferring.
pub fn cedra_account_transfer_coins(
    coin_type: TypeTag,
    to: AccountAddress,
    amount: u64,
) -> TransactionPayload {
...
/// Batch vote on proposal with proposal_id and specified voting power from multiple stake_pools.
pub fn cedra_governance_batch_partial_vote(
    stake_pools: Vec<AccountAddress>,
    proposal_id: u64,
    voting_power: u64,
    should_pass: bool,
...
/// Vote on proposal with proposal_id and all voting power from multiple stake_pools.
pub fn cedra_governance_batch_vote(
    stake_pools: Vec<AccountAddress>,
    proposal_id: u64,
    should_pass: bool,
) -> TransactionPayload {
...
/// @param execution_hash Required. This is the hash of the resolution script. When the proposal is resolved,
/// only the exact script with matching hash can be successfully executed.
pub fn cedra_governance_create_proposal(
    stake_pool: AccountAddress,
    execution_hash: Vec<u8>,
    metadata_location: Vec<u8>,
    metadata_hash: Vec<u8>,
...
/// @param execution_hash Required. This is the hash of the resolution script. When the proposal is resolved,
/// only the exact script with matching hash can be successfully executed.
pub fn cedra_governance_create_proposal_v2(
    stake_pool: AccountAddress,
    execution_hash: Vec<u8>,
    metadata_location: Vec<u8>,
    metadata_hash: Vec<u8>,
...
/// Vote on proposal with `proposal_id` and specified voting power from `stake_pool`.
pub fn cedra_governance_partial_vote(
    stake_pool: AccountAddress,
    proposal_id: u64,
    voting_power: u64,
    should_pass: bool,
...
mod decoder {
    ...
    pub fn cedra_account_fungible_transfer_only(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
    ...
    pub fn cedra_account_set_allow_direct_coin_transfers(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
    ...
    pub fn cedra_coin_claim_mint_capability(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(_script) = payload {
    ...
    pub fn cedra_coin_delegate_mint_capability(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
    ...
    pub fn cedra_governance_add_approved_script_hash_script(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(script) = payload {
            Some(
                EntryFunctionCall::CedraGovernanceAddApprovedScriptHashScript {
    ...
    pub fn cedra_governance_force_end_epoch(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(_script) = payload {
    ...
    pub fn cedra_governance_force_end_epoch_test_only(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(_script) = payload {
    ...
    pub fn transaction_fee_convert_to_cedra_fa_burn_ref(
        payload: &TransactionPayload,
    ) -> Option<EntryFunctionCall> {
        if let TransactionPayload::EntryFunction(_script) = payload {
...
static SCRIPT_FUNCTION_DECODER_MAP: once_cell::sync::Lazy<EntryFunctionDecoderMap> =
    once_cell::sync::Lazy::new(|| {
        ...
        map.insert(
            "cedra_account_batch_transfer".to_string(),
            Box::new(decoder::cedra_account_batch_transfer),
        );
        map.insert(
            "cedra_account_batch_transfer_coins".to_string(),
            Box::new(decoder::cedra_account_batch_transfer_coins),
        );
        map.insert(
            "cedra_account_create_account".to_string(),
            Box::new(decoder::cedra_account_create_account),
        );
        map.insert(
            "cedra_account_fungible_transfer_only".to_string(),
            Box::new(decoder::cedra_account_fungible_transfer_only),
        );
        map.insert(
            "cedra_account_set_allow_direct_coin_transfers".to_string(),
            Box::new(decoder::cedra_account_set_allow_direct_coin_transfers),
        );
        map.insert(
            "cedra_account_transfer".to_string(),
            Box::new(decoder::cedra_account_transfer),
        );
        map.insert(
            "cedra_account_transfer_coins".to_string(),
            Box::new(decoder::cedra_account_transfer_coins),
        );
        map.insert(
            "cedra_coin_claim_mint_capability".to_string(),
            Box::new(decoder::cedra_coin_claim_mint_capability),
        );
        map.insert(
            "cedra_coin_delegate_mint_capability".to_string(),
            Box::new(decoder::cedra_coin_delegate_mint_capability),
        );
        map.insert(
            "cedra_coin_mint".to_string(),
            Box::new(decoder::cedra_coin_mint),
        );
        map.insert(
            "cedra_governance_add_approved_script_hash_script".to_string(),
            Box::new(decoder::cedra_governance_add_approved_script_hash_script),
        );
        map.insert(
            "cedra_governance_batch_partial_vote".to_string(),
            Box::new(decoder::cedra_governance_batch_partial_vote),
        );
        map.insert(
            "cedra_governance_batch_vote".to_string(),
            Box::new(decoder::cedra_governance_batch_vote),
        );
        map.insert(
            "cedra_governance_create_proposal".to_string(),
            Box::new(decoder::cedra_governance_create_proposal),
        );
        map.insert(
            "cedra_governance_create_proposal_v2".to_string(),
            Box::new(decoder::cedra_governance_create_proposal_v2),
        );
        map.insert(
            "cedra_governance_force_end_epoch".to_string(),
            Box::new(decoder::cedra_governance_force_end_epoch),
        );
        map.insert(
            "cedra_governance_force_end_epoch_test_only".to_string(),
            Box::new(decoder::cedra_governance_force_end_epoch_test_only),
        );
        map.insert(
            "cedra_governance_partial_vote".to_string(),
            Box::new(decoder::cedra_governance_partial_vote),
        );
        map.insert(
            "cedra_governance_reconfigure".to_string(),
            Box::new(decoder::cedra_governance_reconfigure),
        );
        map.insert(
            "cedra_governance_vote".to_string(),
            Box::new(decoder::cedra_governance_vote),
        );
        map.insert(
