...
module cedra_framework::primary_fungible_store {
    use cedra_framework::dispatchable_fungible_asset;
    use cedra_framework::fungible_asset::{Self, FungibleAsset, FungibleStore, Metadata, MintRef, TransferRef, BurnRef};
    use cedra_framework::object::{Self, Object, ConstructorRef, DeriveRef};
    ...
    public fun grant_cedra_permission(
        master: &signer,
        permissioned: &signer,
        amount: u64
    ) {
        fungible_asset::grant_permission_by_address(
            master,
            permissioned,
            object::create_user_derived_object_address(signer::address_of(permissioned), @cedra_fungible_asset),
            amount
        );
    }
