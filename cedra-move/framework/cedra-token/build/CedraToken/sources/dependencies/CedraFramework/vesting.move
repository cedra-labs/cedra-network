...
module cedra_framework::vesting {
    use std::bcs;
    use std::error;
    ...
    use cedra_std::pool_u64::{Self, Pool};
    ...
    const VESTING_POOL_SALT: vector<u8> = b"cedra_framework::vesting";
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, Coin<CedraCoin>>,
        vesting_schedule: VestingSchedule,
        withdrawal_address: address,
        operator: address,
        voter: address,
    ...
    public entry fun distribute(contract_address: address) acquires VestingContract {
        ...
        vector::for_each_ref(shareholders, |shareholder| {
            ...
            cedra_account::deposit_coins(recipient_address, share_of_coins);
        });

        // Send any remaining "dust" (leftover due to rounding error) to the withdrawal address.
        if (coin::value(&coins) > 0) {
            cedra_account::deposit_coins(vesting_contract.withdrawal_address, coins);
        } else {
            coin::destroy_zero(coins);
        };
    ...
    fun create_vesting_contract_account(
        ...
        // Register the vesting contract account to receive Cedra as it'll be sent to it when claiming unlocked stake from
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<CedraCoin> {
        ...
        let withdrawn_coins = coin::balance<CedraCoin>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<CedraCoin>(contract_signer, withdrawn_coins)
    }
    ...
    #[test_only]
    use cedra_framework::stake::with_rewards;

    #[test_only]
    use cedra_framework::account::create_account_for_test;
    use cedra_std::math64::min;

    #[test_only]
    const MIN_STAKE: u64 = 100000000000000; // 1M Cedra coins with 8 decimals.

    #[test_only]
    const GRANT_AMOUNT: u64 = 20000000000000000; // 200M Cedra coins with 8 decimals.
    ...
    #[test_only]
    public fun setup(cedra_framework: &signer, accounts: &vector<address>) {
        use cedra_framework::cedra_account::create_account;
        ...
        std::features::change_feature_flags_for_testing(cedra_framework, vector[], vector[std::features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let admin_address = signer::address_of(admin);
        let buy_ins = simple_map::create<address, Coin<CedraCoin>>();
    ...
    #[test(cedra_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            cedra_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<CedraCoin>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<CedraCoin>(shareholder_2_address);
        ...
        assert!(coin::balance<CedraCoin>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<CedraCoin>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<CedraCoin>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<CedraCoin>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<CedraCoin>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }

    #[test(cedra_framework = @0x1, admin = @0x123)]
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(cedra_framework, &vector[admin_address]);
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(cedra_framework: &signer) {
        setup(cedra_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(cedra_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(cedra_framework: &signer) {
        setup(cedra_framework, &vector[]);
    ...
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(cedra_framework: &signer) {
        setup(cedra_framework, &vector[]);
    ...
    #[test(cedra_framework = @0x1, admin = @0x123, shareholder = @0x234)]
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(cedra_framework, &vector[admin_address, shareholder_address]);
    ...
    #[test(cedra_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(cedra_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(coin::balance<CedraCoin>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<CedraCoin>(operator_address) == commission - 1, 1);
    }
    ...
    #[test(cedra_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(cedra_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_commission_percentage_change(
        ...
        assert!(coin::balance<CedraCoin>(operator_address) == expected_commission, 1);
    }
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(cedra_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        ...
        assert!(coin::balance<CedraCoin>(operator_address1) == 0, 1);
        assert!(coin::balance<CedraCoin>(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = coin::balance<CedraCoin>(beneficiary_address);
        ...
        assert!(coin::balance<CedraCoin>(operator_address2) >= expected_commission, 1);
        assert!(coin::balance<CedraCoin>(beneficiary_address) == old_beneficiay_balance, 1);
    }
    ...
    public entry fun test_set_beneficiary_with_unregistered_account(
        ...
        std::features::change_feature_flags_for_testing(cedra_framework, vector[], vector[fa_feature]);
        let admin_address = signer::address_of(admin);
        setup(cedra_framework, &vector[admin_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@1, @2], &vector[GRANT_AMOUNT, GRANT_AMOUNT], admin_address, 0);
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(cedra_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = coin::balance<CedraCoin>(@11);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(cedra_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<CedraCoin>(@11) == vested_amount, 0);
        assert!(coin::balance<CedraCoin>(@12) == 0, 1);
    }

    #[test(cedra_framework = @0x1, admin = @0x123, resetter = @0x234)]
    ...
    #[test(cedra_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
