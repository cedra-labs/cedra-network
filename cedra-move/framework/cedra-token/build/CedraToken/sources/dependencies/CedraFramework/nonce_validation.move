module cedra_framework::nonce_validation {
    use cedra_std::table::{Self, Table};
    use cedra_std::timestamp;
    use cedra_std::big_ordered_map::{Self, BigOrderedMap};
    use cedra_std::cedra_hash::sip_hash_from_value;
    use cedra_std::error;
    ...
    public(friend) fun initialize(cedra_framework: &signer) {
        initialize_nonce_table(cedra_framework);
    }

    public entry fun initialize_nonce_table(cedra_framework: &signer) {
        if (!exists<NonceHistory>(@cedra_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(cedra_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@cedra_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@cedra_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@cedra_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
