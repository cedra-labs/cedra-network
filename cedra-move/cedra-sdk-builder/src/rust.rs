...
/// If `local_types` is true, we generate a file suitable for the Cedra codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the Cedra repository itself (as opposed to generated Cedra types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("cedra_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("cedra_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("cedra_types", vec![
                "AccountAddress",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Cedra `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    cedra_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, cedra_types_version: String) -> Self {
        Installer {
            install_dir,
            cedra_types_version,
        }
...
serde_bytes = "0.11.6"
cedra-types = {{ path = "../cedra-types", version = "{}" }}
"#,
            name, version, self.cedra_types_version,
        )?;
