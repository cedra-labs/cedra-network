...
// This is a hack to recognize CEDRA StdLib, Framework, and Token libs to avoid warnings on some old errors.
// This will be removed after library attributes are cleaned up.
// (See https://github.com/cedra-labs/cedra-network/issues/9410)
fn module_is_in_cedra_libs(module_address: Option<Spanned<Address>>) -> bool {
    const CEDRA_STDLIB_NAME: &str = "cedra_std";
    static CEDRA_STDLIB_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const CEDRA_FRAMEWORK_NAME: &str = "cedra_framework";
    static CEDRA_FRAMEWORK_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const CEDRA_TOKEN_NAME: &str = "cedra_token";
    static CEDRA_TOKEN_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x3").unwrap());
    const CEDRA_TOKEN_OBJECTS_NAME: &str = "cedra_token_objects";
    static CEDRA_TOKEN_OBJECTS_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
    ...
    match &module_address {
        Some(spanned_address) => {
            ...
            match address {
                ...
                {
                    Some(spanned_symbol) => {
                        ((&spanned_symbol.value as &str) == CEDRA_STDLIB_NAME
                            && (spanned_numerical_address.value == *CEDRA_STDLIB_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == CEDRA_FRAMEWORK_NAME
                                && (spanned_numerical_address.value
                                    == *CEDRA_FRAMEWORK_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == CEDRA_TOKEN_NAME
                                && (spanned_numerical_address.value
                                    == *CEDRA_TOKEN_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == CEDRA_TOKEN_OBJECTS_NAME
                                && (spanned_numerical_address.value
                                    == *CEDRA_TOKEN_OBJECTS_NUMERICAL_ADDRESS))
                    },
                    None => false,
                },
...
fn module_(
    ...
    if context.env.flags().warn_of_deprecation_use_in_cedra_libs() {
        context.in_cedra_libs = false;
    } else {
        context.in_cedra_libs = module_is_in_cedra_libs(module_address);
    }
...
fn check_for_deprecated_module_use(context: &mut Context, mident: &ModuleIdent) -> bool {
    ...
    if !warn_deprecation || context.in_deprecated_code || context.in_cedra_libs {
        return false;
    }
