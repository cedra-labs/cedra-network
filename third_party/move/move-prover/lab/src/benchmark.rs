...
pub fn benchmark(args: &[String]) {
    let cmd_line_parser = Command::new("benchmark")
        ...
        .arg(
            Arg::new("cedra-natives")
                .short('a')
                .long("cedra")
                .num_args(0)
                .action(Set)
                .help("whether the cedra-natives should be included."),
        )
        .arg(
    ...
    let use_cedra_natives = matches.contains_id("cedra-natives");
