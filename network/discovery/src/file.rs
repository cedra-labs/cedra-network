...
use cedra_config::config::PeerSet;
#[cfg(test)]
use cedra_logger::spawn_named;
...
mod tests {
    ...
    use cedra_channels::Receiver;
    use cedra_config::{
        config::{Peer, PeerRole},
        network_id::NetworkContext,
    };
    use cedra_event_notifications::DbBackedOnChainConfig;
    use cedra_network::connectivity_manager::{ConnectivityRequest, DiscoverySource};
    ...
    fn create_listener(path: Arc<TempPath>) -> Receiver<ConnectivityRequest> {
        ...
        let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = cedra_channels::new(
            1,
            &cedra_network::counters::PENDING_CONNECTIVITY_MANAGER_REQUESTS,
        );
