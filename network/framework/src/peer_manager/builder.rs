...
use crate::{
    ...
    transport::{self, CedraNetTransport, Connection, CEDRA_TCP_TRANSPORT},
    ProtocolId,
};
use cedra_channels::{self, cedra_channel, message_queues::QueueStyle};
use cedra_config::{config::HANDSHAKE_VERSION, network_id::NetworkContext};
use cedra_crypto::x25519;
use cedra_logger::prelude::*;
#[cfg(any(test, feature = "testing", feature = "fuzzing"))]
use cedra_netcore::transport::memory::MemoryTransport;
use cedra_netcore::transport::{
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: cedra_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: cedra_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: cedra_channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: cedra_channel::Receiver<PeerId, ConnectionRequest>,

    peers_and_metadata: Arc<PeersAndMetadata>,
    upstream_handlers:
        HashMap<ProtocolId, cedra_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>,
...
impl PeerManagerContext {
    ...
    fn add_upstream_handler(
        ...
        channel: cedra_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<CedraNetTransport<MemoryTransport>, NoiseStream<cedra_memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<CedraNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = cedra_channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            cedra_channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> cedra_channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        ...
        let mut cedra_tcp_transport = CEDRA_TCP_TRANSPORT.clone();
        let tcp_cfg = self.get_tcp_buffers_cfg();
        cedra_tcp_transport.set_tcp_buffers(&tcp_cfg);
    ...
    ) -> cedra_channel::Receiver<(PeerId, ProtocolId), ReceivedMessage> {
        // Register the direct send and rpc protocols
        self.transport_context()
            .add_protocols(&config.direct_send_protocols_and_preferences);
        self.transport_context()
