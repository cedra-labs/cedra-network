...
use cedra_crypto::{noise, x25519};
use cedra_infallible::{duration_since_epoch, RwLock};
use cedra_logger::{error, trace};
use cedra_short_hex_str::{AsShortHexStr, ShortHexStr};
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the Cedra validator network, validator peers will only
                ...
                match trusted_peer {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            cedra_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use cedra_config::config::{Peer, PeerRole, RoleType};
    use cedra_crypto::{
