...
fn build_test_peer(
    ...
    PeerHandle,
    MemorySocket,
    cedra_channels::Receiver<TransportNotification<MemorySocket>>,
) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = cedra_channels::new_test(1);
    let (peer_reqs_tx, peer_reqs_rx) =
        cedra_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
async fn assert_disconnected_event(
    ...
    connection_notifs_rx: &mut cedra_channels::Receiver<TransportNotification<MemorySocket>>,
) {
...
#[derive(Clone)]
struct PeerHandle(cedra_channel::Sender<ProtocolId, PeerRequest>);
...
fn test_upstream_handlers() -> (
    ...
    let (sender, receiver) = cedra_channel::new(QueueStyle::FIFO, 100, None);
