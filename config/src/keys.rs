...
impl<T: DeserializeOwned + PrivateKey + ValidCryptoMaterial + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        cedra_crypto::PrivateKey::public_key(&self.key)
    }
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + cedra_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: cedra_crypto::Uniform::generate_for_testing(),
        }
    }
}
